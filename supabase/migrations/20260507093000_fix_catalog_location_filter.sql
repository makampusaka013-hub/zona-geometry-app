-- ─────────────────────────────────────────────────────────────────────────────
-- MIGRATION: FIX CATALOG PRICE SUMMATION (LOCATION FILTERING)
-- ─────────────────────────────────────────────────────────────────────────────
...
-- 2. REBUILD view_katalog_ahsp_lengkap (WITH LOCATION FILTER)
CREATE OR REPLACE VIEW public.view_katalog_ahsp_lengkap 
  WITH (security_invoker = true)
AS
WITH context AS (
  -- Ambil lokasi yang dipilih user saat ini
...
price_resolution AS (
  ...
  -- FILTER LOKASI DISINI: Pastikan hanya mengambil harga di lokasi yang dipilih user
  LEFT JOIN public.master_harga_dasar mhd_mapped ON mhd_mapped.id = gm.mapped_item_id AND mhd_mapped.location_id = ctx.selected_location_id
  LEFT JOIN public.master_harga_dasar mhd_orig ON mhd_orig.kode_item = gm.detail_kode_item AND mhd_orig.location_id = ctx.selected_location_id AND gm.mapped_item_id IS NULL
),
factor_computation AS (
  SELECT
    *,
    CASE 
      -- PRIORITAS 1: Gunakan Faktor dari halaman Sentralisasi (Global Mapping)
      WHEN global_faktor IS NOT NULL AND global_faktor <> 0 THEN global_faktor
      -- PRIORITAS 2: Gunakan Faktor dari AHSP (Raw Factor from CSV)
      WHEN COALESCE(raw_faktor, 1.0) <> 1.0 THEN raw_faktor
      -- PRIORITAS 3: JIKA SATUAN SAMA: Paksa 1.0
  ...
  SELECT
    *,
    CASE 
      ...
      WHEN global_faktor IS NOT NULL AND global_faktor <> 0 THEN global_faktor
      -- PRIORITAS 2: Gunakan Faktor dari AHSP (Raw Factor from CSV)
      WHEN COALESCE(raw_koefisien, 1.0) <> 1.0 THEN raw_koefisien
      -- PRIORITAS 3: JIKA SATUAN SAMA: Paksa 1.0
...
-- 6. REBUILD view_project_resource_summary
...
resolved AS (
  ...
  FROM global_mapping gm
  LEFT JOIN public.master_harga_dasar mhd_mapped ON mhd_mapped.id = gm.mapped_item_id AND mhd_mapped.location_id = gm.loc_id
  ...
  LEFT JOIN public.master_harga_dasar mhd_any ON mhd_any.kode_item = gm.kode_item_dasar AND mhd_loc.id IS NULL AND gm.mapped_item_id IS NULL AND mhd_any.location_id = gm.loc_id
),
factor_computation AS (
  SELECT
    *,
