...
-- =============================================================================
-- MIGRATION: FINAL FIX FOR DOUBLE CONVERSION BUG
-- =============================================================================
...
WITH raw_resources AS (
  ...
  WHERE al.deleted_at IS NULL 
    AND al.analisa_custom IS NULL OR jsonb_array_length(al.analisa_custom) = 0 -- Re-check logic
  
  UNION ALL

  -- B. Dari Snapshots (Fallback)
  SELECT 
    al.project_id,
    p.location_id AS loc_id,
    al.volume AS proj_volume,
...
final_calculation AS (
  SELECT
    *,
    CASE 
      -- FIX: Jika satuan harga dasar SUDAH SAMA dengan satuan target, JANGAN bagi lagi (divisor = 1)
      WHEN LOWER(TRIM(base_satuan)) = LOWER(TRIM(satuan)) THEN 1.0
      -- Jika berbeda, baru gunakan faktor konversi (default 1.0 jika tidak ada)
      ELSE COALESCE(NULLIF(global_faktor, 0), 1.0)
    END AS divisor
...
aggregated AS (
  SELECT
    project_id,
    uraian,
    COALESCE(kode_item, uraian) AS key_item,
    satuan,
    CASE 
