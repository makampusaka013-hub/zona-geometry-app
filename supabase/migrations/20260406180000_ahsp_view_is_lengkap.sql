-- Tambah boolean is_lengkap ke view_katalog_ahsp_lengkap untuk mendeteksi item AHSP yang memiliki komponen kosong (harga 0)
...
WITH detail_calc AS (
  SELECT
    ...
    COALESCE(mhd.tkdn_persen, 0) AS detail_tkdn,
    ...
    ((COALESCE(mhd.harga_satuan, 0) / COALESCE(NULLIF(mk.faktor_konversi, 0::numeric), 1::numeric)) * COALESCE(mad.koefisien, 0)) * (COALESCE(mhd.tkdn_persen, 0) / 100.0) AS nilai_tkdn,
