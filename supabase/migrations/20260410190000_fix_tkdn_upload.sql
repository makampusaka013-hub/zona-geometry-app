-- Perbaikan RPC upload_harga_dasar_csv untuk memastikan sinkronisasi tipe kolom dengan TKDN_PERCENT (bukan tkdn_persen)
DROP FUNCTION IF EXISTS public.upload_harga_dasar_csv(jsonb);
...
begin
  ...
  loop
    ...
    -- Support untuk parameter lama dan baru
    v_tkdn_percent := coalesce(nullif(trim(v_row->>'tkdn_persen'), '')::numeric, nullif(trim(v_row->>'tkdn_percent'), '')::numeric);
    ...
    if exists (select 1 from public.master_harga_dasar m where m.kode_item = v_kode_item) then
      ...
      set
        ...
        tkdn_percent = coalesce(v_tkdn_percent, tkdn_percent),
