-- Migration: 20260505140000_FIX_RPC_AHSP_PARITY
-- Goal: Bring get_ahsp_catalog_v2 RPC in parity with view_katalog_ahsp_lengkap by applying master_konversi global factors.
...
BEGIN
  ...
  official_resolved AS (
    SELECT DISTINCT ON (ob.detail_id)
      ...
      CASE 
        WHEN lower(trim(ob.detail_satuan)) = lower(trim(COALESCE(mhd_mk.satuan, mhd_auto.satuan, ob.detail_satuan))) THEN 1.0
        WHEN ob.detail_faktor <> 1.0 THEN ob.detail_faktor
        ELSE COALESCE(NULLIF(mk.faktor_konversi, 0), 1.0)
      END AS final_faktor
  ...
  official_summary AS (
    SELECT
      ...
      SUM(CASE WHEN upper(left(trim(r.item_kode),1)) = 'L' THEN (r.harga_toko/r.final_faktor)*r.koefisien ELSE 0 END) as upah,
      SUM(CASE WHEN upper(left(trim(r.item_kode),1)) IN ('A','B') THEN (r.harga_toko/r.final_faktor)*r.koefisien ELSE 0 END) as bahan,
      SUM(CASE WHEN upper(left(trim(r.item_kode),1)) = 'M' THEN (r.harga_toko/r.final_faktor)*r.koefisien ELSE 0 END) as alat,
      SUM((r.harga_toko/r.final_faktor)*r.koefisien) as subtotal,
      SUM((r.harga_toko/r.final_faktor)*r.koefisien * (r.tkdn_val/100.0)) as tkdn_sum,
      jsonb_agg(jsonb_build_object(
        'uraian', r.detail_uraian, 'detail_id', r.detail_id, 'kode_item', r.item_kode,
        'satuan', r.detail_satuan, 'koefisien', r.koefisien, 'harga_konversi', (r.harga_toko/r.final_faktor),
        'subtotal', (r.harga_toko/r.final_faktor)*r.koefisien, 'tkdn', r.tkdn_val,
        'jenis_komponen', CASE 
      ...
      BOOL_AND((r.harga_toko/r.final_faktor)*r.koefisien > 0) as complete
