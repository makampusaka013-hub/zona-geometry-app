...
export const exportRab = (project, items) => {
  ...
  items.forEach((it, index) => {
    const jumlah = (safeNum(it.volume) || 0) * (safeNum(it.harga_satuan) || 0);
    totalRAB += jumlah;
    wsData.push([
      index + 1,
...
export const exportMC = (project, items, mcData, mcType) => {
  ...
  items.forEach((it, index) => {
    ...
    const volMc = mcRecord ? safeNum(mcRecord.volume_mc) : 0;
    const jumlahMc = volMc * (safeNum(it.harga_satuan) || 0);
    totalMC += jumlahMc;
