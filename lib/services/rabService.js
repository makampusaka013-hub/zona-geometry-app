...
export const fetchRabData = async (projectId) => {
  try {
    ...
    if (ahspIds.length > 0) {
      ...
      const [mastersRes, overridesRes] = await Promise.all([
        supabase.from('view_katalog_ahsp_lengkap').select('master_ahsp_id, details').in('master_ahsp_id', ahspIds),
        supabase.from('master_harga_custom').select('kode_item, harga_satuan').eq('created_by', currentUserId)
      ]);
    ...
    const { data: allOverrides } = await supabase.from('master_harga_custom').select('*').eq('created_by', currentUserId);
