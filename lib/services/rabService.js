  ...
  try {
    ...
    if (tab === 'proyek' || tab === 'progress' || tab === 'schedule' || tab === 'export') {
      ...
      const [ahspRes, resourceSumRes, overridesRes] = await Promise.all([
        ...
        supabase.from('master_harga_custom').select('*').eq('project_id', projectId)
      ]);
      ...
      // Build a robust map using kode_item as key, filtering out any records with null/empty keys
      const localOverrideMap = {};
      overrides.forEach(o => {
        const key = o.kode_item;
        if (key) {
          localOverrideMap[key] = o;
        }
      });

      const aggregated = {};
      (resourceSum || []).forEach(r => {
        // Ensure k is a string and handle potential nulls from view (though view has COALESCE)
        const k = String(r.key_item || r.uraian || 'NO-REF');
        const ov = localOverrideMap[k];

        if (!aggregated[k]) {
          aggregated[k] = { ...r };
          // If override exists and is not null/undefined (allow 0 if intentionally set, though rare)
          if (ov && ov.harga_satuan !== null && ov.harga_satuan !== undefined) {
            const h = parseFloat(ov.harga_satuan);
            aggregated[k].harga_snapshot = h;
            aggregated[k].tkdn_percent = ov.tkdn_percent ?? r.tkdn_percent;
            aggregated[k].source_table = 'master_harga_custom';
            aggregated[k].overrides_id = ov.id;
            aggregated[k].kontribusi_nilai = (parseFloat(r.total_volume_terpakai) || 0) * h;
            aggregated[k].nilai_tkdn = aggregated[k].kontribusi_nilai * ((ov.tkdn_percent ?? r.tkdn_percent) / 100);
          }
        }
        else {
          const vol = parseFloat(r.total_volume_terpakai) || 0;
          aggregated[k].total_volume_terpakai = (parseFloat(aggregated[k].total_volume_terpakai) || 0) + vol;
          
          if (ov && ov.harga_satuan !== null && ov.harga_satuan !== undefined) {
            const h = parseFloat(ov.harga_satuan);
            const newKontribusi = vol * h;
            aggregated[k].kontribusi_nilai = (parseFloat(aggregated[k].kontribusi_nilai) || 0) + newKontribusi;
            aggregated[k].nilai_tkdn = (parseFloat(aggregated[k].nilai_tkdn) || 0) + (newKontribusi * ((ov.tkdn_percent ?? r.tkdn_percent) / 100));
          } else {
  ...
  try {
    ...
    if (dbResources && dbResources.length > 0) {
      const resources = dbResources.map(r => {
        ...
        const tkdn = ov?.tkdn_percent || r.tkdn_pct || 0;
        return {
          ...r,
          harga_snapshot: harga,
          tkdn: tkdn,
          tkdn_percent: tkdn,
          kontribusi_nilai: (Number(r.total_volume_terpakai) || 0) * harga,
          total_nilai: (Number(r.total_volume_terpakai) || 0) * harga,
          total_tkdn_nilai: (Number(r.total_volume_terpakai) || 0) * harga * (tkdn / 100)
        };
      });
