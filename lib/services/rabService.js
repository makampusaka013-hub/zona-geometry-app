...
export const saveRabData = async (projectId, identityPayload, allLines, deleteMissing = true) => {
  try {
    ...
    // 1. Save/Update Project Identity
    const projectData = {
      ...identityPayload,
      updated_at: new Date().toISOString()
    };
    // Ensure numeric fields are cleaned
    if (projectData.hsp_value) projectData.hsp_value = parseNum(projectData.hsp_value);
    if (projectData.ppn_percent) projectData.ppn_percent = parseNum(projectData.ppn_percent);
    if (projectData.overhead_percent) projectData.overhead_percent = parseNum(projectData.overhead_percent);

    if (currentProjectId && currentProjectId !== 'new') {
      // Tambahkan select().single() agar Supabase melempar error PGRST116 jika proyek (ID) sudah tidak ada di database
      const { data: checkProj, error: projErr } = await supabase
        .from('projects')
        .update(projectData)
      ...
      if (projErr) {
        if (projErr.code === 'PGRST116') {
           throw new Error('Gagal: Proyek Induk tidak ditemukan atau sudah dihapus dari database. Silakan kembali ke menu utama dan buat Proyek Baru.');
        }
        throw projErr;
      }
    } else {
      const { data: newProj, error: projErr } = await supabase
        .from('projects')
        .insert({ ...projectData, user_id: user.id, created_by: user.id })
        .select()
        .single();
      if (projErr) throw projErr;
    ...
    // 2. Proses Line Items (Bulk Upsert) untuk efisiensi ekstrim & pertahankan UUID
    if (currentProjectId && allLines.length > 0) {
      const processedLines = allLines.map(l => {
        const payload = {
          ...
          master_ahsp_id: l.masterAhspId || l.master_ahsp_id || null,
          sort_order: l.sort_order || 0,
          updated_by: user.id
        };

        // HANYA pasang key 'id' jika string UUID valid, mencegah undefined terkirim
        if (typeof l.id === 'string' && l.id.length > 30) {
          payload.id = l.id;
        }
      ...
      // Eksekusi Bulk Upsert secara paralel (Jauh lebih cepat)
      ...
      // Hapus baris yang dibuang oleh user di UI
      if (deleteMissing) {
        const keptIds = processedLines.map(l => l.id).filter(Boolean);
        let delQuery = supabase.from('ahsp_lines').delete().eq('project_id', currentProjectId);
        if (keptIds.length > 0) {
            delQuery = delQuery.not('id', 'in', `(${keptIds.join(',')})`);
        }
        const { error: delErr } = await delQuery;
        if (delErr) console.warn("Peringatan: Gagal menghapus baris lama:", delErr);
      }
    }

    return { projectId: currentProjectId, error: null };
  } catch (error) {
    console.error('Error in manual saveRabData:', error);
    return { error };
  }
};
