...
export const saveRabData = async (projectId, identityPayload, allLines, deleteMissing = true) => {
  try {
    ...
    // 2. Safe Upsert Line Items (Individual processing)
    if (currentProjectId) {
      const results = [];
      for (const l of allLines) {
        const payload = {
          ...
          harga_satuan: parseNum(l.hargaSatuan),
          profit_percent: parseNum(l.profitPercent),
          analisa_custom: l.analisaDetails || [],
          updated_by: user.id
        };

        // If it has an ID, update. Otherwise, insert.
        if (l.id && l.id.length > 10) { // Valid UUID length
          const { error: upErr } = await supabase
            .from('ahsp_lines')
            .update(payload)
            .eq('id', l.id);
          if (upErr) console.error(`Error updating line ${l.uraian}:`, upErr);
        } else {
          const { error: insErr } = await supabase
            .from('ahsp_lines')
            .insert(payload);
          if (insErr) console.error(`Error inserting line ${l.uraian}:`, insErr);
        }
      }
    }
