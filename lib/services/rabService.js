...
export const saveRabData = async (projectId, identityPayload, allLines, deleteMissing = true) => {
  try {
    ...
    // 1. TAHAP PERTAMA: Pastikan Proyek (Induk) Ada atau Dibuat Baru
    if (identityPayload) {
      ...
      // Validasi Ekstra Ketat: Pastikan ID proyek benar-benar kembali dari database
      if (!projData || typeof projData.id !== 'string' || projData.id.length < 30) {
        throw new Error('Fatal: Database gagal mengembalikan ID Proyek yang sah. Proses dibatalkan demi keamanan data.');
      }
    ...
    // Validasi Akhir sebelum menyentuh tabel ahsp_lines
    if (!currentProjectId || currentProjectId === 'new' || currentProjectId === 'null' || currentProjectId === 'undefined') {
        throw new Error("Sistem menolak menyimpan baris RAB karena ID Proyek Induk tidak valid/tidak ditemukan.");
    }
    ...
    if (allLines && allLines.length > 0) {
      const processedLines = allLines.map(l => {
        ...
        const payload = {
          project_id: currentProjectId, // Pasti valid di titik ini
          ...
          updated_by: user.id, // Wajib untuk RLS
        ...
        if (l.id && uuidRegex.test(String(l.id))) {
          payload.id = l.id;
        }
