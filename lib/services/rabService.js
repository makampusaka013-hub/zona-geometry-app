...
export const upsertProject = async (projectId, payload) => {
  try {
    ...
    // Validate payload before saving - Use partial schema for updates
    ...
    // Use Atomic RPC for better consistency and optimistic locking
    const { data, error: rpcErr } = await supabase.rpc('save_project_atomic', {
      p_project_id: projectId === 'new' ? null : projectId,
      p_identity: updatedPayload,
      p_lines: [], // Only updating identity here
      p_delete_missing: false
    });

    if (rpcErr) {
      // Construct a friendly error message for version conflicts
      if (rpcErr.message.includes('Konflik')) {
        ...
        throw new Error(`Konflik Data: Proyek telah diupdate oleh user lain. (Lokal: ${payload.version}, DB: ${current?.version || '?'}). Silakan Refresh.`);
      }
      throw rpcErr;
    }
    ...
    return { data: null, error };
      ...
      // Murni Insert untuk proyek baru tanpa ID
      const { data, error } = await supabase
        .from('projects')
        .insert({ 
          ...updatedPayload, 
 ...
 */
export const getMaxLumpsumSuffix = async (projectId) => {
  try {
...
export const fetchRabData = async (projectId) => {
  try {
    ...
    const resourceMap = {};
    const { data: allOverrides } = await supabase.from('master_harga_custom').select('*');
    ...
    (lines || []).forEach(line => {
      ...
      details.forEach(det => {
        ...
        if (!resourceMap[kode]) {
          const ov = overrideMap[kode] ? { harga_satuan: overrideMap[kode] } : null; // simplified lookup
          ...
          resourceMap[kode] = {
            ...
            satuan: det.satuan,
            jenis_komponen: j,
            harga_snapshot: overrideMap[kode] || det.harga_konversi || det.harga || 0,
            tkdn: det.tkdn || 0,
            tkdn_percent: det.tkdn || 0,
  ...
  try {
    ...
    if (allLines && allLines.length > 0) {
      const processedLines = allLines.map(l => {
        const { start_date, ...rest } = l; // BUANG start_date karena kolom ini tidak ada di tabel ahsp_lines
        return {
          ...rest,
...
/**
 * Save a single Lumpsum item to master catalog, scoped to a project.
 */
export const saveLumpsumToMaster = async (projectId, item) => {
  try {
    const { max } = await getMaxLumpsumSuffix(projectId);
...
/**
 * Delete a project and all its related data (Cascade delete usually handles this in DB, but we do it manually if needed).
 */
export const deleteProject = async (projectId) => {
  try {
    const { data: { user } } = await supabase.auth.getUser();
    if (!user) throw new Error('Anda harus login untuk menghapus data.');

    // We rely on ON DELETE CASCADE in the database. 
    // If not configured, we would need to delete lines, progress, etc. first.
