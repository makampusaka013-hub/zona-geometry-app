  ...
  try {
    ...
    if (currentProjectId) {
      const { error: projErr } = await supabase
        .from('projects')
        .update({ ...identityPayload, updated_at: new Date().toISOString() })
        .eq('id', currentProjectId);
      if (projErr) throw projErr;
    } else {
      const { data: newProj, error: projErr } = await supabase
        .from('projects')
        .insert({ ...identityPayload, created_by: user.id })
        .select()
        .single();
      if (projErr) throw projErr;
    ...
    if (allLines.length > 0) {
      const processedLines = allLines.map(l => ({
        ...
        bab_pekerjaan: l.bab_pekerjaan || 'UMUM',
        master_ahsp_id: l.masterAhspId,
        master_ahsp_kode: l.masterAhspKode,
        uraian: l.uraian,
        satuan: l.satuan,
      ...
      // For simplicity in manual mode, we upsert based on ID if exists
      for (const line of processedLines) {
         const lineToSave = { ...line };
         const rowId = allLines.find(al => al.uraian === line.uraian)?.id;
         
         if (rowId) {
            await supabase.from('ahsp_lines').update(lineToSave).eq('id', rowId);
         } else {
            await supabase.from('ahsp_lines').insert(lineToSave);
         }
      }
    }
