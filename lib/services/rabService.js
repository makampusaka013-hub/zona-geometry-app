...
export const upsertProject = async (projectId, payload) => {
  try {
    ...
    if (projectId) {
      ...
      // Handle legacy NULL version in DB by allowing match if local is 1
      let query = supabase
        .from('projects')
      ...
      if (error) {
        if (error.code === 'PGRST116') {
          // Cari tahu versi aslinya untuk pesan error yang membantu
          const { data: latest } = await supabase.from('projects').select('version').eq('id', projectId).maybeSingle();
          const dbVer = latest?.version ?? 'NULL';
          throw new Error(`Konflik Data: Proyek telah diupdate oleh user lain. (Versi Lokal: ${currentVersion}, Database: ${dbVer}). Silakan refresh halaman untuk mengambil data terbaru.`);
        }
        throw error;
      }
      return { data, error: null };
  ...
  try {
    ...
    if (allLines && allLines.length > 0) {
      const processedLines = allLines.map(l => ({
          ...l,
          project_id: currentProjectId, // Pasti valid
