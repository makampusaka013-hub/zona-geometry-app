...
export const saveRabData = async (projectId, identityPayload, allLines, deleteMissing = true) => {
  try {
    ...
    // 2. Clear & Re-insert Line Items (Cleanest way for manual save without RPC)
    if (currentProjectId) {
      // Delete old lines
      ...
      if (delErr) console.warn('Delete old lines error:', delErr);
      ...
      if (allLines.length > 0) {
        const processedLines = allLines.map(l => ({
          ...
          master_ahsp_id: l.masterAhspId || null,
          master_ahsp_kode: l.masterAhspKode || null,
          uraian: l.uraian,
          satuan: l.satuan,
