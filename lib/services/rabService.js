...
export const upsertProject = async (projectId, payload) => {
  try {
    ...
    if (projectId && projectId !== 'new' && projectId !== 'null' && projectId !== 'undefined') {
      // Simpan Biasa: Langsung update berdasarkan ID tanpa cek versi yang ribet
      const { data, error } = await supabase
        .from('projects')
        .update({
          ...updatedPayload,
          updated_at: new Date().toISOString(),
          version: (payload.version || 0) + 1 // Tetap naikkan versi tapi jangan dijadikan syarat (eq)
        })
        .eq('id', projectId)
        .select()
        .maybeSingle();
      ...
      if (error) {
        console.error('Error saving project identity:', error);
        return { data: null, error };
      }
      return { data, error: null };
    }
