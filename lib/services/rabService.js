...
export const saveRabData = async (projectId, identityPayload, allLines, deleteMissing = true) => {
  try {
    ...
    // RBAC Check for existing projects
    if (projectId) {
      const [{ data: proj }, { data: memberSlot }, { data: memberInfo }] = await Promise.all([
        supabase.from('projects').select('created_by').eq('id', projectId).single(),
        supabase.from('project_members').select('slot_role').eq('project_id', projectId).eq('user_id', user.id).maybeSingle(),
        supabase.from('members').select('*').eq('user_id', user.id).single()
      ]);
      
      if (!canEditProject(memberInfo, proj?.created_by, memberSlot?.slot_role)) {
        throw new Error('Anda tidak memiliki izin untuk mengedit proyek ini.');
      }
    }

    // Call Atomic Save RPC
    const { data, error: rpcError } = await supabase.rpc('save_project_atomic', {
      p_project_id: projectId || null,
      p_identity: identityPayload,
      p_lines: allLines,
      p_delete_missing: deleteMissing,
      p_client_id: (await import('@/lib/supabase')).clientId
    });

    if (rpcError) throw rpcError;

    return { projectId: data.project_id, error: null };
  } catch (error) {
