...
export const fetchRabData = async (projectId) => {
  try {
    ...
    // 4. Aggregate Resources (for Data Terpakai Tab)
    const resourceMap = {};
      ...
      details.forEach(det => {
        ...
        if (!resourceMap[kode]) {
          ...
          resourceMap[kode] = {
            ...
            key_item: kode,
            satuan: det.satuan,
            jenis_komponen: (det.jenis_komponen || '').toLowerCase(),
            harga_snapshot: ov ? ov.harga_satuan : (det.harga_konversi || det.harga || 0),
            tkdn_percent: ov ? ov.tkdn_percent : (det.tkdn || 0),
        ...
        res.kontribusi_nilai += (itemVol * res.harga_snapshot);
      });
    });
