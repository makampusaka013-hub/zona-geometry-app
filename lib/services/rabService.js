...
export const upsertProject = async (projectId, payload) => {
  try {
    ...
    if (projectId && projectId !== 'new' && projectId !== 'null' && projectId !== 'undefined') {
      // Update existing project using Atomic RPC
      // Primary: Use Atomic RPC
      // Fallback: If RPC fails (e.g. schema cache issue), use direct table update
      let rpcResult;
      try {
      ...
      if (rpcResult?.error) {
        ...
        if (rpcErr.message.includes('Konflik')) {
          ...
          // If versions actually match, the RPC might be misbehaving or there's a transient issue.
          // Try a direct table update as a last resort before giving up.
          if (dbVersion === localVersion) {
            console.log('Versions match but RPC reported conflict. Retrying with direct update...');
            const { data: retryData, error: retryErr } = await supabase
              .from('projects')
              .update({ ...updatedPayload, version: dbVersion + 1 })
              .eq('id', projectId)
          ...
          throw new Error(`Konflik Data: Proyek telah diupdate oleh user lain. (Lokal: ${localVersion}, DB: ${dbVersion}). Silakan Refresh.`);
        }
        
        // If it's a "function not found" error, fallback to direct table update
        if (rpcErr.message.includes('Could not find the function')) {
          ...
          if (!updateData) {
            ...
            if (exists) {
              throw new Error(`Konflik Data: Proyek telah diupdate oleh user lain. (Lokal: ${currentVersion}, DB: ${exists.version || 'NULL'}). Silakan Refresh.`);
            }
            throw new Error('Gagal memperbarui data: Proyek tidak ditemukan.');
          }
