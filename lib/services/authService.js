...
export const authService = {
  ...
  async login(email, password) {
    ...
    try {
      ...
      // Force session refresh to ensure cookies are set before returning
      await supabase.auth.getSession();

      // 2. Fire and Forget Heartbeat (Safe Pattern)
      if (data?.session) {
        supabase.rpc('update_user_heartbeat', { 
          p_session_id: data.session.access_token,
          p_client_type: getClientType()
        }).then(({ error: rpcError }) => {
          if (rpcError) console.warn('Heartbeat non-fatal error:', rpcError);
        });
      }

      // Berhasil: Langsung kembalikan data
