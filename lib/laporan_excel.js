...
const generateLaporanReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  for (let i = 1; i <= totalDays; i++) {
    ...
    sortedBabs.forEach((babName, babIdx) => {
      ...
      const activeLines = linesInBab.filter(line => 
        parseFloat(progMap[line.id] || 0) > 0 || (cumulativeVolumes[line.id] || 0) > 0
      );
  ...
  // CLEAN UP DEFINED NAMES (FIX for "We found a problem with some content")
  // Remove all non-built-in defined names that might be corrupted from template
  workbook.definedNames.model.forEach(nameObj => {
    try {
      // Keep only essential ones if needed, but usually safe to clear custom ones
      // or at least check if they point to valid ranges. 
      // For now, let's clear custom ones that aren't print settings.
      if (!nameObj.name.includes('_FilterDatabase') && !nameObj.name.includes('Print_')) {
        workbook.definedNames.remove(nameObj.name);
      }
    } catch (e) {
      console.warn('Failed to clean defined name:', nameObj.name);
    }
  });
