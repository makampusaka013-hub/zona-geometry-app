...
const generateLaporanReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  [dbHarian, dbItems, dbWork, dbWorkMng, dbWorkBln, dbMngMeta, dbBlnMeta].forEach(ws => ws.state = 'hidden');
  ...
  const cumulativeVolumes = {};  // 1a. Metadata & Labor DB (SMART CALENDAR SYSTEM)
  const dbHarian = workbook.addWorksheet('db_harian_metadata');
  ...
  for (let w = 1; w <= totalWeeks; w++) {
    ...
    dbMngMeta.addRow({ id: w, periode: pStr, rencana: '-', realisasi: weekWeight, deviasi: '-' });
  }
  ...
  worksheets.forEach(ws => {
    ...
    if ((isHarian || isMng || isBln) && !name.includes('DB_')) {
      ...
      const maxVal = isHarian ? totalDays : (isMng ? totalWeeks : totalMonths);
      const validationList = Array.from({ length: maxVal }, (_, i) => i + 1).join(',');
      ...
      if (isHarian) {
        ws.getCell('J2').value = { formula: `VLOOKUP($Q$1, db_harian_metadata!$A:$W, 10, FALSE)` };
        ws.getCell('K51').value = { formula: `J2` };
        ws.getCell('L7').value = { formula: `"MINGGU KE : " & VLOOKUP($Q$1, db_harian_metadata!$A:$W, 3, FALSE)` };
        ws.getCell('L8').value = { formula: `"HARI : " & VLOOKUP($Q$1, db_harian_metadata!$A:$W, 4, FALSE)` };
        ws.getCell('L9').value = { formula: `"TANGGAL : " & VLOOKUP($Q$1, db_harian_metadata!$A:$W, 5, FALSE)` };

        // TENAGA KERJA (JUMLAH di kolom F) - Mandor starts at index 12
        const laborCells = ['F13', 'F14', 'F15', 'F16', 'F17', 'F18', 'F20', 'F21', 'F22'];
        laborCells.forEach((cell, idx) => {
          ws.getCell(cell).value = { formula: `IFERROR(VLOOKUP($Q$1, db_harian_metadata!$A:$W, ${12 + idx}, FALSE), "")` };
        });

        // BAHAN (G: Nama, I: Vol, J: Sat) & ALAT (L: Nama, M: JML)
        for (let j = 1; j <= 11; j++) {
          const rowIdx = 11 + j;
          const itemKey = `$Q$1 & "_" & ${j}`;
          ...
          ws.getCell(`L${rowIdx}`).value = { formula: `IFERROR(VLOOKUP(${itemKey}, db_harian_items!$A:$F, 5, FALSE), "")` };
          ws.getCell(`M${rowIdx}`).value = { formula: `IFERROR(VLOOKUP(${itemKey}, db_harian_items!$A:$F, 6, FALSE), "")` };
        }
      } else {
        // MINGGUAN / BULANAN HEADER INFO (K6:K10)
        const label = isMng ? 'MINGGU' : 'BULAN';
        ws.getCell('I4').value = (project.contractor_name || companyName).toUpperCase();
        ws.getCell('K6').value = { formula: `": " & "${label} KE " & $T$1` };
        ws.getCell('K7').value = { formula: `": " & VLOOKUP($T$1, ${metaSheet}!$A:$E, 2, FALSE)` };
        ws.getCell('K8').value = { formula: `": " & VLOOKUP($T$1, ${metaSheet}!$A:$E, 3, FALSE)` };
        ws.getCell('K9').value = { formula: `": " & VLOOKUP($T$1, ${metaSheet}!$A:$E, 4, FALSE)` };
        ws.getCell('K10').value = { formula: `": " & VLOOKUP($T$1, ${metaSheet}!$A:$E, 5, FALSE)` };
      }
      ...
      for (let j = 1; j <= itemRowsCount; j++) {
        ...
        ws.getCell(`B${r}`).value = { formula: `IFERROR(VLOOKUP(${key}, ${dbName}!$A:$J, 2, FALSE), "")` }; // No
        ws.getCell(`C${r}`).value = { formula: `IFERROR(VLOOKUP(${key}, ${dbName}!$A:$J, 3, FALSE), "")` }; // Uraian
        ...
        if (isHarian) {
          ...
          ws.getCell(`M${r}`).value = { formula: `IFERROR(VLOOKUP(${key}, ${dbName}!$A:$J, 10, FALSE), "")` }; // Bobot %
      ...
      if (headerImageId) {
        const logoBr = isHarian ? { col: 13, row: 1 } : { col: 16, row: 1 };
        ws.addImage(headerImageId, { tl: { col: 1, row: 0 }, br: logoBr, editAs: 'twoCell' });
      }

      // Print Area Dinamis: Sampah baris data terakhir + 5 baris footer tanda tangan
      const lastDataRow = startRow + itemRowsCount + 10; 
      const pArea = isHarian ? '$A$1:$N$72' : `$A$1:$Q$${lastDataRow}`;
