...
const generateLaporanReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  for (let i = 1; i <= totalDays; i++) {
    ...
    ahspLines.forEach((line, wIdx) => {
      ...
      const isSection = !line.satuan && !line.satuan_pekerjaan;
      const kode = line.master_ahsp?.kode_ahsp || line.kode_ahsp || '';
      const combinedName = isSection ? (line.uraian || line.nama_pekerjaan) : (kode ? `(${kode}) ${line.uraian || line.nama_pekerjaan}` : (line.uraian || line.nama_pekerjaan));

      dbWork.addRow({
        key: `${i}_${wIdx + 1}`,
        no: isSection ? (line.no || line.kode_pekerjaan || '') : (line.no || ''),
        name: combinedName,
  ...
  for (let m = 1; m <= totalMonths; m++) {
    ...
    dbBlnMeta.addRow({ id: m, periode: pStr, rencana: '-', realisasi: '-', deviasi: '-' });
      ...
      if (isHarian) {
        ...
        ws.getCell('J51').value = companyName;
        ...
        direksiCells.forEach((cell, idx) => {
          ws.getCell(cell).value = { formula: `=IFERROR(VLOOKUP($P$1, 'db_harian_metadata'!$A:$W, ${18 + idx}, FALSE), "")` };
          ws.getCell(cell).numFmt = '#,##0';
        });
      ...
      for (let j = 1; j <= itemRowsCount; j++) {
        ...
        if (isHarian) {
          // Satuan di J, Volume di K, Bobot di L
          const cellJ = ws.getCell(`J${r}`);
          const cellK = ws.getCell(`K${r}`);
          const cellL = ws.getCell(`L${r}`);

          cellJ.value = { formula: `=IFERROR(VLOOKUP(${key}, '${dbName}'!$A:$J, 4, FALSE), "")` }; 
          cellK.value = { formula: `=IFERROR(VLOOKUP(${key}, '${dbName}'!$A:$J, 8, FALSE), "")` }; // Volume Ini
          cellL.value = { formula: `=IFERROR(VLOOKUP(${key}, '${dbName}'!$A:$J, 10, FALSE), "")` }; // Bobot %
          
          // Alignment: Middle & Center for Unit and Volume
