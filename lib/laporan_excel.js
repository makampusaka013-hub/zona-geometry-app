...
const generateLaporanReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  for (let i = 1; i <= totalDays; i++) {
    ...
    sortedBabs.forEach((babName, babIdx) => {
      ...
      const activeLines = linesInBab.filter(line => {
        ...
        const targetVol = Number(line.volume || 0);

        // 1. Tampilkan jika ada progres hari ini (meskipun baru saja kelar hari ini)
        if (volToday > 0) return true;

        // 2. Jika tidak ada progres hari ini, tampilkan hanya jika sudah pernah ada progres 
        //    DAN belum mencapai 100% (kelar) sebelum hari ini.
        return volLalu > 0 && volLalu < (targetVol - 0.00001);
      });
  ...
  // Removed previous cleanup from here to avoid clearing newly created Print_Areas
