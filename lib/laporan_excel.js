...
const generateLaporanReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  for (let i = 1; i <= totalDays; i++) {
    ...
    // Calculate Date & Day Name automatically
    const currentFullDate = new Date(startDate);
    currentFullDate.setDate(startDate.getDate() + (i - 1));
    
    const formattedDate = currentFullDate.toLocaleDateString('id-ID', { 
      day: '2-digit', month: 'long', year: 'numeric' 
    });
    const formattedDayName = dayNames[currentFullDate.getDay()];
    
    dbHarian.addRow({
      hari_ke: i, minggu_ke: mng, minggu_teks: String(mng),
      hari_nama: formattedDayName, 
  ...
  worksheets.forEach(ws => {
    ...
    if ((isHarian || isMng || isBln) && !name.includes('DB_')) {
      ...
      if (isHarian) {
        ...
        // TENAGA KERJA (JUMLAH di kolom F)
        const laborCells = ['F13', 'F14', 'F15', 'F16', 'F17', 'F18', 'F20', 'F21', 'F22'];
        laborCells.forEach((cell, idx) => {
          ws.getCell(cell).value = { formula: `IFERROR(VLOOKUP($Q$1, db_harian_metadata!$A:$S, ${8 + idx}, FALSE), "")` };
        });
