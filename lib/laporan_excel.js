// Vercel Force Rebuild: 2026-05-04 01:10
...
const generateLaporanReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  // FORCE ALL SHEET NAMES TO LOWERCASE
  workbook.worksheets.forEach(ws => {
    ws.name = ws.name.toLowerCase();
  });

  // CLEAN UP ALL DEFINED NAMES FROM TEMPLATE (Avoid corruption from #REF! names)
  if (workbook.definedNames && workbook.definedNames.model) {
    const allNames = workbook.definedNames.model.map(n => n.name);
    allNames.forEach(name => {
      try { workbook.definedNames.remove(name); } catch (e) {}
    });
  }
  ...
  const workCols = [
    ...
    { header: 'BOBOT_LALU', key: 'bobot_lalu' }, { header: 'BOBOT_INI', key: 'bobot_ini' },
    { header: 'BOBOT_TOTAL', key: 'bobot_total' }, { header: 'IS_BAB', key: 'is_bab' }
  ];
  ...
  for (let i = 1; i <= totalDays; i++) {
    ...
    dbHarian.addRow({
      ...
      konsultan_title: project.konsultan_title || 'Team Leader',
      kontraktor_title: project.kontraktor_title || 'Direktur'
    });
    ...
    sortedBabs.forEach((babName, babIdx) => {
      ...
      // FIX: Show items if they have progress TODAY OR if they already have CUMULATIVE progress
      const activeLines = linesInBab.filter(line => {
        const volToday = parseFloat(progMap[line.id] || 0);
        // Hanya tampilkan pekerjaan yang sedang berjalan (ada progres) di hari tersebut
        return volToday > 0;
      });
      ...
      if (activeLines.length > 0) {
        ...
        linesInBab.forEach(ln => {
          ...
          const itemProgress = (vLalu + vToday) / Number(ln.volume || 1);
          sumWeights += Math.min(1, itemProgress); // Cap at 100%
        });
        const totalBabWeight = linesInBab.length > 0 ? (sumWeights / linesInBab.length) : 0;

        // 1. Add Bab Header Row
        dbWork.addRow({
          key: `${i}_${globalRowIdx}`,
          no: romanize(babIdx + 1), // Use existing romanize helper
          name: babName.toUpperCase(),
          unit: '',
          vol_plan: '',
          vol_ini: '',
  ...
  // POPULATE WEEKLY/MONTHLY (SUMMARIES) - WITH BAB GROUPING
  const totalProjectPrice = ahspLines.reduce((s, r) => s + (Number(r.volume || 0) * Number(r.harga_satuan || r.total_subtotal || 0)), 0) || 1;

  const populateGroupedWork = (ws, startDay, endDay, periodId) => {
    // Group ahspLines by bab_pekerjaan
    const groupedLines = {};
    ahspLines.forEach(line => {
      const bab = line.bab_pekerjaan || 'Lain-lain';
    ...
    sortedBabs.forEach((babName, babIdx) => {
      ...
      // Calculate Bab Weights
      let babBobotLalu = 0, babBobotIni = 0, babBobotTotal = 0;
      linesInBab.forEach(line => {
        let volLalu = 0, volIni = 0;
        for (let d = 1; d < startDay; d++) volLalu += Number(rawDaily[d]?.progressMap?.[line.id] || 0);
        for (let d = startDay; d <= endDay; d++) volIni += Number(rawDaily[d]?.progressMap?.[line.id] || 0);
        const itemPrice = Number(line.harga_satuan || line.total_subtotal || 0);
        babBobotLalu += (volLalu * itemPrice / totalProjectPrice);
        babBobotIni += (volIni * itemPrice / totalProjectPrice);
        babBobotTotal += ((volLalu + volIni) * itemPrice / totalProjectPrice);
      });

      // 1. Add Bab Header
      ws.addRow({
        key: `${periodId}_${globalRowIdx}`,
        no: romanize(babIdx + 1),
        name: babName.toUpperCase(),
        bobot_lalu: babBobotLalu,
        bobot_ini: babBobotIni,
        bobot_total: babBobotTotal,
        is_bab: 'BAB'
      });
      globalRowIdx++;

      // 2. Add Items
      linesInBab.forEach((line, itemIdx) => {
        let volLalu = 0, volIni = 0;
        for (let d = 1; d < startDay; d++) volLalu += Number(rawDaily[d]?.progressMap?.[line.id] || 0);
        for (let d = startDay; d <= endDay; d++) volIni += Number(rawDaily[d]?.progressMap?.[line.id] || 0);
        const itemPrice = Number(line.harga_satuan || line.total_subtotal || 0);
        ...
        ws.addRow({
          ...
          vol_lalu: volLalu,
          vol_ini: volIni,
          vol_total: volLalu + volIni,
          bobot_lalu: volLalu * itemPrice / totalProjectPrice,
          bobot_ini: volIni * itemPrice / totalProjectPrice,
          bobot_total: (volLalu + volIni) * itemPrice / totalProjectPrice,
          is_bab: 'ITEM'
        });
        globalRowIdx++;
      });
  ...
  for (let w = 1; w <= totalWeeks; w++) {
    ...
    populateGroupedWork(dbWorkMng, startDay, endDay, w);

    // Metadata remains simple
    let weekWeight = 0;
    ahspLines.forEach(line => {
      let volToDate = 0;
      for (let d = 1; d <= endDay; d++) volToDate += Number(rawDaily[d]?.progressMap?.[line.id] || 0);
      weekWeight += (volToDate * Number(line.harga_satuan || 0) / totalProjectPrice);
    });

    const dStart = new Date(startDate); dStart.setDate(startDate.getDate() + (w-1)*7);
    const dEnd = new Date(startDate); dEnd.setDate(startDate.getDate() + endDay - startDay);
    const pStr = `${dStart.getDate()} ${monthNames[dStart.getMonth()]} s/d ${dEnd.getDate()} ${monthNames[dEnd.getMonth()]} ${dEnd.getFullYear()}`;
    
    dbMngMeta.addRow({ id: w, periode: pStr, realisasi: isNaN(weekWeight) ? 0 : weekWeight });
  }

  // Populate Monthly
  for (let m = 1; m <= totalMonths; m++) {
    const startDay = (m - 1) * 30 + 1;
    const endDay = Math.min(m * 30, totalDays);
    populateGroupedWork(dbWorkBln, startDay, endDay, m);
    ...
    dbBlnMeta.addRow({ id: m, periode: pStr });
  }
      ...
      worksheets.forEach(ws => {
        ...
        if ((isHarian || isMng || isBln) && !name.includes('DB_')) {
          ...
          // DATA VALIDATION (DISABLED to prevent corruption)
      ...
      if (isHarian) {
        ...
        ws.getCell('E5').value = (project.program_name || '-').toUpperCase();
        ws.getCell('E6').value = (project.activity_name || '-').toUpperCase();
        ws.getCell('E7').value = (project.sub_activity_name || '-').toUpperCase();
        ws.getCell('E8').value = (project.work_name || project.name || '').toUpperCase();
        ws.getCell('E9').value = (project.location || '-').toUpperCase();
        ws.getCell('E10').value = (project.fiscal_year || '-').toUpperCase();
      }

      if (isHarian) {
        ws.getCell('J2').value = companyName; // CV/PT Name in J2
        
        // Stakeholders & Tanda Tangan Laporan (FIXED POSITIONS)
        // KONTRAKTOR
        ws.getCell('K58').value = companyName; 
        ws.getCell('K59').value = project.kontraktor_title || 'Direktur';
        
        // KONSULTAN
        ws.getCell('B58').value = project.konsultan_name || '-';
        ws.getCell('B59').value = project.konsultan_title || 'Team Leader';

        // FORCE ROMAN HEADERS FOR LABOR
        ws.getCell('A12').value = 'I';
        ws.getCell('A19').value = 'II';

        ws.getCell('K7').value = { formula: `=": " & IFERROR(VLOOKUP($P$1, 'db_harian_metadata'!$A:$Y, 3, FALSE), "")` };
        ws.getCell('K8').value = { formula: `=": " & IFERROR(VLOOKUP($P$1, 'db_harian_metadata'!$A:$Y, 4, FALSE), "")` };
        ws.getCell('K9').value = { formula: `=": " & IFERROR(VLOOKUP($P$1, 'db_harian_metadata'!$A:$Y, 5, FALSE), "")` };
      ...
      // WORK ITEMS LOOP (PATEN) - Increased limit to 200 to prevent truncation
      const itemRowsCount = 200; 
      const absId = isHarian ? '$P$1' : '$T$1';
      for (let j = 1; j <= itemRowsCount; j++) {
        const r = startRow + j - 1;
        const key = `${absId} & "_" & ${j}`;
        
        // Nomor di B, Uraian di C
        ws.getCell(`B${r}`).value = { formula: `=IFERROR(VLOOKUP(${key}, '${dbName}'!$A:$N, 2, FALSE), "")` };
        ws.getCell(`C${r}`).value = { formula: `=IFERROR(VLOOKUP(${key}, '${dbName}'!$A:$N, 3, FALSE), "")` };
        
        if (isHarian) {
          // Harian: Satuan di J, Volume di K, Bobot di L
          ws.getCell(`J${r}`).value = { formula: `=IFERROR(VLOOKUP(${key}, '${dbName}'!$A:$N, 4, FALSE), "")` }; 
          ws.getCell(`K${r}`).value = { formula: `=IFERROR(VLOOKUP(${key}, '${dbName}'!$A:$N, 8, FALSE), "")` }; // Volume Ini
          ws.getCell(`L${r}`).value = { formula: `=IFERROR(VLOOKUP(${key}, '${dbName}'!$A:$N, 10, FALSE), "")` }; // Bobot %
        } else {
          // MINGGUAN / BULANAN (Match Template Screenshot)
          // F: Harga Satuan, G: Vol Kontrak, H: Jumlah Harga, I: Bobot Kontrak (%)
          ws.getCell(`F${r}`).value = { formula: `=IFERROR(VLOOKUP(${key}, '${dbName}'!$A:$N, 6, FALSE), "")` };
          ws.getCell(`G${r}`).value = { formula: `=IFERROR(VLOOKUP(${key}, '${dbName}'!$A:$N, 5, FALSE), "")` };
          ws.getCell(`H${r}`).value = { formula: `=IF(OR(F${r}="", G${r}=""), "", F${r}*G${r})` };
          ws.getCell(`I${r}`).value = { formula: `=IFERROR(VLOOKUP(${key}, '${dbName}'!$A:$N, 5, FALSE)*VLOOKUP(${key}, '${dbName}'!$A:$N, 6, FALSE)/${totalProjectPrice}, "")` };
          ws.getCell(`I${r}`).numFmt = '0.00%';

          // J: Vol Lalu, K: Bobot Lalu (%)
          ws.getCell(`J${r}`).value = { formula: `=IFERROR(VLOOKUP(${key}, '${dbName}'!$A:$N, 7, FALSE), "")` };
          ws.getCell(`K${r}`).value = { formula: `=IFERROR(VLOOKUP(${key}, '${dbName}'!$A:$N, 11, FALSE), "")` };
          ws.getCell(`K${r}`).numFmt = '0.00%';

          // L: Vol Ini, M: Bobot Ini (%) -> KOLOM L HARUS FORMAT NUMBER
          ws.getCell(`L${r}`).value = { formula: `=IFERROR(VLOOKUP(${key}, '${dbName}'!$A:$N, 8, FALSE), "")` };
          ws.getCell(`M${r}`).value = { formula: `=IFERROR(VLOOKUP(${key}, '${dbName}'!$A:$N, 12, FALSE), "")` };
          ws.getCell(`M${r}`).numFmt = '0.00%';

          // N: Vol Total, O: Bobot Total (%)
          ws.getCell(`N${r}`).value = { formula: `=IFERROR(VLOOKUP(${key}, '${dbName}'!$A:$N, 9, FALSE), "")` };
          ws.getCell(`O${r}`).value = { formula: `=IFERROR(VLOOKUP(${key}, '${dbName}'!$A:$N, 13, FALSE), "")` };
          ws.getCell(`O${r}`).numFmt = '0.00%';

          // P: Bobot Tertimbang (Sama dengan O)
    ...
    if (!isSelected && !ws.name.includes('DB_')) ws.state = 'veryHidden';
  });
