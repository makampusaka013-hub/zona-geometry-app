...
const generateLaporanReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  worksheets.forEach(ws => {
    ...
    if ((isHarian || isMng || isBln) && !name.includes('DB_')) {
      ...
      if (isHarian) {
        ws.getCell('J2').value = { formula: `IFERROR(VLOOKUP($Q$1, 'db_harian_metadata'!$A:$W, 10, FALSE), "")` };
        ws.getCell('K51').value = { formula: `J2` };
        ws.getCell('L7').value = { formula: `IFERROR(VLOOKUP($Q$1, 'db_harian_metadata'!$A:$W, 3, FALSE), "")` };
        ws.getCell('L8').value = { formula: `IFERROR(VLOOKUP($Q$1, 'db_harian_metadata'!$A:$W, 4, FALSE), "")` };
        ws.getCell('L9').value = { formula: `IFERROR(VLOOKUP($Q$1, 'db_harian_metadata'!$A:$W, 5, FALSE), "")` };
