    ...
    dbBlnMeta.addRow({ id: m, periode: pStr, rencana: '-', realisasi: '-', deviasi: '-' });
      ...
      worksheets.forEach(ws => {
        ...
        if ((isHarian || isMng || isBln) && !name.includes('DB_')) {
          ...
          ws.getCell(idCell).dataValidation = null; // Clear old validation
      ...
      if (isHarian) {
        ws.getCell('I2').value = { formula: `IFERROR(VLOOKUP($P$1, 'db_harian_metadata'!$A:$W, 10, FALSE), "")` };
        ws.getCell('J51').value = { formula: `I2` };
        ...
        const laborCells = ['E13', 'E14', 'E15', 'E16', 'E17', 'E18', 'E20', 'E21', 'E22'];
        laborCells.forEach((cell, idx) => {
          ws.getCell(cell).value = { formula: `IFERROR(VLOOKUP($P$1, 'db_harian_metadata'!$A:$W, ${12 + idx}, FALSE), "")` };
        });
        ...
        for (let j = 1; j <= 11; j++) {
          ...
          ws.getCell(`K${rowIdx}`).value = { formula: `IFERROR(VLOOKUP(${itemKey}, 'db_harian_items'!$A:$F, 5, FALSE), "")` }; // Shifted from L
          ws.getCell(`L${rowIdx}`).value = { formula: `IFERROR(VLOOKUP(${itemKey}, 'db_harian_items'!$A:$F, 6, FALSE), "")` }; // Shifted from M
        }
      } else {
      ...
      for (let j = 1; j <= itemRowsCount; j++) {
        ...
        if (isHarian) {
          ws.getCell(`J${r}`).value = { formula: `IFERROR(VLOOKUP(${key}, '${dbName}'!$A:$J, 8, FALSE), "")` }; // Volume Ini shifted from K
          ws.getCell(`L${r}`).value = { formula: `IFERROR(VLOOKUP(${key}, '${dbName}'!$A:$J, 10, FALSE), "")` }; // Bobot % shifted from M
          ws.getCell(`L${r}`).numFmt = '0.00%';
        } else {
          // Mingguan/Bulanan (No shift as K was not removed here?)
