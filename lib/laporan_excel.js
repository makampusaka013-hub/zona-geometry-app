...
const generateLaporanReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  sheets.forEach(sh => {
    ...
    if (ws) {
      // ID Cell moved to T1 for Weekly/Monthly
      const idCell = 'T1';
      ...
      // Work Items for Weekly/Monthly starts from Row 15
      for (let j = 1; j <= 50; j++) { // Increased range to handle more items
        const r = 14 + j; // Starts from row 15
        const k = `${idCell} & "_" & ${j}`;
