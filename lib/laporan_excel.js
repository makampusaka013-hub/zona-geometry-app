  ...
  for (let m = 1; m <= totalMonths; m++) {
    ...
    dbBlnMeta.addRow({ id: m, periode: pStr, rencana: '-', realisasi: '-', deviasi: '-' });
      ...
      for (let j = 1; j <= itemRowsCount; j++) {
        ...
        cellB.value = { formula: `=IFERROR(VLOOKUP(${key}, '${dbName}'!$A:$J, 2, FALSE), "")` };
        cellC.value = { formula: `=IFERROR(VLOOKUP(${key}, '${dbName}'!$A:$J, 3, FALSE), "")` };
        
        // Alignment: B (Center), C (Left & Middle)
      ...
      // Conditional Bold for C26:C48 and L26:L48 if it's a BAB
      if (isHarian) {
        ws.addConditionalFormatting({
          ...
          rules: [
            {
              type: 'expression',
              formulae: [`VLOOKUP($P$1&"_"&(ROW()-25),'db_harian_work'!$A:$K,11,FALSE)="BAB"`],
