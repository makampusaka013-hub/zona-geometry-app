...
export const generateCoverImage = async (project, user, options = {}) => {
  ...
  // Kita turunkan koordinat Y agar tidak menimpa judul (sekitar pixel 750)
  let currentY = 750; 
  let currentX = 300; // Agak ke kanan agar tidak kena garis segitiga di kiri

  // Nomor Kontrak
  ctx.font = 'bold 35px Arial';
  const projectCode = project.project_code || project.id?.substring(0, 8).toUpperCase() || 'ZG-PRJ-2026';
  ctx.fillText(projectCode, currentX, currentY);
  currentY += 60;

  // Nama Proyek
  ctx.font = 'bold 55px Arial';
  const projectName = (project.work_name || project.name || '-').toUpperCase();
  currentY = wrapText(ctx, projectName, currentX, currentY, width - 450, 70);
  currentY += 30;
  ...
  wrapText(ctx, location, currentX, currentY, width - 450, 45);
  ctx.globalAlpha = 1.0;
