...
export const generateDokumentasiReport = async (project, documentationData, options = {}) => {
  ...
  try {
    ...
    if (allPhotos.length === 0) {
      ...
      // 4. AGGRESSIVE CLEANUP: Delete every single sheet that is NOT 'dokumentasi'
      // We do this by name comparison to be 100% sure.
      const targetName = 'dokumentasi';
      
      // Get a fresh list of all sheets currently in the workbook
      ...
      // 5. Final check and Page Setup
      let finalWs = workbook.getWorksheet(targetName);
      if (!finalWs) {
        // If for some crazy reason it's missing, use the only one left
        finalWs = workbook.worksheets[0];
      }
