...
export const generateDokumentasiReport = async (project, documentationData, options = {}) => {
  ...
  try {
    ...
    for (let b = 0; b < totalBlocks; b++) {
      ...
      // Fill Metadata (D3:D8) with robust mapping
      ws.getCell(`D${offset + 3}`).value = project.program_name || project.nama_program || '-';
      ws.getCell(`D${offset + 4}`).value = project.activity_name || project.nama_kegiatan || '-';
      ws.getCell(`D${offset + 5}`).value = project.sub_activity || project.sub_kegiatan || '-';
      ws.getCell(`D${offset + 6}`).value = project.work_name || project.name || project.uraian_pekerjaan || '-';
      ws.getCell(`D${offset + 7}`).value = project.location || project.lokasi || '-';
      ws.getCell(`D${offset + 8}`).value = project.fiscal_year || project.tahun_anggaran || '-';
