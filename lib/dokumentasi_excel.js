...
export const generateDokumentasiReport = async (project, documentationData, options = {}) => {
  ...
  try {
    ...
    if (allPhotos.length === 0) {
      ...
      // 4. Page Setup (Print Area A:P, Repeat Rows 1:9)
...
async function fillPhotoSlot(workbook, ws, offset, slotIdx, photo) {
  ...
  const colEnd = slotIdx === 0 ? 'E' : (slotIdx === 1 ? 'J' : 'O');
  ...
  if (photo.photo_url) {
    try {
      ...
      ws.addImage(imgId, {
        ...
        ext: { width: 330, height: 210 } // Adjust to fit merge area B13:E23 approx
      });
    } catch (e) {
      console.warn('Failed to load photo:', photo.photo_url, e);
      ws.getCell(`${colStart}${offset + 13}`).value = '[Gagal memuat gambar]';
    }
  }
}
