 ...
 */
const generateDokumentasiReport = async (project, documentationData, options = {}) => {
  ...
  for (const report of documentationData) {
    ...
    ws.mergeCells(`B${currentRow}:D${currentRow}`);
    ws.getCell(`B${currentRow}`).value = `Cuaca: ${report.weather || '-'}`;
    applyBorder(ws, `B${currentRow}:D${currentRow}`);
    currentRow++;
...
export { generateDokumentasiReport };
