...
export function addDays(dateStr, days) {
  if (!dateStr || days == null || isNaN(days)) return null;
  try {
...
export function calculateWorkersAndDuration({
  ...
  const maxRawDurasi = Math.max(...durationsRaw.filter(v => isFinite(v)));
  const finalDurasi = Math.ceil(Math.max(isFinite(maxRawDurasi) ? maxRawDurasi : 1, 1));
