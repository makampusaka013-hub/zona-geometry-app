...
if (selectedSheets.includes('HSP') || selectedSheets.includes('AHSP')) {
  ...
  const wsAHSP = workbook.getWorksheet('ahsp') || workbook.getWorksheet('AHSP');
    ...
    if (wsHSP) {
      ...
      for(let r=1; r<=6; r++) {
        // Only clear Column G to preserve logo/title
        const cellG = wsHSP.getRow(r).getCell(7);
        cellG.border = { top:{style:'none'}, left:{style:'none'}, bottom:{style:'none'}, right:{style:'none'} };
      }
    }
    if (wsAHSP) {
      ...
      for(let r=1; r<=6; r++) {
        // Only clear Column G to preserve logo/title
        const cellG = wsAHSP.getRow(r).getCell(7);
        cellG.border = { top:{style:'none'}, left:{style:'none'}, bottom:{style:'none'}, right:{style:'none'} };
      }
      wsAHSP.getCell('B2').value = null; 
      wsAHSP.getCell('D2').value = null; 
    }
