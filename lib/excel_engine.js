...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  const response = await fetch(`https://raw.githubusercontent.com/makampusaka013-hub/zona-geometry-app/main/public/templates/master_template_custom.xlsx`);
  ...
  // ==========================================
  // 0. Process cover (Pass-through Mode)
  // ==========================================
  // Semua desain dilakukan manual oleh user di template.
  // Sistem hanya memastikan sheet ini tetap ada jika dipilih.
  // ==========================================
  // 0. Process cover (Pure Pass-through Mode)
  // ==========================================
  if (selectedSheets.some(s => s.toLowerCase() === 'cover')) {
    const wsCover = workbook.getWorksheet('cover') || workbook.getWorksheet('COVER') || workbook.getWorksheet('Cover');
    if (wsCover) {
      // Biarkan sheet utuh tanpa diedit (Pure Pass-through untuk menjaga Textbox & Image)
      // Set Print Area sesuai permintaan
      setupPrinter(wsCover, null, 'A1:I44', paperSize, 'portrait');
    }
  }
        ...
        groups.forEach(group => {
          ...
          if (items.length > 0) {
            ...
            items.forEach((r, idx) => {
              ...
              ws.getCell(`I${currentRow}`).value = { formula: `G${currentRow}*H${currentRow}`, result: safeNum(r.totalVolume * r.harga) || 0 };
          ...
          Object.entries(categories).forEach(([catTitle, items], cIdx) => {
            ...
            items.forEach((line) => {
              ...
              if (wsHSP) {
                ...
                if (isStandalone || !selectedSheets.some(s => s.toLowerCase() === 'ahsp')) {
                  ...
                  wsHSP.getCell(`D${hspRow}`).value = { formula: `IFERROR(VLOOKUP(A${hspRow}, ahsp!A:N, 4, FALSE), "")`, result: String(uraianValue || "") };
                  wsHSP.getCell(`E${hspRow}`).value = { formula: `IFERROR(VLOOKUP(A${hspRow}, ahsp!A:N, 6, FALSE), "")`, result: String(satuanValue || "") };
                  wsHSP.getCell(`F${hspRow}`).value = { formula: `ROUND(IFERROR(VLOOKUP(A${hspRow}, ahsp!A:N, 13, FALSE), 0), 0)`, result: safeNum(hargaValue) || 0 };
                  wsHSP.getCell(`G${hspRow}`).value = { formula: `IFERROR(VLOOKUP(A${hspRow}, ahsp!A:N, 14, FALSE), 0)`, result: safeNum(tkdnValue) || 0 };
                }
              ...
              if (wsAHSP) {
                ...
                ahspCats.forEach(cat => {
                  ...
                  filtered.forEach((d) => {
                    ...
                    if (isStandalone || !selectedSheets.some(s => s.toLowerCase() === 'harga satuan')) {
                      ...
                      wsAHSP.getCell(`H${ahspRow}`).value = {
                        ...
                        result: safeNum(itemPrice) || 0
                      };
                    }
                    wsAHSP.getCell(`H${ahspRow}`).numFmt = '#,##0.00';

                    const subtotalFormula = { formula: `G${ahspRow}*H${ahspRow}`, result: safeNum(subtotal) || 0 };
              ...
              wsAHSP.getCell(`M${mainHeaderRow}`).value = { 
                formula: `ROUND((I${mainHeaderRow}+J${mainHeaderRow}+K${mainHeaderRow})*(1+L${mainHeaderRow}), 0)`, 
                result: safeNum(line.rounded_harga) || 0 
              };
    ...
    if (wsRAB) {
      ...
      Object.entries(groupedRAB).forEach(([babTitle, items], bIdx) => {
        ...
        items.forEach((line, iIdx) => {
          ...
          // Otomatis standalone jika sheet rujukan HSP tidak ikut diekspor
          const rabStandalone = isStandalone || !selectedSheets.some(s => s.toLowerCase() === 'hsp');
          ...
          if (rabStandalone) {
            ...
            wsRAB.getCell(`H${rabRow}`).value = { 
              formula: `ROUND(IFERROR(VLOOKUP(C${rabRow}, 'hsp'!D:F, 3, FALSE), 0), 0)`,
              result: safeNum(line.rounded_harga) || 0
            };
            wsRAB.getCell(`J${rabRow}`).value = { 
              formula: `IFERROR(VLOOKUP(C${rabRow}, 'hsp'!D:G, 4, FALSE), 0)`,
              result: safeNum(line.calculated_tkdn) || 0
            };
          }
          ...
          wsRAB.getCell(`I${rabRow}`).value = { 
            ...
            result: safeNum(Math.round(safeNum(line.volume || 0) * line.rounded_harga)) || 0
          };
          wsRAB.getCell(`K${rabRow}`).value = { 
            formula: `I${rabRow}*J${rabRow}`,
            result: safeNum(Math.round(safeNum(line.volume || 0) * line.rounded_harga * line.calculated_tkdn)) || 0
          };
  ...
  if (selectedSheets.includes('REKAP')) {
    ...
    if (wsRekap) {
      ...
      Object.keys(groupedRekap).forEach((babTitle) => {
        ...
        if (!selectedSheets.some(s => s.toLowerCase() === 'rab')) {
      ...
      wsRekap.getCell(`F${totalRow}`).value = { formula: `SUM(F12:F${endDataRow})`, result: 0 };
      ...
      wsRekap.getCell(`F${ppnRow}`).value = { formula: `ROUND(F${totalRow}*${ppnPercent/100}, 0)`, result: 0 };
      
      const grandTotalRow = rekapRow + 2;
      wsRekap.getCell(`C${grandTotalRow}`).value = 'JUMLAH TOTAL HARGA PEKERJAAN';
      wsRekap.getCell(`F${grandTotalRow}`).value = { formula: `ROUND(F${totalRow}+F${ppnRow}, 0)`, result: 0 };
    ...
    if (wsSched) {
      ...
      Object.entries(groupedSched).forEach(([babTitle, items], bIdx) => {
        ...
        items.forEach((item) => {
          ...
          if (isStandalone || !selectedSheets.some(s => s.toLowerCase() === 'rab')) {
          ...
          for(let w = 0; w < itemWeeksSpanned; w++) {
            ...
            if (targetCol <= endCol) {
               wsSched.getCell(currentRow, targetCol).value = { formula: `IFERROR($E$${currentRow}/${itemWeeksSpanned}, 0)`, result: 0 };
      ...
      // tRow sudah dihitung di awal
        ...
        if (isStandalone) {
          ...
          wsSched.getCell(`D${tRow}`).value = { formula: `SUM(D8:D${tRow-1})`, result: safeNum(projectSubtotal) || 0 };
        ...
        for (let c = startCol; c <= endCol; c++) {
          ...
          wsSched.getCell(`${colLetter}${rPM}`).value = { formula: `SUM(${colLetter}8:${colLetter}${tRow-1})`, result: 0 };
          wsSched.getCell(`${colLetter}${rPM}`).numFmt = '0.00%';

          if (c === startCol) wsSched.getCell(`${colLetter}${kPM}`).value = { formula: `${colLetter}${rPM}`, result: 0 };
          else wsSched.getCell(`${colLetter}${kPM}`).value = { formula: `${prevColLetter}${kPM}+${colLetter}${rPM}`, result: 0 };
          ...
          if (c === startCol) wsSched.getCell(`${colLetter}${kA}`).value = { formula: `IF(${colLetter}${aPM}="","",${colLetter}${aPM})`, result: 0 };
          else wsSched.getCell(`${colLetter}${kA}`).value = { formula: `IF(${colLetter}${aPM}="","",${prevColLetter}${kA}+${colLetter}${aPM})`, result: 0 };
          wsSched.getCell(`${colLetter}${kA}`).numFmt = '0.00%';

          wsSched.getCell(`${colLetter}${dev}`).value = { formula: `IF(${colLetter}${kA}="","",${colLetter}${kA}-${colLetter}${kPM})`, result: 0 };
  ...
  // 1. KEMBALIKAN ARRAY INI UNTUK MENAMPUNG ID
  ...
  worksheets.forEach(ws => {
    ...
    if (!isSelected) {
      sheetsToRemove.push(ws.id); // JANGAN LANGSUNG DIHAPUS DI SINI
      return;
    }
    ...
    if (!hasData) {
      ...
      if (name.includes('ahsp')) lastCol = 'O';
      else if (name.includes('hsp')) lastCol = 'I';
      else if (name.includes('terpakai')) lastCol = 'M';
      else if (name === 'harga satuan' || name === 'harga_satuan' || name === 'harga satuan master') lastCol = 'J';
      else if (name.includes('rekap')) lastCol = 'I';
      
      const pArea = `A1:${lastCol}${lastRow}`;
      let orient = 'portrait';
      if (name.includes('schedule') || name.includes('kurva-s')) orient = 'landscape';
