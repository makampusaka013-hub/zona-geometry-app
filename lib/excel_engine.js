...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  const globalOverhead = project.overhead_percent || project.profit_percent || 15;
          ...
          Object.entries(categories).forEach(([catTitle, items], cIdx) => {
            ...
            items.forEach((line) => {
              ...
              if (wsAHSP) {
                ...
                wsAHSP.getCell(`L${mainHeaderRow}`).value = (Number(line.profitPercent || line.overhead_profit || globalOverhead)) / 100;
                wsAHSP.getCell(`L${mainHeaderRow}`).numFmt = '0.00%';
                
                // Atur border C sampai K hanya Atas & Bawah
                for (let i = 3; i <= 11; i++) {
                  wsAHSP.getCell(mainHeaderRow, i).border = { top: { style: 'thin' }, bottom: { style: 'thin' } };
                }
                // Kolom B, F, L, M, N menggunakan kotak penuh (box)
                [2, 6, 12, 13, 14].forEach(colIndex => {
                  wsAHSP.getCell(mainHeaderRow, colIndex).border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } };
                });
    ...
    if (wsRekap) {
      ...
      Object.keys(groupedRekap).forEach((babTitle) => {
        ...
        ['B', 'C', 'D', 'E', 'F', 'G', 'H'].forEach(col => {
          wsRekap.getCell(`${col}${rekapRow}`).border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } };
        });
      ...
      wsRekap.getCell(`C${ppnRow}`).value = `Pajak Pertambahan Nilai ( PPN ) = ${ppnPercent}%`;
      ...
      for(let r = totalRow; r <= grandTotalRow; r++) {
        ...
        // Box border dari B sampai H untuk total
        ['B','C','D','E','F','G','H'].forEach(col => {
          wsRekap.getCell(`${col}${r}`).border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } };
        });
      }
  ...
  if (selectedSheets.some(s => s.toLowerCase() === 'schedule') && Array.isArray(options.scheduleData) && options.scheduleData.length > 0) {
    ...
    if (wsSched) {
      clearDataRows(wsSched, 4, 1000); // Bersihkan dari baris 4
      ...
      // 2. Tulis Header Waktu Dinamis (Baris 4, 5, 6)
      wsSched.mergeCells(4, startCol, 4, endCol);
      wsSched.getCell(4, startCol).value = "WAKTU PELAKSANAAN";
      wsSched.getCell(4, startCol).font = { bold: true };
      wsSched.getCell(4, startCol).alignment = { horizontal: 'center', vertical: 'middle' };
      ...
      for (let w = 0; w < totalWeeks; w++) {
        ...
        // Baris 6: M1, M2, M3, M4 (Reset cycle)
        wsSched.getCell(6, col).value = `M${mCount}`;
        wsSched.getCell(6, col).alignment = { horizontal: 'center' };
        mCount++; if (mCount > 4) mCount = 1;

        // Baris 5: Merge Nama Bulan
        ...
        if (currentMonth !== thisMonth || w === totalWeeks - 1) {
          ...
          if (monthStartCol <= endMerge) {
            wsSched.mergeCells(5, monthStartCol, 5, endMerge);
            const monthNames = ["Januari", "Februari", "Maret", "April", "Mei", "Juni", "Juli", "Agustus", "September", "Oktober", "November", "Desember"];
            wsSched.getCell(5, monthStartCol).value = monthNames[currentMonth];
            wsSched.getCell(5, monthStartCol).alignment = { horizontal: 'center', vertical: 'middle' };
          }
      ...
      wsSched.getCell(4, ketCol).value = "KET";
      wsSched.mergeCells(4, ketCol, 6, ketCol);
      wsSched.getCell(4, ketCol).alignment = { horizontal: 'center', vertical: 'middle' };

      // 3. Tulis Data Baris Item (Mulai Baris 7)
      let currentRow = 7;
      const totalItemRow = 7 + options.scheduleData.length; // Penanda baris JUMLAH nanti

      options.scheduleData.forEach((item, idx) => {
        wsSched.getCell(`B${currentRow}`).value = idx + 1;
        wsSched.getCell(`C${currentRow}`).value = item.uraian;
        
        // D: Harga (VLOOKUP ke sheet RAB. Uraian di RAB = C, Jumlah = I. Index 7)
        wsSched.getCell(`D${currentRow}`).value = { formula: `IFERROR(VLOOKUP(C${currentRow}, 'rab'!C:K, 7, FALSE), 0)` };
        wsSched.getCell(`D${currentRow}`).numFmt = '#,##0.00';

        // E: Bobot % (Harga / Total Harga)
        wsSched.getCell(`E${currentRow}`).value = { formula: `IFERROR(D${currentRow}/D${totalItemRow}, 0)` };
        wsSched.getCell(`E${currentRow}`).numFmt = '0.00%';

        // Distribusi Bobot ke kolom minggu
        const itemDays = Math.ceil((new Date(item.seq_end || maxDate) - new Date(item.seq_start || minDate)) / (1000*60*60*24)) || 1;
        const itemWeeks = Math.ceil(itemDays / 7) || 1;
        const startW = Math.max(0, Math.floor((new Date(item.seq_start || minDate) - minDate) / (1000*60*60*24*7)));
        
        for(let w = 0; w < itemWeeks; w++) {
          const targetCol = startCol + startW + w;
          if (targetCol <= endCol) {
             wsSched.getCell(currentRow, targetCol).value = { formula: `IFERROR($E$${currentRow}/${itemWeeks}, 0)` };
      ...
      wsSched.getCell(`B${tRow}`).font = { bold: true };
      wsSched.getCell(`D${tRow}`).value = { formula: `SUM(D7:D${tRow-1})` };
      wsSched.getCell(`E${tRow}`).value = { formula: `SUM(E7:E${tRow-1})` };
      wsSched.getCell(`D${tRow}`).numFmt = '#,##0.00';
      wsSched.getCell(`E${tRow}`).numFmt = '0.00%';
      
      const rPM = tRow + 1; // RENCANA PROGRESS MINGGUAN
      const kPM = tRow + 2; // KUMULATIF PROGRESS MINGGUAN
      const aPM = tRow + 3; // AKTUAL PROGRESS MINGGUAN
      const kA  = tRow + 4; // KUMULATIF PROGRESS AKTUAL
      const dev = tRow + 5; // DEVIASI

      [rPM, kPM, aPM, kA, dev].forEach(r => {
        wsSched.mergeCells(`B${r}:E${r}`);
        wsSched.getCell(`B${r}`).alignment = { horizontal: 'right' };
        wsSched.getCell(`B${r}`).font = { bold: true };
      });
      ...
      for (let c = startCol; c <= endCol; c++) {
        ...
        wsSched.getCell(`${colLetter}${rPM}`).value = { formula: `SUM(${colLetter}7:${colLetter}${tRow-1})` };
        ...
        // Aktual disiapkan kosong agar bisa diinput manual user
        wsSched.getCell(`${colLetter}${aPM}`).value = ''; 
