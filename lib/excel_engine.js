...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  // 5. Process Schedule
  // ==========================================
  if (selectedSheets.includes('schedule') && Array.isArray(options.scheduleData) && options.scheduleData.length > 0) {
    const wsSched = workbook.getWorksheet('schedule') || workbook.getWorksheet('SCHEDULE') || workbook.getWorksheet('Kurva-S');
    if (wsSched) {
      clearDataRows(wsSched, 6, 1000);
      let currentRow = 6;
      ...
      options.scheduleData.forEach((item, idx) => {
        ...
        wsSched.getCell(`D${currentRow}`).value = Number(item.durasi_hari || 0);
        
        if (item.seq_start) {
          const dStart = new Date(item.seq_start);
          wsSched.getCell(`E${currentRow}`).value = dStart;
          wsSched.getCell(`E${currentRow}`).numFmt = 'dd mmm yyyy';
        }
        if (item.seq_end) {
          const dEnd = new Date(item.seq_end);
          wsSched.getCell(`F${currentRow}`).value = dEnd;
          wsSched.getCell(`F${currentRow}`).numFmt = 'dd mmm yyyy';
        }
        
        // Border B sampai F
        for (let i = 2; i <= 6; i++) {
          wsSched.getCell(currentRow, i).border = {
            top: { style: 'thin' }, 
            left: { style: 'thin' }, 
            bottom: { style: 'thin' }, 
            right: { style: 'thin' }
      ...
      setupPrinter(wsSched, companyName, 'A:I', paperSize, 'landscape');
      if (headerImageId !== null) {
