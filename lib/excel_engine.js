...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  if (selectedSheets.includes('REKAP')) {
    ...
    if (wsRekap) {
      ...
      wsRekap.getCell(`F${grandTotalRow}`).value = { formula: `CEILING(F${totalRow}+F${ppnRow}, 1000)` };
  ...
  if (wsDb) {
    ...
    if (Array.isArray(ahspLines)) {
      ahspLines.forEach(l => {
        totalProjectPrice += (Number(l.volume || 0) * Number(l.harga_satuan || l.total_subtotal || 0));
      });
    }
    ...
    // Header Logs (Mulai dari kolom K)
    const logHeaders = ['ID_ITEM', 'TANGGAL', 'VOL_HARIAN', 'HARI_KE'];
    wsDb.getCell('K1').value = logHeaders[0];
    wsDb.getCell('L1').value = logHeaders[1];
    wsDb.getCell('M1').value = logHeaders[2];
    wsDb.getCell('N1').value = logHeaders[3];
    wsDb.getRow(1).getCell(11).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'CBD5E1' } };
    wsDb.getRow(1).getCell(12).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'CBD5E1' } };
    wsDb.getRow(1).getCell(13).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'CBD5E1' } };
    wsDb.getRow(1).getCell(14).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'CBD5E1' } };
  ...
  worksheets.forEach(ws => {
    ...
    // Proteksi sheet database agar tidak terhapus (karena digunakan untuk VLOOKUP)
  ...
  a.href = url;
  a.download = "HASIL_EKSPOR_ZONA_GEOMETRY.xlsx";
