...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  if (selectedSheets.some(s => s.toLowerCase() === 'cover')) {
    ...
    if (wsCover) {
      ...
      const tags = {
        '<TANGGAL>': new Date().toLocaleString('id-ID', { month: 'long', year: 'numeric' }).toUpperCase(),
        '<KONTRAK>': project.project_code || project.id?.substring(0, 8).toUpperCase() || '',
        '<PROYEK>': (project.work_name || project.name || '').toUpperCase(),
        '<ALAMAT>': (project.location || project.address || '').toUpperCase(),
        '<HARGA>': formatIdr(grandTotal)
      };

      // Cari dan Ganti Tag di seluruh sel
      wsCover.eachRow((row) => {
        row.eachCell((cell) => {
          if (cell.value && typeof cell.value === 'string') {
            ...
            Object.keys(tags).forEach(tag => {
              if (newVal.includes(tag)) {
                newVal = newVal.replace(tag, tags[tag]);
                changed = true;
              }
            });
            if (changed) cell.value = newVal;
          }
        });
      });
