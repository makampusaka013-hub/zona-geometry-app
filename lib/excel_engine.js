...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  // ==========================================
  // 0. Process cover (Auto-Design / Premium Layout)
  // ==========================================
  if (selectedSheets.some(s => s.toLowerCase() === 'cover')) {
    let wsCover = workbook.getWorksheet('cover') || workbook.getWorksheet('COVER') || workbook.getWorksheet('Cover');
    
    // Jika sheet cover tidak ada di template, buat baru otomatis
    if (!wsCover) {
      wsCover = workbook.addWorksheet('cover');
    }

    if (wsCover) {
      // Jika ada gambar hasil Auto-Design dari Canvas
      if (options.coverImage) {
        const coverImageId = workbook.addImage({
          base64: options.coverImage.includes(',') ? options.coverImage.split(',')[1] : options.coverImage,
          extension: 'png',
        });
        wsCover.addImage(coverImageId, {
          tl: { col: 0, row: 0 },
          br: { col: 9, row: 45 }, // Mencakup satu halaman penuh A4
          editAs: 'twoCell'
        });
      } else {
        // Fallback: Layout berbasis Sel (jika Auto-Design tidak ada)
        ...
        const year = project.tahun_anggaran || new Date().getFullYear();
        const month = new Date().toLocaleString('id-ID', { month: 'long' }).toUpperCase();
        
        wsCover.getCell('B3').value = `${month} - ${year}`;
        wsCover.getCell('G2').value = projectCode;
        wsCover.getCell('B5').value = "LAPORAN";
        wsCover.getCell('B6').value = "RENCANA ANGGARAN BIAYA";
        wsCover.getRow(5).font = { size: 28, bold: true };
        wsCover.getRow(6).font = { size: 28, bold: true };
        wsCover.getCell('B10').value = projectCode;
        wsCover.getCell('B11').value = projectName;
        wsCover.getCell('B12').value = location;
        [10, 11, 12].forEach(r => {
          wsCover.getRow(r).font = { size: 12, bold: true };
          wsCover.getCell(`B${r}`).alignment = { horizontal: 'left', wrapText: true };
        });
        wsCover.getCell('G30').value = "Disusun Oleh :";
        wsCover.getCell('G31').value = (user?.full_name || 'ZONA GEOMETRY').toUpperCase();
        wsCover.getCell('G30').alignment = { horizontal: 'right' };
        wsCover.getCell('G31').alignment = { horizontal: 'right' };
        wsCover.getCell('G31').font = { size: 14, bold: true };
        
        const subtotal = enrichedLines.reduce((acc, l) => acc + (Number(l.volume || 0) * l.rounded_harga), 0);
        const ppnVal = Math.round(subtotal * (ppnPercent / 100));
        const grandTotal = subtotal + ppnVal;
        wsCover.getCell('B35').value = `ESTIMASI TOTAL : ${formatIdr(grandTotal)}`;
        wsCover.getCell('B35').font = { size: 10, bold: true };
      }

      setupPrinter(wsCover, companyName, 'A1:H45', paperSize, 'portrait');
    }
  }
  ...
  worksheets.forEach(ws => {
    ...
    if (ws.name.toLowerCase() === 'cover') {
      hasData = true;
      highestRowWithData = 45; // Mencakup layout gambar A4
    } else {
