...
const drawSignatures = (ws, startRow, project) => {
  const p = project || {};
  const city = p.location || 'Kotamobagu';
  const dateStr = new Date().toLocaleDateString('id-ID', { day: 'numeric', month: 'long', year: 'numeric' });

  const r1 = startRow + 2;
  ws.getCell(`H${r1}`).value = `${city}, ${dateStr}`;
  ws.getCell(`H${r1}`).alignment = { horizontal: 'center' };

  const r2 = r1 + 1;
  // Left: Kontraktor, Middle: Konsultan, Right: PPK
  ws.getCell(`C${r2}`).value = p.kontraktor_title || "Penyedia Jasa";
  ws.getCell(`C${r2}`).alignment = { horizontal: 'center' };
  ws.getCell(`F${r2}`).value = p.konsultan_title || "Konsultan Pengawas";
  ws.getCell(`F${r2}`).alignment = { horizontal: 'center' };
  ws.getCell(`I${r2}`).value = "Pejabat Pembuat Komitmen";
  ws.getCell(`I${r2}`).alignment = { horizontal: 'center' };

  const r3 = r2 + 1;
  ws.getCell(`C${r3}`).value = p.kontraktor_name || "";
  ws.getCell(`C${r3}`).alignment = { horizontal: 'center' };
  ws.getCell(`C${r3}`).font = { bold: true };
  
  ws.getCell(`F${r3}`).value = p.konsultan_name || "";
  ws.getCell(`F${r3}`).alignment = { horizontal: 'center' };
  ws.getCell(`F${r3}`).font = { bold: true };

  ws.getCell(`I${r3}`).value = p.department_name || "DINAS PEKERJAAN UMUM";
  ws.getCell(`I${r3}`).alignment = { horizontal: 'center' };
  ws.getCell(`I${r3}`).font = { bold: true };

  const r4 = r3 + 4; // Space for signature
  ws.getCell(`C${r4}`).value = p.kontraktor_director || "";
  ws.getCell(`C${r4}`).font = { bold: true, underline: true };
  ws.getCell(`C${r4}`).alignment = { horizontal: 'center' };

  ws.getCell(`F${r4}`).value = p.konsultan_supervisor || "";
  ws.getCell(`F${r4}`).font = { bold: true, underline: true };
  ws.getCell(`F${r4}`).alignment = { horizontal: 'center' };

  ws.getCell(`I${r4}`).value = p.ppk_name || "";
  ws.getCell(`I${r4}`).font = { bold: true, underline: true };
  ws.getCell(`I${r4}`).alignment = { horizontal: 'center' };

  const r5 = r4 + 1;
  ws.getCell(`I${r5}`).value = p.ppk_nip ? `NIP. ${p.ppk_nip}` : "";
  ws.getCell(`I${r5}`).alignment = { horizontal: 'center' };
