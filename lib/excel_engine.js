              ...
              ahspCats.forEach(cat => {
                ...
                if (filtered.length > 0) {
                  ...
                  mergedFiltered.forEach((d) => {
                    ...
                    // Safety scaling for common material parity (Ton -> kg scale)
                    // Only apply if the data is raw (not already scaled by the DB view)
                    // and matches common large-scale construction materials.
                    const isAlreadyScaled = d.harga_konversi !== undefined && d.harga_konversi !== null && d.harga_konversi !== d.harga_satuan;
                    
                    if (!isAlreadyScaled && rawKoef < 0.1 && (d.satuan?.toLowerCase() === 'kg')) {
                      const u = (d.uraian || d.nama_item || d.uraian_ahsp || "").toLowerCase();
                      if (u.includes('semen') || u.includes('pasir') || u.includes('kerikil') || u.includes('pc')) {
                        koef = rawKoef * 1000;
                      }
                    }
