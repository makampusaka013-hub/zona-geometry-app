...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  if (wsSched && Array.isArray(scheduleData) && scheduleData.length > 0) {
    ...
    let cR = 8; let gI = 0; const tB = Object.keys(gS).length; const tI = scheduleData.length;
    const pS = enrichedLines.reduce((acc, l) => acc + (Number(l.volume || 0) * Number(l.harga_satuan || 0)), 0) || 1;
    
    // Predeteksi baris total untuk rumus
    const tR_estimated = 8 + tB + tI + 1;
    ...
    Object.entries(gS).forEach(([bT, its], bIx) => {
      ...
      cR++; its.forEach((it) => {
        ...
        else { wsSched.getCell(`D${cR}`).value = { formula: `IFERROR(VLOOKUP(C${cR}, 'rab'!C:K, 7, FALSE), 0)`, result: iTV }; wsSched.getCell(`E${cR}`).value = { formula: `IFERROR(D${cR}/D${tR_estimated}, 0)`, result: iBV }; }
