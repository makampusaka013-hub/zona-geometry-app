...
const setupPrinter = (ws, companyName, printArea = null, paperSize = 'A4', orientation = 'portrait', skipTitles = false) => {
  if (!ws) return;
  ...
  if (!skipTitles && printArea) {
    // Validasi: printTitlesRow (5:5) hanya boleh ada jika printArea mencakup baris 5
    const match = printArea.match(/:[A-Z]+(\d+)$/);
    const lastRowInArea = match ? parseInt(match[1], 10) : 0;
    if (lastRowInArea >= 5) {
  ...
  // Tambahkan spasi setelah &KFF8C00 untuk kompatibilitas lebih baik
...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  if (selectedSheets.some(s => s.toLowerCase() === 'cover')) {
    ...
    if (wsCover) {
      ...
      // Printer setup akan ditangani di loop final untuk menghindari duplikasi
    }
  }
  ...
  if (selectedSheets.includes('HARGA SATUAN') || selectedSheets.includes('HARGA SATUAN TERPAKAI')) {
    ...
    if (selectedSheets.includes('HARGA SATUAN')) {
      ...
      if (ws) {
        ...
        // Printer setup akan ditangani di loop final
            ...
            items.forEach((line) => {
              ...
              if (wsHSP) {
                ...
                if (isStandalone) {
                  ...
                  wsHSP.getCell(`D${hspRow}`).value = { formula: `IFERROR(VLOOKUP(A${hspRow}, ahsp!A:N, 4, FALSE), "")`, result: uraianValue };
                  wsHSP.getCell(`E${hspRow}`).value = { formula: `IFERROR(VLOOKUP(A${hspRow}, ahsp!A:N, 6, FALSE), "")`, result: satuanValue };
                  wsHSP.getCell(`F${hspRow}`).value = { formula: `ROUND(IFERROR(VLOOKUP(A${hspRow}, ahsp!A:N, 13, FALSE), 0), 0)`, result: hargaValue };
                  wsHSP.getCell(`G${hspRow}`).value = { formula: `IFERROR(VLOOKUP(A${hspRow}, ahsp!A:N, 14, FALSE), 0)`, result: tkdnValue };
                }
              ...
              if (wsAHSP) {
                ...
                ahspCats.forEach(cat => {
                  ...
                  filtered.forEach((d) => {
                    ...
                    const itemCode = d.kode_item || d.kode || d.id;
                    const itemPrice = Number(priceMap[itemCode] || d.harga_satuan_snapshot || d.harga_satuan || d.harga || 0);
      ...
      Object.entries(groupedRAB).forEach(([babTitle, items], bIdx) => {
        ...
        items.forEach((line, iIdx) => {
          ...
          wsRAB.getCell(`I${rabRow}`).value = { 
            ...
            result: Number(Math.round(Number(line.volume || 0) * line.rounded_harga)) || 0
          };
          wsRAB.getCell(`K${rabRow}`).value = { 
            formula: `I${rabRow}*J${rabRow}`,
            result: Number(Math.round(Number(line.volume || 0) * line.rounded_harga * line.calculated_tkdn)) || 0
          };
  ...
  if (selectedSheets.includes('REKAP')) {
    const wsRekap = workbook.worksheets.find(s => s.name.toLowerCase() === 'rekap rab' || s.name.toLowerCase() === 'rekap' || s.name.toLowerCase() === 'rekapitulasi');
    const wsRABActual = workbook.worksheets.find(s => s.name.toLowerCase() === 'rab');
    const rabName = wsRABActual ? wsRABActual.name : 'RAB';
    ...
    if (wsRekap) {
      ...
      wsRekap.getCell('E9').value = cleanStr(String(project?.tahun_anggaran || project?.fiscal_year || '2026'));
      ...
      Object.keys(groupedRekap).forEach((babTitle) => {
        ...
        wsRekap.getCell(`C${rekapRow}`).value = cleanStr((babTitle || '-').toUpperCase());
        
        // VLOOKUP dari sheet 'rab'
        // C:K -> C=1, D=2, E=3, F=4, G=5, H=6, I(Jumlah)=7, J(TKDN %)=8, K(Jumlah TKDN)=9
        // Gunakan nilai statis jika isStandalone aktif (untuk menghindari error dependensi antar sheet)
        if (!selectedSheets.includes('RAB') || isStandalone) {
          const babLines = groupedRekap[babTitle] || [];
          const babSubtotal = babLines.reduce((s, l) => s + Math.round(Number(l.volume || 0) * l.rounded_harga), 0);
          const babTkdn = babLines.reduce((s, l) => s + Math.round(Number(l.volume || 0) * l.rounded_harga * l.calculated_tkdn), 0);
          ...
          wsRekap.getCell(`F${rekapRow}`).value = { 
            formula: `IFERROR(VLOOKUP("SUB TOTAL "&$C${rekapRow}, '${rabName}'!C:K, 7, FALSE), 0)`,
            result: Number(babSubtotal) || 0
          };
          wsRekap.getCell(`G${rekapRow}`).value = { 
            formula: `IFERROR(VLOOKUP("SUB TOTAL "&$C${rekapRow}, '${rabName}'!C:K, 8, FALSE), 0)`,
            result: Number(babTkdnPercent) || 0
          };
          wsRekap.getCell(`H${rekapRow}`).value = { 
            formula: `IFERROR(VLOOKUP("SUB TOTAL "&$C${rekapRow}, '${rabName}'!C:K, 9, FALSE), 0)`,
            result: Number(babTkdn) || 0
          };
        }
      ...
      wsRekap.getCell(`F${ppnRow}`).value = { formula: `ROUND(F${totalRow}*${ppnPercent/100}, 0)` };
      
      const grandTotalRow = rekapRow + 2;
      wsRekap.getCell(`C${grandTotalRow}`).value = 'JUMLAH TOTAL HARGA PEKERJAAN';
      wsRekap.getCell(`F${grandTotalRow}`).value = { formula: `ROUND(F${totalRow}+F${ppnRow}, 0)` };
    ...
    if (wsSched) {
      ...
      // tRow sudah dihitung di awal
        ...
        if (isStandalone) {
          ...
          wsSched.getCell(`D${tRow}`).value = { formula: `SUM(D8:D${tRow-1})`, result: Number(projectSubtotal) || 0 };
  ...
  workbook.eachSheet(ws => {
    ...
    // Periksa apakah sheet memiliki data aktual
    let highestRowWithData = 0;
    let hasData = false;

    if (lowName === 'cover' || lowName.includes('halaman depan')) {
      hasData = true;
      highestRowWithData = 44; 
    } else {
      ws.eachRow({ includeEmpty: true }, (row, rowNumber) => {
        let rowHasContent = false;
        // Cek kolom B sampai O (2-15)
        for (let i = 2; i <= 15; i++) {
    ...
    if (!hasData) {
      ...
      // Konfigurasi Printer & Area Cetak
      const lastRow = Math.max(highestRowWithData + 1, 10);
      let lastCol = 'K'; 
      ...
      let orient = 'portrait';
      if (lowName.includes('schedule') || lowName.includes('kurva-s') || lowName.includes('gantt')) {
        orient = 'landscape';
      }
      
      const isSpecialSheet = lowName === 'cover' || lowName.includes('halaman depan') || lowName.includes('dokumentasi') || lowName.includes('schedule');
      setupPrinter(ws, companyName, pArea, paperSize, orient, isSpecialSheet);
    }
  });

  // Hapus/Sembunyikan sheet yang tidak terpilih atau kosong
  const role = user?.role || 'normal';
  const isAdvance = role === 'advance' || role === 'pro' || role === 'admin';
  ...
  sheetsToRemove.forEach(id => {
    try { 
      ...
      if (wsDelete) {
        if (isAdvance) {
          // Sangat penting: Untuk template kompleks (Advance), 
          // JANGAN hapus worksheet karena sering merusak XML (Textbox, SharedStrings, DefinedNames).
          // Cukup sembunyikan agar tidak terlihat oleh user.
