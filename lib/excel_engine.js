...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  if (selectedSheets.some(s => s.toLowerCase() === 'schedule') && Array.isArray(options.scheduleData) && options.scheduleData.length > 0) {
    ...
    if (wsSched) {
      ...
      let currentRow = 8;
      const totalItemRow = 8 + options.scheduleData.length; // Penanda baris JUMLAH nanti
      ...
      wsSched.getCell(`D${tRow}`).value = { formula: `SUM(D8:D${tRow-1})` };
      wsSched.getCell(`E${tRow}`).value = { formula: `SUM(E8:E${tRow-1})` };
      ...
      // Dari baris 5 sampai baris Deviasi, dari kolom B sampai kolom KET
