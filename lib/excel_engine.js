...
export async function generateProjectReport(project, member, ahspLines, selectedSheets, extra = {}) {
  ...
  workbook.eachSheet((sheet) => {
      ...
      if (lowerName.includes('harga_satuan') && selectedSheets.includes('HARGA SATUAN')) shouldKeep = true;
      if (lowerName === 'hsp' && selectedSheets.includes('HSP')) shouldKeep = true;
      if (lowerName === 'ahsp' && selectedSheets.includes('AHSP')) shouldKeep = true;
      if (lowerName === 'schedule' && selectedSheets.includes('SCHEDULE')) shouldKeep = true;
      if (lowerName.includes('rab') && selectedSheets.includes('RAB')) shouldKeep = true;
      
      if (shouldKeep) sheetsToKeep.push(sheet.name);
  });

  // ExcelJS needs at least one sheet or it corrupts. Only remove if we have at least one kept.
