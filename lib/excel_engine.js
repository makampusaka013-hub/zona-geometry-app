...
const clearDataRows = (ws, startRow, count) => {
    for (let i = 0; i < count; i++) {
        ...
        // Explicitly clear columns 1 to 20 to catch any column-level borders
        for (let c = 1; c <= 20; c++) {
...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  if (selectedSheets.includes('HARGA SATUAN')) {
    ...
    if (ws) {
      ...
      // Remove any borders from row 1 to 5
      for(let r=1; r<=5; r++) {
        ws.getRow(r).eachCell({ includeEmpty: true }, (cell) => { cell.border = {}; });
      }
      ws.getCell('B2').value = null; // Clear B2
      ...
      groups.forEach(group => {
        ...
        if (items.length > 0) {
          currentRow++;
          ws.getCell(`B${currentRow}`).value = null; // Clear B to match C
...
if (selectedSheets.includes('HSP') || selectedSheets.includes('AHSP')) {
  ...
  const wsAHSP = workbook.getWorksheet('ahsp') || workbook.getWorksheet('AHSP');
    ...
    if (wsHSP) {
      ...
      for(let r=1; r<=6; r++) {
        wsHSP.getRow(r).eachCell({ includeEmpty: true }, (cell) => { cell.border = {}; });
      }
    }
    if (wsAHSP) {
      clearDataRows(wsAHSP, 7, 5000);
      for(let r=1; r<=7; r++) {
        wsAHSP.getRow(r).eachCell({ includeEmpty: true }, (cell) => { cell.border = {}; });
      }
      wsAHSP.getCell('B2').value = null; // Clear B2
      wsAHSP.getCell('D2').value = null; // Clear D2
    }
  ...
  enrichedLines.forEach(line => {
    ...
    Object.entries(groupedItems).forEach(([babTitle, subBabs], bIdx) => {
      ...
      if (wsAHSP) {
        wsAHSP.getCell(`B${ahspRow}`).value = null; // Clear B to match empty G in header
      ...
      Object.entries(subBabs).forEach(([subTitle, categories], sIdx) => {
        if (subTitle) {
          ...
          if (wsAHSP) {
            ahspRow++;
            wsAHSP.getCell(`B${ahspRow}`).value = null; // Match G
        ...
        Object.entries(categories).forEach(([catTitle, items], cIdx) => {
          ...
          items.forEach((line) => {
            ...
            if (wsHSP) {
              // B: Kode AHSP-CK, C: Kode AHSP-Umum (empty), D: Uraian, E: Sat, F: Harga Satuan, G: TKDN, H: Keterangan
              wsHSP.getCell(`B${hspRow}`).value = itemCode; 
              wsHSP.getCell(`C${hspRow}`).value = null; // Explicitly null as requested
