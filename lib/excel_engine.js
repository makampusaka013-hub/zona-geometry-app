...
// Fungsi clearDataRows AMAN (Tanpa Bug Corrupt)
      ...
      if (ws) {
        ...
        if (headerImageId !== null) {
          ws.addImage(headerImageId, {
            ...
            br: { col: 9, row: 4 }, // Sampai I4 agar proporsional
            editAs: 'twoCell'
          });
        }
      }
          ...
          items.forEach((line) => {
            ...
            if (wsAHSP) {
              ...
              if (startDetailRow <= endDetailRow) {
                wsAHSP.getCell('I' + baseCostRow).value = { formula: `SUM(I${startDetailRow}:I${endDetailRow})`, result: 0 };
                wsAHSP.getCell('J' + baseCostRow).value = { formula: `SUM(J${startDetailRow}:J${endDetailRow})`, result: 0 };
                wsAHSP.getCell('K' + baseCostRow).value = { formula: `SUM(K${startDetailRow}:K${endDetailRow})`, result: 0 };
              }
              wsAHSP.getCell('M' + baseCostRow).value = { formula: `(I${baseCostRow}+J${baseCostRow}+K${baseCostRow})`, result: 0 };
              applyBorder(wsAHSP, baseCostRow, 'B', 'N');
              ahspRow++;

              // B. OVERHEAD & PROFIT (Dinamis merujuk ke Kolom L)
              const ohpRow = ahspRow;
              wsAHSP.getCell('D' + ohpRow).value = { formula: `="OVERHEAD & PROFIT (" & (L${headerRow} * 100) & "%)"`, result: `OVERHEAD & PROFIT (${profitVal}%)` };
              wsAHSP.getCell('D' + ohpRow).font = { italic: true };
              wsAHSP.getCell('M' + ohpRow).value = { formula: `M${baseCostRow} * L${headerRow}`, result: 0 };
              ...
              wsAHSP.getCell('M' + totalRow).value = { formula: `ROUND(M${baseCostRow}+M${ohpRow}, 0)`, result: 0 };
              applyBorder(wsAHSP, totalRow, 'B', 'N');

              // Sinkronisasi ke Header (Column I, J, K, M, N)
              wsAHSP.getCell('I' + headerRow).value = { formula: `I${baseCostRow}`, result: 0 };
              wsAHSP.getCell('J' + headerRow).value = { formula: `J${baseCostRow}`, result: 0 };
              wsAHSP.getCell('K' + headerRow).value = { formula: `K${baseCostRow}`, result: 0 };
              wsAHSP.getCell('M' + headerRow).value = { formula: `M${totalRow}`, result: 0 };
              ...
              if (startDetailRow <= endDetailRow) {
                wsAHSP.getCell(`N${headerRow}`).value = { formula: `AVERAGE(N${startDetailRow}:N${endDetailRow})`, result: 0 };
              } else {
                wsAHSP.getCell(`N${headerRow}`).value = Number(line.tkdn || line.total_tkdn_percent || 0) / 100;
              }
  ...
  if (selectedSheets.includes('REKAP')) {
    ...
    if (wsRekap) {
      ...
      for (let r = totalRow; r <= grandTotalRow; r++) {
        wsRekap.getCell(`F${r}`).value = {
          formula: (r === grandTotalRow) ? `ROUND(F${totalRow}+F${ppnRow}, 0)` : (r === ppnRow ? `F${totalRow}*${ppnPercent / 100}` : `SUM(F12:F${endDataRow})`)
        };
