...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
    ...
    if (Array.isArray(projectPrices) && projectPrices.length > 0 && !isCatalog) {
      projectPrices.forEach(p => {
        ...
        resources[kode] = {
          ...
          totalVolume: Number(p.total_volume_terpakai || 0)
        };
      });
      ...
      Object.entries(subBabs).forEach(([subTitle, categories], sIdx) => {
        ...
        Object.entries(categories).forEach(([catTitle, items], cIdx) => {
          ...
          items.forEach((line) => {
            ...
            if (wsHSP) {
              wsHSP.getCell(`A${hspRow}`).value = itemCode;
              wsHSP.getCell(`B${hspRow}`).value = hspCounter++;
              ...
              if (isStandalone) {
                ...
                // VLOOKUP Harga ke AHSP: Cari Kode di A, ambil M (kolom 9 dari E)
                wsHSP.getCell(`G${hspRow}`).value = { 
                  formula: `IFERROR(VLOOKUP(A${hspRow}, 'ahsp'!E:M, 9, FALSE), 0)`,
                  result: Number(line.harga_satuan || 0)
                };
                // VLOOKUP TKDN ke AHSP: Cari Kode di A, ambil N (kolom 10 dari E)
                wsHSP.getCell(`H${hspRow}`).value = {
                  formula: `IFERROR(VLOOKUP(A${hspRow}, 'ahsp'!E:N, 10, FALSE), 0)`,
      ...
      Object.entries(groupedRAB).forEach(([babTitle, items], bIdx) => {
        ...
        items.forEach((line, iIdx) => {
          ...
          if (isStandalone) {
            ...
            // VLOOKUP Harga ke HSP: Cari Kode di B, ambil F (kolom 5 dari B)
            wsRAB.getCell(`H${rabRow}`).value = {
              formula: `IFERROR(VLOOKUP("${itemCodeForLookup}", 'hsp'!B:F, 5, FALSE), 0)`,
              result: Number(line.harga_satuan || 0)
            };
            // VLOOKUP TKDN ke AHSP: Cari Kode di E, ambil N (kolom 10 dari E)
            wsRAB.getCell(`J${rabRow}`).value = {
              formula: `IFERROR(VLOOKUP("${itemCodeForLookup}", 'ahsp'!E:N, 10, FALSE), 0)`,
  ...
  worksheets.forEach(ws => {
    ...
    if (!hasData && !isSelected) {
      ...
      // 6. TANDA TANGAN (Hanya untuk sheet tertentu)
      if (['rab', 'rekap', 'hsp', 'ahsp', 'harga satuan', 'harga_satuan'].includes(ws.name.toLowerCase())) {
...
function renderSignatures(ws, startRow, project) {
  ...
  // 1. TANGGAL (Pojok Kanan Atas Blok TTD)
  ws.mergeCells(`I${startRow}:K${startRow}`);
  ws.getCell(`I${startRow}`).value = dateStr;
  ws.getCell(`I${startRow}`).alignment = { horizontal: 'center' };
  ws.getCell(`I${startRow}`).font = { size: 10 };
  ...
  // Diperiksa (PPTK)
  ws.mergeCells(`C${row1}:E${row1}`);
  ws.getCell(`C${row1}`).value = "Diperiksa :";
  ws.getCell(`C${row1}`).alignment = { horizontal: 'center' };
  
  ws.mergeCells(`C${row1 + 1}:E${row1 + 2}`);
  ws.getCell(`C${row1 + 1}`).value = "FUNGSIONAL TEKNIK PENYEHATAN\nLINGKUNGAN";
  ws.getCell(`C${row1 + 1}`).alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };
  ws.getCell(`C${row1 + 1}`).font = { bold: true, size: 9 };

  ws.mergeCells(`C${rowNames1}:E${rowNames1}`);
  ws.getCell(`C${rowNames1}`).value = project.pptk_name || "( Nama )";
  ws.getCell(`C${rowNames1}`).alignment = { horizontal: 'center' };
  ws.getCell(`C${rowNames1}`).font = { bold: true, underline: true };

  ws.mergeCells(`C${rowNips1}:E${rowNips1}`);
  ws.getCell(`C${rowNips1}`).value = project.pptk_nip ? `NIP. ${project.pptk_nip}` : "NIP. ..................................";
  ws.getCell(`C${rowNips1}`).alignment = { horizontal: 'center' };
  ws.getCell(`C${rowNips1}`).font = { size: 9 };

  // Disusun Oleh (Kontraktor)
  ws.mergeCells(`I${row1}:K${row1}`);
  ws.getCell(`I${row1}`).value = "Disusun Oleh :";
  ws.getCell(`I${row1}`).alignment = { horizontal: 'center' };

  ws.mergeCells(`I${row1 + 1}:K${row1 + 2}`);
  ws.getCell(`I${row1 + 1}`).value = "PELAKSANA BIDANG CIPTA KARYA";
  ws.getCell(`I${row1 + 1}`).alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };
  ws.getCell(`I${row1 + 1}`).font = { bold: true, size: 9 };

  ws.mergeCells(`I${rowNames1}:K${rowNames1}`);
  ws.getCell(`I${rowNames1}`).value = project.kontraktor_director || "( Nama )";
  ws.getCell(`I${rowNames1}`).alignment = { horizontal: 'center' };
  ws.getCell(`I${rowNames1}`).font = { bold: true, underline: true };

  ws.mergeCells(`I${rowNips1}:K${rowNips1}`);
  ws.getCell(`I${rowNips1}`).value = "Pimpinan Perusahaan"; 
  ws.getCell(`I${rowNips1}`).alignment = { horizontal: 'center' };
  ws.getCell(`I${rowNips1}`).font = { size: 9 };
  ...
  // Mengetahui (Kadis)
  ws.mergeCells(`C${row2}:E${row2}`);
  ws.getCell(`C${row2}`).value = "Mengetahui :";
  ws.getCell(`C${row2}`).alignment = { horizontal: 'center' };

  ws.mergeCells(`C${row2 + 1}:E${row2 + 2}`);
  ws.getCell(`C${row2 + 1}`).value = "KEPALA DINAS PEKERJAAN UMUM DAN\nPENATAAN RUANG";
  ws.getCell(`C${row2 + 1}`).alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };
  ws.getCell(`C${row2 + 1}`).font = { bold: true, size: 9 };

  ws.mergeCells(`C${rowNames2}:E${rowNames2}`);
  ws.getCell(`C${rowNames2}`).value = project.kadis_name || "( Nama )";
  ws.getCell(`C${rowNames2}`).alignment = { horizontal: 'center' };
  ws.getCell(`C${rowNames2}`).font = { bold: true, underline: true };

  ws.mergeCells(`C${rowNips2}:E${rowNips2}`);
  ws.getCell(`C${rowNips2}`).value = project.kadis_nip ? `NIP. ${project.kadis_nip}` : "NIP. ..................................";
  ws.getCell(`C${rowNips2}`).alignment = { horizontal: 'center' };
  ws.getCell(`C${rowNips2}`).font = { size: 9 };

  // Diteliti (Kabid)
  ws.mergeCells(`I${row2}:K${row2}`);
  ws.getCell(`I${row2}`).value = "Diteliti :";
  ws.getCell(`I${row2}`).alignment = { horizontal: 'center' };

  ws.mergeCells(`I${row2 + 1}:K${row2 + 2}`);
  ws.getCell(`I${row2 + 1}`).value = "KEPALA BIDANG CIPTA KARYA";
  ws.getCell(`I${row2 + 1}`).alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };
  ws.getCell(`I${row2 + 1}`).font = { bold: true, size: 9 };

  ws.mergeCells(`I${rowNames2}:K${rowNames2}`);
  ws.getCell(`I${rowNames2}`).value = project.kabid_name || "( Nama )";
  ws.getCell(`I${rowNames2}`).alignment = { horizontal: 'center' };
  ws.getCell(`I${rowNames2}`).font = { bold: true, underline: true };

  ws.mergeCells(`I${rowNips2}:K${rowNips2}`);
  ws.getCell(`I${rowNips2}`).value = project.kabid_nip ? `NIP. ${project.kabid_nip}` : "NIP. ..................................";
  ws.getCell(`I${rowNips2}`).alignment = { horizontal: 'center' };
  ws.getCell(`I${rowNips2}`).font = { size: 9 };
