...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  const { isCatalog = false, catAhsp = [], catPrice = [] } = options;
  ...
  // 1. Process Resources (Harga Satuan Terpakai)
  if (selectedSheets.includes('HARGA SATUAN')) {
        ...
        enrichedLines.forEach(line => {
            ...
            details.forEach(d => {
                if (!d.kode_item) return;
                if (!resources[d.kode_item]) {
                    resources[d.kode_item] = {
                        kode: d.kode_item,
                        uraian: d.uraian,
                        satuan: d.satuan,
                        harga: Number(d.harga_satuan_snapshot || d.harga_satuan || 0),
                        jenis: (d.jenis_komponen || d.jenis || 'Lainnya').toLowerCase(),
        ...
        const ws = workbook.getWorksheet('harga_satuan_terpakai');
        if (ws) {
            clearDataRows(ws, 7, 5000);
            ...
            groups.forEach(group => {
                ...
                if (items.length > 0) {
                    ...
                    applyBorder(ws.getRow(currentRow));
                    currentRow++;
  ...
  if (selectedSheets.includes('HSP') || selectedSheets.includes('AHSP')) {
    ...
    if (wsHSP) clearDataRows(wsHSP, 7, 10000);
    if (wsAHSP) clearDataRows(wsAHSP, 7, 25000);

    if (wsAHSP) {
        wsAHSP.getCell('B2').value = `TAHUN ANGGARAN ${project.fiscal_year || '2026'} ${(projectLocation || '-').toUpperCase()}`;
    }
            ...
            Object.entries(categories).forEach(([catTitle, items], cIdx) => {
                ...
                items.forEach((line) => {
                    ...
                    if (wsAHSP) {
                        ...
                        applyBorder(wsAHSP.getRow(mainHeaderRow));
                        ...
                        ahspCats.forEach(cat => {
                            ...
                            if (filtered.length > 0) {
                                ...
                                applyBorder(wsAHSP.getRow(ahspRow));
                                ahspRow++;
                                ...
                                filtered.forEach((d) => {
                                    ...
                                    // VLOOKUP for Harga Dasar (Column H) from harga_satuan_terpakai
                                    wsAHSP.getCell(`H${ahspRow}`).value = { 
                                        formula: `IFERROR(VLOOKUP(E${ahspRow}, harga_satuan_terpakai!E:G, 3, FALSE), 0)`, 
                                        result: Number(d.harga_satuan_snapshot || d.harga_satuan || 0) 
                                    };
    ...
    if (selectedSheets.includes('RAB')) {
        ...
        if (wsRAB) {
            ...
            Object.entries(groupedRAB).forEach(([babTitle, subBabs], bIdx) => {
                ...
                applyBorder(wsRAB.getRow(rabRow));
