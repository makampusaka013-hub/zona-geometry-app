              ...
              ahspCats.forEach(cat => {
                ...
                if (filtered.length > 0) {
                  ...
                  mergedFiltered.forEach((d) => {
                    ...
                    // If koef is very small and unit is kg for common bulk materials, it's likely a Ton-scale koef.
                    if (rawKoef < 0.1 && (d.satuan?.toLowerCase() === 'kg')) {
                      const u = (d.uraian || d.nama_item || d.uraian_ahsp || "").toLowerCase();
                      if (u.includes('semen') || u.includes('pasir') || u.includes('kerikil') || u.includes('batu') || u.includes('pc')) {
                        koef = rawKoef * 1000;
                      }
                    }
                    ...
                    wsAHSP.getCell(`G${ahspRow}`).numFmt = '#,##0.000000'; // Set 6 decimal places for coefficient
