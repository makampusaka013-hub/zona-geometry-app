...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  if (Array.isArray(projectPrices) && projectPrices.length > 0 && !isCatalog) {
    projectPrices.forEach(p => {
      ...
      if (!nameToCanonical[name]) nameToCanonical[name] = code;
      const canonicalCode = nameToCanonical[name];
            ...
            if (wsAHSP) {
              ...
              ahspCats.forEach(cat => {
                ...
                if (filtered.length > 0) {
                  ...
                  const nameMap = {};
                  filtered.forEach(d => {
                    const name = d.uraian || d.nama_item || d.uraian_ahsp;
                    if (nameMap[name]) {
                      nameMap[name].koefisien = Number(nameMap[name].koefisien || 0) + Number(d.koefisien || 0);
                    } else {
                      const copy = { ...d, uraian: name };
                      nameMap[name] = copy;
                      mergedFiltered.push(copy);
                    }
                  });

                    const itemName = (d.uraian || d.nama_item || d.uraian_ahsp || '-').trim();
                    const canonicalCode = nameToCanonical[itemName] || d.kode_item || d.kode;
                    const koef = Number(d.koefisien || 0);
                    let itemPrice = Number(d.harga_satuan_snapshot || d.harga_satuan || d.harga || priceMap[d.kode_item] || priceMap[canonicalCode] || 0);
    ...
    if (wsRAB) {
      ...
      Object.entries(groupedRAB).forEach(([babTitle, items], bIdx) => {
        ...
        items.forEach((line, iIdx) => {
          ...
          if (isStandalone || (isLumpsum && !hasAnalysis)) {
