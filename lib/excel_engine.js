...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  if (selectedSheets.includes('HARGA SATUAN')) {
    ...
    if (ws) {
      ...
      // Pre-apply borders for B6:I for a cleaner "table" look (e.g., 100 rows)
      for (let r = 6; r <= 100; r++) applyBorder(ws, r, 'B', 'I');
...
if (selectedSheets.includes('HSP') || selectedSheets.includes('AHSP')) {
  ...
  const wsAHSP = workbook.getWorksheet('ahsp') || workbook.getWorksheet('AHSP');
    ...
    if (wsHSP) {
      ...
      for (let r = 6; r <= 100; r++) applyBorder(wsHSP, r, 'B', 'H');
    }
    if (wsAHSP) {
      clearDataRows(wsAHSP, 9, 5000);
      wsAHSP.getCell('B2').value = `TAHUN ANGGARAN ${project.fiscal_year || '2026'} ${(projectLocation || '-').toUpperCase()}`;
      // Pre-apply borders for B6:N for AHSP (500 rows for large AHSPs)
      for (let r = 6; r <= 500; r++) applyBorder(wsAHSP, r, 'B', 'N');
    }
