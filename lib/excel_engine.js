          ...
          Object.entries(categories).forEach(([catTitle, items], cIdx) => {
            ...
            items.forEach((line) => {
              ...
              if (wsHSP) {
                ...
                const hargaValue = safeNum(line.harga_satuan || line.total_subtotal || 0);
                const tkdnValue = safeNum(line.tkdn || line.total_tkdn_percent || 100) / 100;
              ...
              if (wsAHSP) {
                ...
                detailsForProfit.forEach(d => {
                   ...
                   const p = safeNum(d.harga_satuan_snapshot || d.harga_satuan || d.harga || priceMap[d.kode_item || d.kode || d.id] || 0);
                   calcBase += (k * p);
                });
                ...
                ahspCats.forEach(cat => {
                  ...
                  filtered.forEach((d) => {
                    ...
                    const itemPrice = safeNum(d.harga_satuan_snapshot || d.harga_satuan || d.harga || priceMap[d.kode_item] || 0);
              ...
              wsAHSP.getCell(`M${mainHeaderRow}`).value = { formula: `ROUND((I${mainHeaderRow}+J${mainHeaderRow}+K${mainHeaderRow})*(1+L${mainHeaderRow}), 0)`, result: safeNum(line.rounded_harga || line.harga_satuan || 0) };
    ...
    if (wsRAB) {
      ...
      Object.entries(groupedRAB).forEach(([babTitle, items], bIdx) => {
        ...
        items.forEach((line, iIdx) => {
          ...
          if (isStandalone) {
            wsRAB.getCell(`H${rabRow}`).value = safeNum(line.harga_satuan || 0);
            wsRAB.getCell(`J${rabRow}`).value = safeNum(line.tkdn || 0) / 100;
          } else {
            wsRAB.getCell(`H${rabRow}`).value = { 
              formula: `IFERROR(VLOOKUP(C${rabRow}, 'hsp'!D:F, 3, FALSE), 0)`,
              result: safeNum(line.harga_satuan || 0)
            };
            wsRAB.getCell(`J${rabRow}`).value = { 
              formula: `IFERROR(VLOOKUP(C${rabRow}, 'hsp'!D:G, 4, FALSE), 0)`,
              result: safeNum(line.tkdn || 0) / 100
            };
          }
          ...
          wsRAB.getCell(`I${rabRow}`).value = { 
            ...
            result: Math.round(safeNum(line.volume || 0) * safeNum(line.harga_satuan || 0))
          };
          wsRAB.getCell(`K${rabRow}`).value = { 
            formula: `I${rabRow}*J${rabRow}`,
            result: Math.round(safeNum(line.volume || 0) * safeNum(line.harga_satuan || 0) * (safeNum(line.tkdn || 0) / 100))
          };
        ...
        const babSubtotal = items.reduce((s, l) => s + Math.round(safeNum(l.volume || 0) * safeNum(l.harga_satuan || 0)), 0);
        const babTkdnTotal = items.reduce((s, l) => s + Math.round(safeNum(l.volume || 0) * safeNum(l.harga_satuan || 0) * (safeNum(l.tkdn || 0) / 100)), 0);
      ...
      const grandSubtotal = enrichedLines.reduce((s, l) => s + Math.round(safeNum(l.volume || 0) * safeNum(l.harga_satuan || 0)), 0);
      const grandTkdnTotal = enrichedLines.reduce((s, l) => s + Math.round(safeNum(l.volume || 0) * safeNum(l.harga_satuan || 0) * (safeNum(l.tkdn || 0) / 100)), 0);
  ...
  if (selectedSheets.includes('REKAP')) {
    ...
    if (wsRekap) {
      ...
      Object.keys(groupedRekap).forEach((babTitle) => {
        ...
        const babSubtotal = babLines.reduce((s, l) => s + (safeNum(l.volume || 0) * safeNum(l.harga_satuan || 0)), 0);
        const babTkdn = babLines.reduce((s, l) => s + (safeNum(l.volume || 0) * safeNum(l.harga_satuan || 0) * (safeNum(l.tkdn || 100) / 100)), 0);
      ...
      // BAGIAN TOTAL & PPN
      // -------------------------------------
      const totalSum = Object.values(groupedRekap).flat().reduce((s, l) => s + Math.round(safeNum(l.volume || 0) * safeNum(l.harga_satuan || 0)), 0);
