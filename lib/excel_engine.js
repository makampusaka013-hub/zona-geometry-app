...
export async function generateProjectReport(project, member, ahspLines, selectedSheets, extra = {}) {
            ...
            Object.entries(categories).forEach(([catTitle, items], cIdx) => {
                ...
                items.forEach((line) => {
                    ...
                    if (wsAHSP) {
                        ...
                        wsAHSP.getCell(`N${ahspRow}`).value = Number(line.tkdn || 0) / 100;
                        wsAHSP.getCell(`P${ahspRow}`).value = ",..," ; // Item trigger
                        ahspRow++;
  ...
  if (selectedSheets.includes('RAB')) {
    ...
    if (wsRAB) {
        ...
        const ppnAmount = totalBiayaPekerjaan * (ppnPercent / 100);
