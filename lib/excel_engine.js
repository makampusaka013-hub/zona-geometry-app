...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
    ...
    if (selectedSheets.includes('HARGA SATUAN')) {
      ...
      if (ws) {
        ...
        groups.forEach(group => {
          ...
          if (items.length > 0) {
            currentRow++;
            const headerCell = ws.getCell(`C${currentRow}`);
            ...
            items.forEach((line) => {
              ...
              if (wsHSP) {
                ...
                if (isStandalone) {
                  ...
                  wsHSP.getCell(`D${hspRow}`).value = { formula: `IFERROR(VLOOKUP(B${hspRow}, ahsp!B:N, 3, FALSE), "")`, result: String(uraianValue || "") };
                  wsHSP.getCell(`E${hspRow}`).value = { formula: `IFERROR(VLOOKUP(B${hspRow}, ahsp!B:F, 5, FALSE), "")`, result: String(satuanValue || "") };
                  wsHSP.getCell(`F${hspRow}`).value = { formula: `IFERROR(VLOOKUP(B${hspRow}, ahsp!B:N, 12, FALSE), 0)`, result: Number(hargaValue) || 0 };
                  wsHSP.getCell(`G${hspRow}`).value = { formula: `IFERROR(VLOOKUP(B${hspRow}, ahsp!B:N, 13, FALSE), 0)`, result: Number(tkdnValue) || 0 };
                }
                  ...
                  filtered.forEach((d) => {
                    ...
                    if (isStandalone) {
                      ...
                      wsAHSP.getCell(`H${ahspRow}`).value = {
                        formula: `IFERROR(VLOOKUP(E${ahspRow}, 'Harga Satuan'!E:G, 3, FALSE), 0)`,
  ...
  if (selectedSheets.includes('REKAP')) {
    ...
    if (wsRekap) {
      ...
      wsRekap.getCell(`G${totalRow}`).value = { formula: `AVERAGE(G12:G${endDataRow})`, result: 0 };
      wsRekap.getCell(`G${totalRow}`).numFmt = '0.00%';
