...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  if (headerImage) {
      // LANGKAH 1: Ubah gambar menjadi format Base64 (Logic from Tester)
      const base64Murni = headerImage.includes(',') ? headerImage.split(',')[1] : headerImage;
      const extension = headerImage.includes('image/jpeg') || headerImage.includes('image/jpg') ? 'jpeg' : 'png';
