            ...
            Object.entries(categories).forEach(([catTitle, items], cIdx) => {
                ...
                items.forEach((line) => {
                    ...
                    if (wsAHSP) {
                        ...
                        // Fallback for catalog 'nama_pekerjaan'
                        wsAHSP.getCell(`D${ahspRow}`).value = ((line.uraian || line.nama_pekerjaan || '-') + "").toUpperCase();
                        wsAHSP.getCell(`E${ahspRow}`).value = itemCode;
                        wsAHSP.getCell(`F${ahspRow}`).value = line.satuan || line.satuan_pekerjaan;
                        // Fallback for catalog 'total_subtotal'
                        wsAHSP.getCell(`M${ahspRow}`).value = Number(line.harga_satuan || line.total_subtotal || 0);
                        wsAHSP.getCell(`N${ahspRow}`).value = Number(line.tkdn || line.total_tkdn_percent || 0) / 100;
