...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
        ...
        Object.entries(categories).forEach(([catTitle, items], cIdx) => {
          ...
          items.forEach((line) => {
            ...
            if (wsAHSP) {
              ...
              let profitMargin = line.profit_margin !== undefined && line.profit_margin !== null ? Number(line.profit_margin) : null;
              
              if (profitMargin === null) {
                if (line.profit_percent !== null && line.profit_percent !== undefined) {
                  profitMargin = Number(line.profit_percent);
                } else if (line.profitPercent !== null && line.profitPercent !== undefined) {
                  profitMargin = Number(line.profitPercent);
                } else if (line.overhead_percent !== null && line.overhead_percent !== undefined) {
                  profitMargin = Number(line.overhead_percent);
                } else if (line.overheadPercent !== null && line.overheadPercent !== undefined) {
                  profitMargin = Number(line.overheadPercent);
                } 
              }
              
              // 2. Prioritas Kedua: Reverse Engineer dari Harga Satuan jika data di atas kosong
              if (profitMargin === null && calcBase > 0 && Number(line.harga_satuan) > 0) {
                profitMargin = Math.round(((Number(line.harga_satuan) / calcBase) - 1) * 100);
              } 
              ...
              wsAHSP.getCell(`L${mainHeaderRow}`).value = Number(profitMargin) / 100;
              ...
              const targetCellHargaDasar = `(I${baseCostRow}+J${baseCostRow}+K${baseCostRow})`;
              wsAHSP.getCell(`M${baseCostRow}`).value = { formula: targetCellHargaDasar, result: 0 };
              applyBorder(wsAHSP, baseCostRow, 'B', 'N');
              ahspRow++;

              // Tambahkan Baris OVERHEAD & PROFIT
              const ohpRow = ahspRow;
              wsAHSP.getCell(`D${ohpRow}`).value = 'OVERHEAD & PROFIT (' + profitMargin + '%)';
              wsAHSP.getCell(`D${ohpRow}`).font = { italic: true };
              wsAHSP.getCell(`M${ohpRow}`).value = { formula: targetCellHargaDasar + '*(' + profitMargin + '/100)', result: 0 };
              applyBorder(wsAHSP, ohpRow, 'B', 'N');
              ahspRow++;

              // Update Total Harga Satuan Pekerjaan
              ...
              // Sinkronisasi ke Header (Column I, J, K, M)
              wsAHSP.getCell(`I${mainHeaderRow}`).value = { formula: `I${baseCostRow}`, result: 0 };
              wsAHSP.getCell(`J${mainHeaderRow}`).value = { formula: `J${baseCostRow}`, result: 0 };
              wsAHSP.getCell(`K${mainHeaderRow}`).value = { formula: `K${baseCostRow}`, result: 0 };
              wsAHSP.getCell(`M${mainHeaderRow}`).value = { formula: `M${totalRow}`, result: 0 };
  ...
  worksheets.forEach(ws => {
    ...
    if (!hasData) {
      // Jika kosong (tidak ada data), jangan di print (sembunyikan saja)
      ws.state = 'veryHidden';
    } else {
