...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  // 1b. Sheet: Harga Satuan Terpakai
  const wsTerpakai = workbook.getWorksheet('harga satuan terpakai') || workbook.getWorksheet('Harga Satuan Terpakai') || workbook.getWorksheet('HARGA SATUAN TERPAKAI');
  if (wsTerpakai) {
    clearDataRows(wsTerpakai, 6, 1000);
    let currentRow = 6;
    const groups = [
      { label: 'TENAGA KERJA', types: ['upah', 'tenaga'] },
      { label: 'BAHAN', types: ['bahan'] },
    ...
    for (let i = 2; i <= 12; i++) {
      wsTerpakai.getCell(6, i).border = { top: { style: 'thin' }, bottom: { style: 'thin' }, left: i === 2 ? { style: 'thin' } : undefined, right: i === 12 ? { style: 'thin' } : undefined };
    }
    groups.forEach(group => {
      const items = Object.values(resources).filter(r => group.types.some(t => r.jenis.includes(t)));
      if (items.length > 0) {
        currentRow++;
        wsTerpakai.getCell(`C${currentRow}`).value = group.label;
        wsTerpakai.getRow(currentRow).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'F1F5F9' } };
        for (let i = 2; i <= 12; i++) {
          wsTerpakai.getCell(currentRow, i).border = { top: { style: 'thin' }, bottom: { style: 'thin' }, left: i === 2 ? { style: 'thin' } : undefined, right: i === 12 ? { style: 'thin' } : undefined };
        }
        currentRow++;
        items.forEach((r, idx) => {
          wsTerpakai.getCell(`B${currentRow}`).value = idx + 1;
          wsTerpakai.getCell(`B${currentRow}`).alignment = { vertical: 'middle', horizontal: 'center' };
          wsTerpakai.getCell(`C${currentRow}`).value = r.uraian;
          wsTerpakai.getCell(`E${currentRow}`).value = r.kode;
          wsTerpakai.getCell(`E${currentRow}`).alignment = { vertical: 'middle', horizontal: 'center' };
          wsTerpakai.getCell(`F${currentRow}`).value = r.satuan;
          wsTerpakai.getCell(`F${currentRow}`).alignment = { vertical: 'middle', horizontal: 'center' };
          wsTerpakai.getCell(`G${currentRow}`).value = Number(r.totalVolume.toFixed(4));
          wsTerpakai.getCell(`G${currentRow}`).numFmt = '#,##0.00';
          wsTerpakai.getCell(`H${currentRow}`).value = Number(r.harga);
          wsTerpakai.getCell(`I${currentRow}`).value = { formula: `G${currentRow}*H${currentRow}`, result: r.totalVolume * r.harga };
          wsTerpakai.getCell(`K${currentRow}`).value = totalProjectCostForPercent > 0 ? (r.totalVolume * r.harga) / totalProjectCostForPercent : 0;
          wsTerpakai.getCell(`L${currentRow}`).value = Number(r.tkdn) / 100;
          wsTerpakai.getCell(`H${currentRow}`).numFmt = '#,##0.00';
          wsTerpakai.getCell(`I${currentRow}`).numFmt = '#,##0.00';
          wsTerpakai.getCell(`K${currentRow}`).numFmt = '0.00%';
          wsTerpakai.getCell(`L${currentRow}`).numFmt = '0.00%';
          for (let i = 2; i <= 12; i++) {
            if (i === 3 || i === 4) wsTerpakai.getCell(currentRow, i).border = { top: { style: 'thin' }, bottom: { style: 'thin' } };
            else wsTerpakai.getCell(currentRow, i).border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } };
          }
          currentRow++;
        });
      }
    });
    if (headerImageId !== null) wsTerpakai.addImage(headerImageId, { tl: { col: 1, row: 0 }, br: { col: 12, row: 1 }, editAs: 'twoCell' });
  }
      ...
      Object.entries(categories).forEach(([catTitle, items]) => {
        ...
        items.forEach((line) => {
          ...
          if (wsAHSP) {
            ...
            const aCs = [{ label: 'TENAGA KERJA', types: ['upah', 'tenaga'] }, { label: 'BAHAN', types: ['bahan'] }, { label: 'PERALATAN', types: ['alat', 'peralatan', 'mesin'] }];
            aCs.forEach(cat => {
              const filtered = details.filter(d => { const lJ = (d.jenis_komponen || d.jenis || d.jenis_uraian || '').toLowerCase(); return cat.types.some(t => lJ.includes(t)); });
  ...
  // FINAL CLEANUP
  const sheetMap = { 'COVER': ['Cover', 'COVER'], 'RAB': ['RAB', 'rab'], 'HSP': ['HSP', 'hsp'], 'AHSP': ['AHSP', 'ahsp'], 'HARGA SATUAN': ['Harga Satuan', 'HARGA SATUAN', 'harga satuan'], 'HARGA SATUAN TERPAKAI': ['harga satuan terpakai', 'Harga Satuan Terpakai', 'HARGA SATUAN TERPAKAI'], 'REKAP': ['REKAP', 'rekap', 'rekap rab'], 'schedule': ['schedule', 'SCHEDULE', 'Kurva-S'] };
  ...
  worksheets.forEach(ws => {
    ...
    if (!isSelected) { workbook.removeWorksheet(ws.id); return; }
    let hasData = false; const sR = ws.name.toLowerCase().includes('rab') ? 12 : 6; let hRWD = sR;
    ws.eachRow({ includeEmpty: true }, (row, rN) => { if (rN >= sR) { let rHC = false; for (let i = 2; i <= 14; i++) { if (row.getCell(i).value !== null && row.getCell(i).value !== "") { rHC = true; break; } } if (rHC) { hasData = true; hRWD = Math.max(hRWD, rN); } } });
    if (!hasData && !ws.name.toLowerCase().includes('cover')) { workbook.removeWorksheet(ws.id); } else {
      const lR = hRWD + 2; let lC = 'K'; const n = ws.name.toLowerCase(); if (n.includes('ahsp')) lC = 'O'; else if (n.includes('hsp')) lC = 'I'; else if (n.includes('terpakai')) lC = 'M'; else if (n.includes('harga satuan')) lC = 'J'; else if (n.includes('rekap')) lC = 'I';
      setupPrinter(ws, companyName, `A1:${lC}${lR}`, paperSize, (n.includes('schedule') || n.includes('kurva-s')) ? 'landscape' : 'portrait');
    }
  });
