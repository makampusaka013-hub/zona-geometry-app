...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  if (selectedSheets.includes('RAB')) {
    ...
    if (wsRAB) {
      ...
      wsRAB.getCell('E7').value = 'ITEM PEKERJAAN';
  ...
  if (selectedSheets.includes('REKAP')) {
    ...
    if (wsRekap) {
      ...
      wsRekap.getCell(`I${totalRow}`).value = { formula: `SUM(I12:I${endDataRow})` };
      
      const ppnRow = rekapRow + 1;
      wsRekap.getCell(`C${ppnRow}`).value = `Pajak Pertambahan Nilai ( PPN ) = ${ppnPercent}%`;
      wsRekap.getCell(`I${ppnRow}`).value = { formula: `I${totalRow}*${ppnPercent/100}` };
      
      const grandTotalRow = rekapRow + 2;
      wsRekap.getCell(`C${grandTotalRow}`).value = 'JUMLAH TOTAL HARGA PEKERJAAN';
      wsRekap.getCell(`I${grandTotalRow}`).value = { formula: `I${totalRow}+I${ppnRow}` };

      // Formatting & Border Bagian Total
      for(let r = totalRow; r <= grandTotalRow; r++) {
        wsRekap.getCell(`I${r}`).numFmt = '#,##0.00';
        wsRekap.getRow(r).font = { bold: true };
        wsRekap.getCell(`C${r}`).alignment = { horizontal: 'right' };
        
        // Box border dari B sampai I untuk total
        ['B','C','D','E','F','G','H','I','J','K'].forEach(col => {
          if(col <= 'I') wsRekap.getCell(`${col}${r}`).border = { top: { style: 'thin' }, left: { style: 'thin' }, bottom: { style: 'thin' }, right: { style: 'thin' } };
        });
      }
      ...
      wsRekap.getCell(`I${sigRow}`).value = `Kotamobagu,    ${new Date().toLocaleString('id-ID', { month: 'long', year: 'numeric' })}`;
      
      sigRow += 3;
      wsRekap.getCell(`C${sigRow}`).value = 'Diperiksa :';
      wsRekap.getCell(`C${sigRow+1}`).value = 'FUNGSIONAL TEKNIK PENYEHATAN LINGKUNGAN';
      wsRekap.getCell(`I${sigRow}`).value = 'Disusun Oleh :';
      wsRekap.getCell(`I${sigRow+1}`).value = 'PELAKSANA BIDANG CIPTA KARYA';
      
      sigRow += 6;
      wsRekap.getCell(`C${sigRow}`).value = '( NAMA PEJABAT )';
      wsRekap.getCell(`C${sigRow+1}`).value = 'Nip. ...........................';
      wsRekap.getCell(`I${sigRow}`).value = '( NAMA PENYUSUN )';
      wsRekap.getCell(`I${sigRow+1}`).value = 'Nip. ...........................';

      // Print Setup
      setupPrinter(wsRekap, companyName, `B1:K${sigRow+3}`, paperSize, 'portrait');
      
      // Inject Logo
      if (headerImageId !== null) {
        wsRekap.addImage(headerImageId, { tl: { col: 1, row: 0 }, br: { col: 11, row: 1 }, editAs: 'twoCell' });
      }
    }
  }
