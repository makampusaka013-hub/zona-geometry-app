...
const setupPrinter = (ws, companyName, printArea = null) => {
  if (!ws) return;
  ws.pageSetup = {
    paperSize: 9, // A4
...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  if (selectedSheets.includes('HARGA SATUAN')) {
    ...
    if (ws) {
      // Bersihkan area baris 1-5 secara eksplisit agar border bawaan kolom terhapus
      for (let r = 1; r <= 5; r++) {
      ...
      // Data ditulis mulai dari baris 6 (Header di-apply saat baris ke-6)
      let currentRow = 6;
      const groups = [
      ...
      applyBorder(ws, 6, 'B', 'I'); // Header border
      ...
      groups.forEach(group => {
        ...
        if (items.length > 0) {
          currentRow++;
          ws.getCell(`B${currentRow}`).value = null;
      ...
      setupPrinter(ws, companyName, 'A:J');
    }
  }
  ...
  if (selectedSheets.includes('HSP') || selectedSheets.includes('AHSP')) {
    ...
    if (wsHSP) {
      ...
      // Row 6: Only clear Column G border, keep Fill
      wsHSP.getRow(6).getCell(7).border = { top: { style: 'none' }, left: { style: 'none' }, bottom: { style: 'none' }, right: { style: 'none' } };
      clearDataRows(wsHSP, 7, 1000);
    }
    if (wsAHSP) {
      for (let r = 1; r <= 5; r++) {
      ...
      // Row 6: Only clear Column G border, keep Fill
      wsAHSP.getRow(6).getCell(7).border = { top: { style: 'none' }, left: { style: 'none' }, bottom: { style: 'none' }, right: { style: 'none' } };
      clearDataRows(wsAHSP, 7, 5000);
    }
        ...
        Object.entries(categories).forEach(([catTitle, items], cIdx) => {
          ...
          items.forEach((line) => {
            ...
            if (wsHSP) {
              ...
              wsHSP.getCell(`D${hspRow}`).value = { formula: `IFERROR(VLOOKUP(B${hspRow}, ahsp!B:N, 3, FALSE), "${line.uraian || line.nama_pekerjaan}")`, result: line.uraian || line.nama_pekerjaan };
              wsHSP.getCell(`E${hspRow}`).value = { formula: `IFERROR(VLOOKUP(B${hspRow}, ahsp!B:F, 5, FALSE), "${line.satuan || line.satuan_pekerjaan}")`, result: line.satuan || line.satuan_pekerjaan };
              wsHSP.getCell(`F${hspRow}`).value = { formula: `IFERROR(VLOOKUP(B${hspRow}, ahsp!B:N, 12, FALSE), ${Number(line.harga_satuan || line.total_subtotal || 0)})`, result: Number(line.harga_satuan || line.total_subtotal || 0) };
              wsHSP.getCell(`F${hspRow}`).numFmt = '_(Rp* #,##0.00_);_(Rp* (#,##0.00);_(Rp* "-"??_);_(@_)';
              wsHSP.getCell(`G${hspRow}`).value = { formula: `IFERROR(VLOOKUP(B${hspRow}, ahsp!B:N, 13, FALSE), ${Number(line.tkdn || line.total_tkdn_percent || 100) / 100})`, result: Number(line.tkdn || line.total_tkdn_percent || 100) / 100 };
