...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  const { isCatalog = false, catAhsp = [], catPrice = [], projectPrices = [], headerImage = null } = options;
  ...
  if (headerImage) {
    try {
      ...
      const base64Data = headerImage.includes('base64,') ? headerImage.split('base64,')[1] : headerImage;
      const extension = headerImage.includes('image/jpeg') ? 'jpeg' : 
                        headerImage.includes('image/jpg') ? 'jpeg' : 
                        headerImage.includes('image/gif') ? 'gif' : 'png';
                ...
                if (filtered.length > 0) {
                  ...
                  filtered.forEach((d) => {
                    ...
                    wsAHSP.getCell(`H${ahspRow}`).value = {
                      formula: `IFERROR(VLOOKUP(E${ahspRow}, 'harga satuan terpakai'!E:G, 3, FALSE), "")`,
                      result: itemPrice
                    };
