...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  if (selectedSheets.includes('RAB')) {
    ...
    if (wsRAB) {
      ...
      wsRAB.getCell('E7').value = (project?.lokasi || projectLocation || '-').toUpperCase();
      wsRAB.getCell('E8').value = project?.tahun_anggaran || project?.fiscal_year || '2026';
      ...
      Object.entries(groupedRAB).forEach(([babTitle, items], bIdx) => {
        ...
        items.forEach((line, iIdx) => {
          ...
          // HSP Sheet Mapping: D = Uraian, F = Harga Satuan Jadi, G = TKDN
