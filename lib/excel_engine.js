...
export async function generateProjectReport(project, member, ahspLines, selectedSheets, extra = {}) {
  ...
  const response = await fetch(`/templates/master_template_${templateType}.xlsx?v=${Date.now()}`);
            ...
            Object.entries(categories).forEach(([catTitle, items], cIdx) => {
                ...
                items.forEach((line) => {
                    ...
                    if (wsHSP) {
                        ...
                        wsHSP.getCell(`G${hspRow}`).value = line.satuan || line.satuan_pekerjaan;
                        
                        // H: TKDN
                        wsHSP.getCell(`H${hspRow}`).value = Number(line.tkdn || line.total_tkdn_percent || 100) / 100;
                        wsHSP.getCell(`H${hspRow}`).numFmt = '0.00%';

                        // I: Harga Satuan Jadi
                        wsHSP.getCell(`I${hspRow}`).value = Number(line.harga_satuan || line.total_subtotal || 0);
                        wsHSP.getCell(`I${hspRow}`).numFmt = '#,##0.00';
                    ...
                    if (wsAHSP) {
                        ...
                        // L - PROFIT (%)
                        const profitPct = (line.overhead_profit || 15) / 100;
                        ...
                        const categories = [
                            ...
                            { label: 'PERALATAN', types: ['alat', 'peralatan'] }
                        ];
                        ...
                        categories.forEach(cat => {
                            ...
                            if (filtered.length > 0) {
                                ...
                                wsAHSP.getCell(`D${ahspRow}`).font = { bold: true };
                                ahspRow++;
                                ...
                                filtered.forEach((d, idx) => {
                                    ...
                                    const subtotalFormula = { formula: `G${ahspRow}*H${ahspRow}`, result: 0 };
  ...
  if (selectedSheets.includes('RAB')) {
    ...
    if (wsRAB) {
        ...
        rabRow += 1;
        const totalBiayaRow = rabRow;
        wsRAB.getCell(`B${totalBiayaRow}`).value = 'TOTAL HARGA PEKERJAAN';
        wsRAB.getCell(`G${totalBiayaRow}`).value = { formula: `SUM(G${summaryStartRow}:G${summaryEndRow})`, result: totalBiayaPekerjaan };
        wsRAB.getCell(`G${totalBiayaRow}`).numFmt = '#,##0.00';
        wsRAB.getCell(`B${totalBiayaRow}`).font = { bold: true };

        rabRow++;
        const ppnRow = rabRow;
        wsRAB.getCell(`B${ppnRow}`).value = `PPN ${ppnPercent}%`;
        wsRAB.getCell(`G${ppnRow}`).value = { formula: `G${totalBiayaRow}*${ppnPercent/100}`, result: ppnAmount };
        wsRAB.getCell(`G${ppnRow}`).numFmt = '#,##0.00';

        rabRow++;
        const grandTotalRow = rabRow;
        wsRAB.getCell(`B${grandTotalRow}`).value = 'TOTAL KESELURUHAN';
        wsRAB.getCell(`G${grandTotalRow}`).value = { formula: `G${totalBiayaRow}+G${ppnRow}`, result: grandTotal };
        wsRAB.getCell(`G${grandTotalRow}`).numFmt = '#,##0.00';
        wsRAB.getCell(`B${grandTotalRow}`).font = { bold: true };

        rabRow++;
        const roundedRow = rabRow;
        wsRAB.getCell(`B${roundedRow}`).value = 'DIBULATKAN';
        // Formula to round to nearest 1000
        wsRAB.getCell(`G${roundedRow}`).value = { formula: `ROUND(G${grandTotalRow}/1000,0)*1000`, result: rounded };
        wsRAB.getCell(`G${roundedRow}`).numFmt = '#,##0.00';
        wsRAB.getCell(`B${roundedRow}`).font = { bold: true };
