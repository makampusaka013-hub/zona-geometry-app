...
export async function generateProjectReport(project, member, ahspLines, selectedSheets, extra = {}) {
  ...
  const templateType = extra.templateType || (isCatalog ? 'common' : 'rab');
    ...
    if (ws) {
      ...
      if (isCatalog && currentPrices) {
        currentPrices.forEach(p => {
            const key = p.master_items?.kode || p.master_items?.uraian || p.uraian;
            resources.set(key, {
                kode: p.master_items?.kode || '-',
                uraian: p.master_items?.uraian || p.uraian,
                satuan: p.master_items?.satuan || p.satuan,
                jenis: p.master_items?.jenis_komponen || '-',
                harga: p.harga_dasar || 0,
                tkdn: p.master_items?.tkdn || 100
            });
        });
      } else {
      ...
      const types = [
        { key: 'upah', label: 'UPAH TENAGA KERJA', code: 'A' },
        { key: 'bahan', label: 'BAHAN LOKAL', code: 'B' },
      ...
      let currentRow = 6;
      types.forEach((type) => {
        const filtered = sorted.filter(r => (r.jenis || '').toLowerCase().includes(type.key));
        if (filtered.length > 0) {
          ...
          currentRow++; // 1 empty row between sections as requested
        }
      });
  ...
  if (selectedSheets.includes('HSP') || selectedSheets.includes('AHSP')) {
    const wsHSP = workbook.getWorksheet('hsp');
    const wsAHSP = workbook.getWorksheet('ahsp');
    
    let hspRow = 7;
    let ahspRow = 7;

    // Helper to group by hierarchy: Bab -> SubBab -> Category
    // Expected ahsp_lines have these fields or we infer from code/uraian
    const groupedItems = {};
    ...
    Object.entries(groupedItems).forEach(([babTitle, subBabs], bIdx) => {
        ...
        if (wsHSP) {
            ...
            wsHSP.getCell(`J${hspRow}`).value = ",.."; // Bab formatting
            hspRow++;
        }
        if (wsAHSP) {
            wsAHSP.getCell(`B${ahspRow}`).value = bIdx + 1;
            wsAHSP.getCell(`C${ahspRow}`).value = babTitle.toUpperCase();
            wsAHSP.getCell(`P${ahspRow}`).value = ",.."; // Bab formatting
            ahspRow++;
        }
            ...
            Object.entries(categories).forEach(([catTitle, items], cIdx) => {
                ...
                items.forEach((line) => {
                    ...
                    if (wsHSP) {
                        ...
                        wsHSP.getCell(`F${hspRow}`).value = line.satuan;
                        wsHSP.getCell(`H${hspRow}`).value = Number(line.harga_satuan || 0);
                        wsHSP.getCell(`G${hspRow}`).value = Number(line.tkdn || 100) / 100;
                        wsHSP.getCell(`J${hspRow}`).value = ",..," ; // Item formatting
                        hspRow++;
                    }
                    ...
                    if (wsAHSP) {
                        ...
                        wsAHSP.getCell(`E${ahspRow}`).value = itemCode;
                        wsAHSP.getCell(`F${ahspRow}`).value = line.satuan;
                        wsAHSP.getCell(`M${ahspRow}`).value = Number(line.harga_satuan || 0);
                        wsAHSP.getCell(`P${ahspRow}`).value = ",..," ; // Item formatting
                        ahspRow++;
                        ...
                        details.forEach(d => {
                            ...
                            const total = Number(d.jumlah_harga_snapshot || d.jumlah_harga || 0);
                            const lowJenis = (d.jenis_komponen || d.jenis || '').toLowerCase();
                            
                            if (lowJenis.includes('upah')) wsAHSP.getCell(`I${ahspRow}`).value = total;
                            else if (lowJenis.includes('bahan')) wsAHSP.getCell(`J${ahspRow}`).value = total;
                            else wsAHSP.getCell(`K${ahspRow}`).value = total;
                        ...
                        // Overhead/Profit row
                        wsAHSP.getCell(`D${ahspRow}`).value = 'OVERHEAD & PROFIT';
                        wsAHSP.getCell(`L${ahspRow}`).value = Number(line.profit_val || 0);
                        wsAHSP.getCell(`P${ahspRow}`).value = ",..,";
                        ahspRow++;

                        wsAHSP.getCell(`D${ahspRow}`).value = 'TOTAL HARGA SATUAN PEKERJAAN';
                        wsAHSP.getCell(`M${ahspRow}`).value = Number(line.harga_satuan || 0);
                        wsAHSP.getCell(`P${ahspRow}`).value = ",..,";
                        ahspRow += 2;
                    }
  ...
  if (selectedSheets.includes('RAB')) {
    const wsRAB = workbook.worksheets.find(s => s.name.startsWith('RAB'));
    if (wsRAB) {
        // Headers - Adjusting to typical template rows
        wsRAB.getCell('D7').value = project.work_name || project.name;
        wsRAB.getCell('D8').value = project.location || '-';
        wsRAB.getCell('D9').value = '2026';

        // Clear existing data area (Row 15 and down) - optional but good for "clean" replacement
        for (let r = 15; r < 500; r++) {
            wsRAB.getRow(r).values = [];
        }
        ...
        Object.entries(groupedRAB).forEach(([babTitle, subBabs], bIdx) => {
            ...
            // Add control column if needed for RAB, assuming same logic as HSP/AHSP
        ...
        // Summary footer
        const ppnVal = totalBiayaPekerjaan * (ppnPercent / 100);
        const grandTotal = totalBiayaPekerjaan + ppnVal;
        ...
        wsRAB.getCell(`G${rabRow}`).value = ppnVal;
  ...
  // Finalize: Remove sheets not in selectedSheets
  workbook.eachSheet((sheet) => {
      // Map names logically: Template has 'HARGA SATUAN', 'HSP', 'AHSP', and 'RAB_...'
      let shouldKeep = false;
      if (sheet.name === 'HARGA SATUAN' && selectedSheets.includes('HARGA SATUAN')) shouldKeep = true;
      if (sheet.name === 'HSP' && selectedSheets.includes('HSP')) shouldKeep = true;
      if (sheet.name === 'AHSP' && selectedSheets.includes('AHSP')) shouldKeep = true;
      if (sheet.name === 'SCHEDULE' && selectedSheets.includes('SCHEDULE')) shouldKeep = true;
      if (sheet.name.startsWith('RAB') && selectedSheets.includes('RAB')) shouldKeep = true;
      
      // Always remove the template raw if not explicitly chosen
      if (!shouldKeep) {
  ...
  a.href = url;
  a.download = `Laporan_Lengkap_${project.name}.xlsx`;
  a.click();
}
...
function setupPrinter(ws, companyName) {
    ws.pageSetup = {
        ...
        printTitlesRow: '1:13' // Assuming header info ends around row 13
    };

    ws.headerFooter = {
        oddHeader: `&L&B${companyName}&R&BBuildCalc Pro`,
        oddFooter: `&L&8Dicetak pada: ${new Date().toLocaleDateString('id-ID')}&C&8Halaman &P dari &N&R&8BuildCalc System`
    };
}
