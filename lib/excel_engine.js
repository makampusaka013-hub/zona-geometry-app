...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  if (selectedSheets.some(s => s.toLowerCase() === 'schedule') && Array.isArray(options.scheduleData) && options.scheduleData.length > 0) {
    ...
    if (wsSched) {
      ...
      // Hitung baris akhir total (JUMLAH) terlebih dahulu untuk referensi rumus Bobot %
      let numTotalRows = options.scheduleData.length + Object.keys(groupedSched).length;
      const totalItemRow = 8 + numTotalRows;
      ...
      Object.entries(groupedSched).forEach(([babTitle, items], bIdx) => {
        ...
        wsSched.getRow(currentRow).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'F8FAFC' } };
        currentRow++;
        ...
        items.forEach((item) => {
          ...
          // E: Bobot % (Harga / Total Harga di baris totalItemRow)
