...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  // Fetch all prices for this location to ensure we have fallback prices
  let masterPrices = [];
  try {
      ...
      console.error("Failed to fetch master prices", e);
  }
  const priceMap = Object.fromEntries(masterPrices.map(p => [p.kode_item, p.harga_satuan]));
    ...
    enrichedLines.forEach(line => {
      ...
      details.forEach(d => {
        ...
        if (!resources[d.kode_item]) {
          const itemPrice = Number(d.harga_satuan_snapshot || d.harga_satuan || priceMap[d.kode_item] || 0);
    ...
    if (ws) {
      ...
      groups.forEach(group => {
        ...
        if (items.length > 0) {
          ...
          // Removed bold font to follow user preference (conditional formatting handled in Excel)
          ...
          items.forEach((r, idx) => {
            ...
            // Explicitly ensure NO bold in data rows
  ...
  if (selectedSheets.includes('HSP') || selectedSheets.includes('AHSP')) {
    ...
    if (wsAHSP) clearDataRows(wsAHSP, 7, 5000);
                ...
                if (filtered.length > 0) {
                  ...
                  filtered.forEach((d) => {
                    ...
                    wsAHSP.getCell(`H${ahspRow}`).value = { 
                      ...
                      result: Number(d.harga_satuan_snapshot || d.harga_satuan || priceMap[d.kode_item] || 0) 
                    };
