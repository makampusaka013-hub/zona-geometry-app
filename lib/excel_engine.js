...
function renderSignatures(ws, startRow, project) {
  ...
  ws.mergeCells(\I\:K\\);
  ws.getCell(\I\\).value = dateStr;
  ws.getCell(\I\\).alignment = { horizontal: 'center' };
  ws.getCell(\I\\).font = { size: 10 };
  ...
  // Diperiksa (PPTK)
  ws.mergeCells(\C\:E\\);
  ws.getCell(\C\\).value = "Diperiksa :";
  ws.getCell(\C\\).alignment = { horizontal: 'center' };
  
  ws.mergeCells(\C\:E\\);
  ws.getCell(\C\\).value = "FUNGSIONAL TEKNIK PENYEHATAN\nLINGKUNGAN";
  ws.getCell(\C\\).alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };
  ws.getCell(\C\\).font = { bold: true, size: 9 };

  ws.mergeCells(\C\:E\\);
  ws.getCell(\C\\).value = project.pptk_name || "( Nama )";
  ws.getCell(\C\\).alignment = { horizontal: 'center' };
  ws.getCell(\C\\).font = { bold: true, underline: true };

  ws.mergeCells(\C\:E\\);
  ws.getCell(\C\\).value = project.pptk_nip ? \NIP. \\ : "NIP. ..................................";
  ws.getCell(\C\\).alignment = { horizontal: 'center' };
  ws.getCell(\C\\).font = { size: 9 };

  // Disusun Oleh (Kontraktor)
  ws.mergeCells(\I\:K\\);
  ws.getCell(\I\\).value = "Disusun Oleh :";
  ws.getCell(\I\\).alignment = { horizontal: 'center' };

  ws.mergeCells(\I\:K\\);
  ws.getCell(\I\\).value = "PELAKSANA BIDANG CIPTA KARYA";
  ws.getCell(\I\\).alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };
  ws.getCell(\I\\).font = { bold: true, size: 9 };

  ws.mergeCells(\I\:K\\);
  ws.getCell(\I\\).value = project.kontraktor_director || "( Nama )";
  ws.getCell(\I\\).alignment = { horizontal: 'center' };
  ws.getCell(\I\\).font = { bold: true, underline: true };

  ws.mergeCells(\I\:K\\);
  ws.getCell(\I\\).value = "Pimpinan Perusahaan"; 
  ws.getCell(\I\\).alignment = { horizontal: 'center' };
  ws.getCell(\I\\).font = { size: 9 };
  ...
  // Mengetahui (Kadis)
  ws.mergeCells(\C\:E\\);
  ws.getCell(\C\\).value = "Mengetahui :";
  ws.getCell(\C\\).alignment = { horizontal: 'center' };

  ws.mergeCells(\C\:E\\);
  ws.getCell(\C\\).value = "KEPALA DINAS PEKERJAAN UMUM DAN\nPENATAAN RUANG";
  ws.getCell(\C\\).alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };
  ws.getCell(\C\\).font = { bold: true, size: 9 };

  ws.mergeCells(\C\:E\\);
  ws.getCell(\C\\).value = project.kadis_name || "( Nama )";
  ws.getCell(\C\\).alignment = { horizontal: 'center' };
  ws.getCell(\C\\).font = { bold: true, underline: true };

  ws.mergeCells(\C\:E\\);
  ws.getCell(\C\\).value = project.kadis_nip ? \NIP. \\ : "NIP. ..................................";
  ws.getCell(\C\\).alignment = { horizontal: 'center' };
  ws.getCell(\C\\).font = { size: 9 };

  // Diteliti (Kabid)
  ws.mergeCells(\I\:K\\);
  ws.getCell(\I\\).value = "Diteliti :";
  ws.getCell(\I\\).alignment = { horizontal: 'center' };

  ws.mergeCells(\I\:K\\);
  ws.getCell(\I\\).value = "KEPALA BIDANG CIPTA KARYA";
  ws.getCell(\I\\).alignment = { horizontal: 'center', vertical: 'middle', wrapText: true };
  ws.getCell(\I\\).font = { bold: true, size: 9 };

  ws.mergeCells(\I\:K\\);
  ws.getCell(\I\\).value = project.kabid_name || "( Nama )";
  ws.getCell(\I\\).alignment = { horizontal: 'center' };
  ws.getCell(\I\\).font = { bold: true, underline: true };

  ws.mergeCells(\I\:K\\);
  ws.getCell(\I\\).value = project.kabid_nip ? \NIP. \\ : "NIP. ..................................";
  ws.getCell(\I\\).alignment = { horizontal: 'center' };
  ws.getCell(\I\\).font = { size: 9 };
