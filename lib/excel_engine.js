...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  const ppnPercent = project?.ppn_percent !== undefined ? Number(project.ppn_percent) : (project?.ppn !== undefined ? Number(project.ppn) : 11);
  const globalOverhead = project?.profit_percent !== undefined ? Number(project.profit_percent) : (project?.overhead_percent !== undefined ? Number(project.overhead_percent) : 15);
