...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  if (selectedSheets.includes('HARGA SATUAN')) {
    ...
    // First pass: Calculate total usage (Volume) for each resource across the project
    if (Array.isArray(enrichedLines)) {
      enrichedLines.forEach(line => {
        // Fallback robust untuk mengambil details
        ...
        details.forEach(d => {
          ...
          const usage = Number(d.koefisien || 0) * rabVolume;
          resources[itemCode].totalVolume += usage;
        });
      });
    }

    // Calculate total cost for % basis
    Object.values(resources).forEach(r => {
      totalProjectCostForPercent += r.totalVolume * r.harga;
    });

    // Lookup sheet dengan berbagai kemungkinan nama (Case Sensitive & Spaces)
    const ws = workbook.getWorksheet('Harga Satuan') || 
               workbook.getWorksheet('harga satuan terpakai') || 
               workbook.getWorksheet('harga_satuan_terpakai') ||
               workbook.getWorksheet('Harga_Satuan');
    ...
    if (ws) {
      ...
      applyBorder(ws, 6, 'B', 'L'); // Header border up to L
      ...
      groups.forEach(group => {
        ...
        if (items.length > 0) {
          ...
          items.forEach((r, idx) => {
            ...
            ws.getCell(`J${currentRow}`).value = ''; // Ket
            
            const itemTotal = r.totalVolume * r.harga;
            const percent = totalProjectCostForPercent > 0 ? itemTotal / totalProjectCostForPercent : 0;
            ws.getCell(`K${currentRow}`).value = percent;
