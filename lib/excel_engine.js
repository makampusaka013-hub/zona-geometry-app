...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  if (selectedSheets.includes('HARGA SATUAN')) {
    ...
    enrichedLines.forEach(line => {
      ...
      details.forEach(d => {
        const itemCode = d.kode_item || d.kode;
        if (!itemCode) return;
        if (!resources[itemCode]) {
          ...
          if (itemPrice === 0) {
            itemPrice = Number(priceMap[itemCode] || d.harga_satuan || 0);
          }
  ...
  if (selectedSheets.includes('HSP') || selectedSheets.includes('AHSP')) {
    ...
    Object.entries(groupedItems).forEach(([babTitle, subBabs], bIdx) => {
      ...
      if (wsAHSP) {
        ...
        wsAHSP.getCell(`C${ahspRow}`).font = { color: { argb: 'FACC15' } }; // Yellowish for header
        ...
        Object.entries(categories).forEach(([catTitle, items], cIdx) => {
          ...
          items.forEach((line) => {
            ...
            if (wsAHSP) {
              ...
              wsAHSP.getCell(`M${mainHeaderRow}`).font = { bold: false }; // No bold
              ...
              wsAHSP.getCell(`N${mainHeaderRow}`).font = { bold: false }; // No bold
