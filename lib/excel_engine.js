...
const setupPrinter = (ws, companyName, printArea = null, paperSize = 'A4', orientation = 'portrait', skipTitles = false) => {
  ...
  ws.headerFooter.oddFooter = `&L&8By : &"Arial,Bold"&KFF8C00 ZG &R&8&P / &N`;
};
...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
      ...
      Object.entries(groupedRAB).forEach(([babTitle, items], bIdx) => {
        ...
        items.forEach((line, iIdx) => {
          ...
          wsRAB.getCell(`I${rabRow}`).value = { 
            ...
            result: Math.round(Number(line.volume || 0) * line.rounded_harga)
          };
          wsRAB.getCell(`K${rabRow}`).value = { 
            formula: `I${rabRow}*J${rabRow}`,
            result: Math.round(Number(line.volume || 0) * line.rounded_harga * line.calculated_tkdn)
          };
  ...
  worksheets.forEach(ws => {
    ...
    // BYPASS KHUSUS UNTUK COVER:
    // Karena cover menggunakan injeksi gambar (image injection), 
    // secara teknis selnya kosong sehingga hasData akan false.
    // Kita harus mem-bypass ini agar sheet cover tidak terhapus.
  ...
  workbook.eachSheet(ws => {
    ...
    if (lowName === 'cover' || lowName.includes('halaman depan')) {
      ...
      // Periksa konten di area data (Kolom B sampai O)
    ...
    if (!hasData) {
      ...
      const lastRow = Math.max(highestRowWithData + 1, 10); // Minimal 10 baris agar aman
      
      let lastCol = 'K'; 
      if (lowName.includes('ahsp')) {
        lastCol = 'O';
      } else if (lowName.includes('hsp')) {
        lastCol = 'I';
      } else if (lowName.includes('terpakai')) {
        lastCol = 'M';
      } else if (lowName === 'harga satuan' || lowName === 'harga_satuan' || lowName === 'harga satuan master') {
        lastCol = 'J';
      } else if (lowName.includes('rekap')) {
        lastCol = 'I';
      }
  ...
  // Hapus sheet kosong setelah loop selesai (mencegah korupsi index)
