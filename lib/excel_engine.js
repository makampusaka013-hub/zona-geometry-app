  ...
  worksheets.forEach(ws => {
    ...
    ws.eachRow({ includeEmpty: false }, (row, rowNumber) => {
      if (rowNumber >= startRow) {
        // Check if row has any non-empty cell in core columns
        for (let i = 2; i <= 10; i++) {
    ...
    if (!hasData) {
      ...
      // Tentukan Print Area secara otomatis berdasarkan baris terakhir yang ada datanya
      const lastRow = ws.actualRowCount;
      const lastCol = ws.name.toLowerCase().includes('ahsp') ? 'N' : 
                      ws.name.toLowerCase().includes('terpakai') ? 'L' : 'K';
      const pArea = `B1:${lastCol}${lastRow}`;
