...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  if (selectedSheets.includes('RAB')) {
    ...
    if (wsRAB) {
      ...
      renderSignatures(wsRAB, rabRow + 4, projectIdentity);
    }
  }
  ...
  if (selectedSheets.includes('REKAP')) {
    ...
    if (wsRekap) {
      ...
      wsRekap.getCell(`G${grandTotalRow}`).value = { formula: `G${totalRow}+G${ppnRow}` };
      ...
      // --- RENDER SIGNATURES ---
      renderSignatures(wsRekap, rekapRow + 4, projectIdentity);
...
function renderSignatures(ws, startRow, project) {
  ...
  const leftColRange = isRekap ? `C:E` : `C:E`;
  const rightColRange = isRekap ? `G:I` : `I:K`;
  const leftStart = isRekap ? 'C' : 'C';
  const rightStart = isRekap ? 'G' : 'I';

  // 1. TANGGAL
  ws.mergeCells(`${rightStart}${startRow}:${isRekap ? 'I' : 'K'}${startRow}`);
  ...
  // Diperiksa
  ws.mergeCells(`${leftStart}${row1}:${isRekap ? 'E' : 'E'}${row1}`);
  ws.getCell(`${leftStart}${row1}`).value = "Diperiksa :";
  ws.getCell(`${leftStart}${row1}`).alignment = { horizontal: 'center' };
  
  ws.mergeCells(`${leftStart}${row1 + 1}:${isRekap ? 'E' : 'E'}${row1 + 2}`);
  ...
  ws.mergeCells(`${leftStart}${rowNames1}:${isRekap ? 'E' : 'E'}${rowNames1}`);
  ...
  ws.mergeCells(`${leftStart}${rowNips1}:${isRekap ? 'E' : 'E'}${rowNips1}`);
  ...
  ws.mergeCells(`${rightStart}${rowNames1}:${isRekap ? 'I' : 'K'}${rowNames1}`);
  ...
  ws.mergeCells(`${rightStart}${rowNips1}:${isRekap ? 'I' : 'K'}${rowNips1}`);
  ...
  ws.mergeCells(`${leftStart}${rowNames2}:${isRekap ? 'E' : 'E'}${rowNames2}`);
  ...
  ws.mergeCells(`${leftStart}${rowNips2}:${isRekap ? 'E' : 'E'}${rowNips2}`);
  ...
  // Diteliti
  ws.mergeCells(`${rightStart}${row2}:${isRekap ? 'I' : 'K'}${row2}`);
  ws.getCell(`${rightStart}${row2}`).value = "Diteliti :";
  ws.getCell(`${rightStart}${row2}`).alignment = { horizontal: 'center' };

  ws.mergeCells(`${rightStart}${row2 + 1}:${isRekap ? 'I' : 'K'}${row2 + 2}`);
  ...
  ws.mergeCells(`${rightStart}${rowNames2}:${isRekap ? 'I' : 'K'}${rowNames2}`);
  ...
  ws.mergeCells(`${rightStart}${rowNips2}:${isRekap ? 'I' : 'K'}${rowNips2}`);
