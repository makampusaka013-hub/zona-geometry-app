        ...
        groups.forEach(group => {
          ...
          if (items.length > 0) {
            ...
            items.forEach((r, idx) => {
              ...
              ws.getCell(`I${currentRow}`).value = { formula: `G${currentRow}*H${currentRow}`, result: Number(r.totalVolume * r.harga) || 0 };
            ...
            items.forEach((line) => {
              ...
              if (wsHSP) {
                ...
                if (isStandalone) {
                  ...
                  wsHSP.getCell(`D${hspRow}`).value = { formula: `IFERROR(VLOOKUP(A${hspRow}, ahsp!A:N, 4, FALSE), "")`, result: Number(uraianValue) || 0 };
                  wsHSP.getCell(`E${hspRow}`).value = { formula: `IFERROR(VLOOKUP(A${hspRow}, ahsp!A:N, 6, FALSE), "")`, result: Number(satuanValue) || 0 };
                  wsHSP.getCell(`F${hspRow}`).value = { formula: `ROUND(IFERROR(VLOOKUP(A${hspRow}, ahsp!A:N, 13, FALSE), 0), 0)`, result: Number(hargaValue) || 0 };
                  wsHSP.getCell(`G${hspRow}`).value = { formula: `IFERROR(VLOOKUP(A${hspRow}, ahsp!A:N, 14, FALSE), 0)`, result: Number(tkdnValue) || 0 };
                }
                  ...
                  filtered.forEach((d) => {
                    ...
                    if (isStandalone || !selectedSheets.includes('HARGA SATUAN')) {
                      ...
                      wsAHSP.getCell(`H${ahspRow}`).value = {
                        ...
                        result: Number(itemPrice) || 0
                      };
                    }
                    wsAHSP.getCell(`H${ahspRow}`).numFmt = '#,##0.00';

                    const subtotalFormula = { formula: `G${ahspRow}*H${ahspRow}`, result: Number(subtotal) || 0 };
