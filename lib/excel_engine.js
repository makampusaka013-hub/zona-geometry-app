...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  // ==========================================
  // 0. Process cover (Premium Layout based on Image)
  // ==========================================
  if (selectedSheets.some(s => s.toLowerCase() === 'cover')) {
    ...
    if (wsCover) {
      ...
      // 1. Header (Top Area)
      wsCover.getCell('B3').value = `${month} - ${year}`; // Top Left
      wsCover.getCell('G2').value = projectCode; // Top Right

      // 2. Main Title (Center)
      wsCover.getCell('B5').value = "LAPORAN";
      wsCover.getCell('B6').value = "RENCANA ANGGARAN BIAYA";
      wsCover.getRow(5).font = { size: 28, bold: true, color: { argb: 'FFFFFFFF' } }; // Putih (Asumsi Bg Gelap)
      wsCover.getRow(6).font = { size: 28, bold: true, color: { argb: 'FFFFFFFF' } };

      // 3. Project Detail (Below Title)
      wsCover.getCell('B10').value = projectCode;
      wsCover.getCell('B11').value = projectName;
      wsCover.getCell('B12').value = location;
      
      [10, 11, 12].forEach(r => {
        wsCover.getRow(r).font = { size: 12, bold: true, color: { argb: 'FFFFFFFF' } };
        wsCover.getCell(`B${r}`).alignment = { horizontal: 'left', wrapText: true };
      });

      // 4. Compiled By (Bottom Right Area)
      ...
      wsCover.getCell('G31').font = { size: 14, bold: true, color: { argb: 'FFFFFFFF' } };

      // 5. Total Project Cost (Optional floating value)
      ...
      wsCover.getCell('B35').font = { size: 10, bold: true, color: { argb: 'FF666666' } };
