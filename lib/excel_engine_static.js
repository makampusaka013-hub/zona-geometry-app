...
const setupPrinter = (ws, companyName, printArea = null, paperSize = 'A4', orientation = 'portrait') => {
  ...
  ws.headerFooter.oddFooter = isCover ? '' : '&L&8By : ZG &R&8&P / &N';
};
...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  // ==========================================
  // 1. Process Resources (Always computed for DB sheets)
  // ==========================================
  const resources = {};
          ...
          items.forEach((line) => {
            ...
            if (wsAHSP) {
              ...
              if (startDetailRow <= endDetailRow) {
                ...
                details.forEach(d => {
                  const k = Number(d.koefisien || 0);
                  const p = Number(priceMap[d.kode_item || d.kode || d.id] || d.harga_satuan_snapshot || d.harga_satuan || d.harga || 0);
    ...
    if (wsRAB) {
      ...
      Object.entries(groupedRAB).forEach(([babTitle, items], bIdx) => {
        ...
        items.forEach((line, iIdx) => {
          ...
          wsRAB.getCell(`I${rabRow}`).value = Math.round(Number(line.volume || 0) * line.rounded_harga);
          wsRAB.getCell(`K${rabRow}`).value = Math.round(Number(line.volume || 0) * line.rounded_harga * line.calculated_tkdn);
        ...
        const babSubtotal = items.reduce((s, l) => s + Math.round(Number(l.volume || 0) * l.rounded_harga), 0);
        const babTkdnTotal = items.reduce((s, l) => s + Math.round(Number(l.volume || 0) * l.rounded_harga * l.calculated_tkdn), 0);
      ...
      // Total Keseluruhan
      const grandSubtotal = enrichedLines.reduce((s, l) => s + Math.round(Number(l.volume || 0) * l.rounded_harga), 0);
      const grandTkdnTotal = enrichedLines.reduce((s, l) => s + Math.round(Number(l.volume || 0) * l.rounded_harga * l.calculated_tkdn), 0);
  ...
  if (selectedSheets.includes('REKAP')) {
    ...
    if (wsRekap) {
      ...
      // BAGIAN TOTAL & PPN
      // -------------------------------------
      const totalSum = Object.values(groupedRekap).flat().reduce((s, l) => s + Math.round(Number(l.volume || 0) * l.rounded_harga), 0);
  ...
  worksheets.forEach(ws => {
    ...
    if (!hasData) {
      ...
      else if (lowName === 'cover') lastCol = 'N';
  ...
  // ==========================================
  // 4. Create Hidden Database Sheets
  // ==========================================
  const dbSheets = [
    { name: 'database pekerja', type: 'resource', filter: ['upah', 'tenaga', 'pekerja'] },
    { name: 'database bahan', type: 'resource', filter: ['bahan'] },
    { name: 'database alat', type: 'resource', filter: ['alat', 'peralatan', 'mesin'] },
    { name: 'database harga', type: 'resource', filter: 'all' },
    { name: 'database volume', type: 'volume' }
  ];

  for (const db of dbSheets) {
    let ws = workbook.getWorksheet(db.name);
    if (!ws) ws = workbook.addWorksheet(db.name);
    
    // Reset Worksheet
    ws.eachRow((row) => { ws.removeRow(row.number); });
    
    // Setup Headers
    ws.columns = [
      { header: 'NO', key: 'no', width: 8 },
      { header: 'URAIAN PEKERJAAN / KOMPONEN', key: 'uraian', width: 50 },
      { header: 'KODE', key: 'kode', width: 20 },
      { header: 'SATUAN', key: 'satuan', width: 12 },
      { header: 'HARGA SATUAN', key: 'harga', width: 20 },
      { header: 'VOLUME TOTAL', key: 'volume', width: 20 },
      { header: 'JUMLAH HARGA', key: 'total', width: 25 }
    ];
    
    // Styling Header
    ws.getRow(1).font = { bold: true, color: { argb: 'FFFFFFFF' } };
    ws.getRow(1).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FF1E293B' } }; // Slate-800
    ws.getRow(1).alignment = { horizontal: 'center', vertical: 'middle' };

    // Populate
    if (db.type === 'resource') {
      const items = Object.values(resources || {}).filter(r => {
        if (db.filter === 'all') return true;
        const lowJenis = (r.jenis || '').toLowerCase();
        return db.filter.some(f => lowJenis.includes(f));
      });
      items.forEach((item, idx) => {
        const row = ws.addRow({
          ...
          total: (item.harga || 0) * (item.totalVolume || 0)
        });
        row.getCell('harga').numFmt = '#,##0.00';
        row.getCell('volume').numFmt = '#,##0.0000';
        row.getCell('total').numFmt = '#,##0.00';
      });
    } else if (db.type === 'volume') {
      enrichedLines.forEach((line, idx) => {
        const row = ws.addRow({
          no: idx + 1,
          uraian: line.uraian || line.nama_pekerjaan,
          kode: line.master_ahsp?.kode_ahsp || line.kode_ahsp || '-',
          satuan: line.satuan || line.satuan_pekerjaan,
          harga: line.harga_satuan,
          volume: line.volume,
          total: (line.volume || 0) * (line.harga_satuan || 0)
        });
        row.getCell('harga').numFmt = '#,##0.00';
        row.getCell('volume').numFmt = '#,##0.0000';
        row.getCell('total').numFmt = '#,##0.00';
    ...
    // Hide the sheet but keep it available for unhiding
    ws.state = 'hidden';
  }
