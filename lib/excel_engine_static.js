...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  if (selectedSheets.includes('HARGA SATUAN') || selectedSheets.includes('HARGA SATUAN TERPAKAI')) {
    ...
    // Calculate usage
    if (options.isCatalog && options.catPrice && options.catPrice.length > 0) {
      options.catPrice.forEach(p => {
        const itemCode = p.kode_item || p.kode || p.id;
        const mItem = Array.isArray(p.master_items) ? p.master_items[0] : p.master_items;
        resources[itemCode] = {
           kode: itemCode,
           uraian: mItem?.uraian || p.uraian || '-',
           satuan: mItem?.satuan || p.satuan || '-',
           harga: p.harga_satuan,
           jenis: (mItem?.jenis_komponen || p.jenis_komponen || 'Lainnya').toLowerCase(),
           tkdn: p.tkdn_percent || 0,
           totalVolume: 0
        };
      });
    ...
    if (selectedSheets.includes('HARGA SATUAN')) {
      ...
      if (ws) {
        ...
        const groups = [
          ...
          { label: 'LAIN-LAIN', types: ['lainnya', 'lain-lain', ''] }
        ];
