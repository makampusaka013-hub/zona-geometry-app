...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  if (isCatalog && Array.isArray(catPrice) && catPrice.length > 0) {
    catPrice.forEach(p => { 
      const itemCode = p.kode_item || p.kode || p.id;
      if (itemCode) priceMap[itemCode] = safeNum(p.harga_satuan || p.harga); 
    });
  }
  ...
  if (headerImage) {
    try {
      ...
      // Fungsi untuk memaksa convert gambar apa pun (webp/svg/dll) menjadi PNG murni via Canvas
      const forceValidPngBase64 = async (imgSrc) => {
      ...
      if (headerImage.startsWith('http')) {
        ...
        base64Murni = await forceValidPngBase64(objectUrl);
        URL.revokeObjectURL(objectUrl);
      } else {
        base64Murni = await forceValidPngBase64(headerImage);
      }
      extension = 'png';
  ...
  if (selectedSheets.includes('HARGA SATUAN') || selectedSheets.includes('HARGA SATUAN TERPAKAI')) {
    ...
    if (options.isCatalog && options.catPrice && options.catPrice.length > 0) {
      options.catPrice.forEach(p => {
        const itemCode = p.kode_item;
        resources[itemCode] = {
           kode: itemCode,
           uraian: p.master_items?.uraian || p.uraian || '-',
           satuan: p.master_items?.satuan || p.satuan || '-',
           harga: p.harga_satuan,
           jenis: (p.master_items?.jenis_komponen || p.jenis_komponen || 'Lainnya').toLowerCase(),
          ...
          Object.entries(categories).forEach(([catTitle, items], cIdx) => {
            ...
            items.forEach((line) => {
              ...
              if (wsHSP) {
                ...
                if (!selectedSheets.includes('AHSP')) {
              ...
              if (startDetailRow <= endDetailRow) {
                ...
                wsAHSP.getCell(`I${mainHeaderRow}`).value = { formula: `SUM(I${startDetailRow}:I${endDetailRow})`, result: sT };
                wsAHSP.getCell(`J${mainHeaderRow}`).value = { formula: `SUM(J${startDetailRow}:J${endDetailRow})`, result: sB };
                wsAHSP.getCell(`K${mainHeaderRow}`).value = { formula: `SUM(K${startDetailRow}:K${endDetailRow})`, result: sA };
              }
              ...
              wsAHSP.getCell(`M${mainHeaderRow}`).value = { formula: `ROUND((I${mainHeaderRow}+J${mainHeaderRow}+K${mainHeaderRow})*(1+L${mainHeaderRow}), 0)`, result: line.rounded_harga };
              wsAHSP.getCell(`M${mainHeaderRow}`).numFmt = '_(Rp* #,##0.00_);_(Rp* (#,##0.00);_(Rp* "-"??_);_(@_)';
              wsAHSP.getCell(`M${mainHeaderRow}`).font = { bold: false };
              if (startDetailRow <= endDetailRow) {
                wsAHSP.getCell(`N${mainHeaderRow}`).value = { formula: `AVERAGE(N${startDetailRow}:N${endDetailRow})`, result: safeNum(line.tkdn || line.total_tkdn_percent || 0) / 100 };
              } else {
    ...
    if (wsRAB) {
      ...
      Object.entries(groupedRAB).forEach(([babTitle, items], bIdx) => {
        ...
        items.forEach((line, iIdx) => {
          ...
          if (!selectedSheets.includes('HSP')) {
            ...
            wsRAB.getCell(`H${rabRow}`).value = { 
              ...
              result: line.rounded_harga
            };
            wsRAB.getCell(`J${rabRow}`).value = { 
              formula: `IFERROR(VLOOKUP(C${rabRow}, 'hsp'!D:G, 4, FALSE), 0)`,
              result: line.calculated_tkdn
            };
          }
          
          wsRAB.getCell(`I${rabRow}`).value = { 
            formula: `G${rabRow}*H${rabRow}`,
            result: Math.round(safeNum(line.volume || 0) * line.rounded_harga)
          };
          wsRAB.getCell(`K${rabRow}`).value = { 
            formula: `I${rabRow}*J${rabRow}`,
            result: Math.round(safeNum(line.volume || 0) * line.rounded_harga * line.calculated_tkdn)
          };
        ...
        wsRAB.getCell(`I${rabRow}`).value = { formula: `SUM(I${startBabRow}:I${endBabRow})`, result: babSubtotal };
        wsRAB.getCell(`J${rabRow}`).value = { formula: `IFERROR(K${rabRow}/I${rabRow}, 0)`, result: babTkdnPercent };
        wsRAB.getCell(`K${rabRow}`).value = { formula: `SUM(K${startBabRow}:K${endBabRow})`, result: babTkdnTotal };
        wsRAB.getRow(rabRow).font = { bold: true };
        wsRAB.getCell(`I${rabRow}`).numFmt = '#,##0.00';
      ...
      wsRAB.getCell(`I${rabRow}`).value = { formula: `SUM(I12:I${rabRow - 1})/2`, result: grandSubtotal };
      wsRAB.getCell(`J${rabRow}`).value = { formula: `IFERROR(K${rabRow}/I${rabRow}, 0)`, result: grandTkdnPercent };
      wsRAB.getCell(`K${rabRow}`).value = { formula: `SUM(K12:K${rabRow - 1})/2`, result: grandTkdnTotal };
      wsRAB.getRow(rabRow).font = { bold: true };
      wsRAB.getCell(`I${rabRow}`).numFmt = '#,##0.00';
      wsRAB.getCell(`J${rabRow}`).numFmt = '0.00%'; // Format TKDN
      wsRAB.getCell(`K${rabRow}`).numFmt = '#,##0.00';
  ...
  if (selectedSheets.includes('REKAP')) {
    ...
    if (wsRekap) {
      ...
      wsRekap.getCell(`F${totalRow}`).value = { formula: `SUM(F12:F${endDataRow})`, result: totalSum };
      ...
      wsRekap.getCell(`F${ppnRow}`).value = { formula: `F${totalRow}*${ppnPercent/100}`, result: ppnVal };
      
      const grandTotalRow = rekapRow + 2;
      wsRekap.getCell(`C${grandTotalRow}`).value = 'JUMLAH TOTAL HARGA PEKERJAAN';
      wsRekap.getCell(`F${grandTotalRow}`).value = { formula: `F${totalRow}+F${ppnRow}`, result: totalSum + ppnVal };
      ...
      Object.entries(groupedSched).forEach(([babTitle, items], bIdx) => {
        ...
        items.forEach((item) => {
          ...
          for(let w = 0; w < itemWeeksSpanned; w++) {
            ...
            if (targetCol <= endCol) {
               wsSched.getCell(currentRow, targetCol).value = { formula: `IFERROR($E$${currentRow}/${itemWeeksSpanned}, 0)`, result: itemBobotVal / itemWeeksSpanned };
  ...
  worksheets.forEach(ws => {
    ...
    // Trik Anti Corrupt 1: Hapus Print Area jika hidden
    if (!isSelected) {
    ...
    // Trik Anti Corrupt 2: Hapus Print Area jika sheet kosong
    if (!hasData) {
  ...
  // Trik Anti Corrupt 3: Active Tab & First Sheet WAJIB sejajar dengan sheet yang visible
