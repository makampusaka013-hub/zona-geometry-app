...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  const globalOverhead = project?.profit_margin ?? project?.profit_percent ?? project?.overhead_percent ?? 11;
        ...
        Object.entries(categories).forEach(([catTitle, items], cIdx) => {
          ...
          items.forEach((line) => {
            ...
            if (wsAHSP) {
              ...
              // 1. Prioritas Utama: Nilai Profit spesifik per baris (profit_margin)
              let profitMargin = line.profit_margin !== undefined && line.profit_margin !== null ? safeNum(line.profit_margin) : null;

              if (profitMargin === null) {
                if (line.profit_percent !== null && line.profit_percent !== undefined) {
                  profitMargin = safeNum(line.profit_percent);
                } else if (line.profitPercent !== null && line.profitPercent !== undefined) {
                  profitMargin = safeNum(line.profitPercent);
                } else if (line.overhead_percent !== null && line.overhead_percent !== undefined) {
                  profitMargin = safeNum(line.overhead_percent);
                } else if (line.overheadPercent !== null && line.overheadPercent !== undefined) {
                  profitMargin = safeNum(line.overheadPercent);
                }
              }
              
              // 2. Prioritas Kedua: Reverse Engineer dari Harga Satuan jika data di atas kosong
              if (profitMargin === null && calcBase > 0 && safeNum(line.harga_satuan) > 0) {
                profitMargin = Math.round(((safeNum(line.harga_satuan) / calcBase) - 1) * 100);
              }
              
              // 3. Prioritas Ketiga: Nilai Profit/Overhead Global Proyek
              if (profitMargin === null) {
                profitMargin = options.globalOverhead || globalOverhead || 11;
              }

              wsAHSP.getCell(`L${mainHeaderRow}`).value = safeNum(profitMargin) / 100;
              ...
              if (startDetailRow <= endDetailRow) {
                // Kalkulasi manual untuk summary baris utama
                const currentDetails = details.filter(d => d.koefisien); // Sederhana
                let sT = 0, sB = 0, sA = 0;
                details.forEach(d => {
                  const k = safeNum(d.koefisien || 0);
                ...
                wsAHSP.getCell(`I${mainHeaderRow}`).value = sT;
                wsAHSP.getCell(`J${mainHeaderRow}`).value = sB;
                wsAHSP.getCell(`K${mainHeaderRow}`).value = sA;
              }
              ...
              wsAHSP.getCell(`M${mainHeaderRow}`).value = line.rounded_harga;
              wsAHSP.getCell(`M${mainHeaderRow}`).numFmt = '_(Rp* #,##0.00_);_(Rp* (#,##0.00);_(Rp* "-"??_);_(@_)';
              wsAHSP.getCell(`M${mainHeaderRow}`).font = { bold: false };
              if (startDetailRow <= endDetailRow) {
                wsAHSP.getCell(`N${mainHeaderRow}`).value = safeNum(line.tkdn || line.total_tkdn_percent || 0) / 100;
              } else {
                wsAHSP.getCell(`N${mainHeaderRow}`).value = safeNum(line.tkdn || line.total_tkdn_percent || 0) / 100;
              }
