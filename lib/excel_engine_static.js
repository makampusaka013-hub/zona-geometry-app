        ...
        Object.entries(categories).forEach(([catTitle, items], cIdx) => {
          ...
          items.forEach((line) => {
            ...
            if (wsAHSP) {
              ...
              // 1. Prioritas Utama: Nilai Profit spesifik per baris
              let smartProfit = null;

              if (line.profit_percent !== null && line.profit_percent !== undefined) {
                smartProfit = Number(line.profit_percent);
              } else if (line.profit !== null && line.profit !== undefined) {
                smartProfit = Number(line.profit);
                ...
                smartProfit = Number(line.overhead_percent);
              } else if (line.overheadPercent !== null && line.overheadPercent !== undefined) {
                smartProfit = Number(line.overheadPercent);
              } else if (line.profit_margin !== undefined && line.profit_margin !== null) {
                smartProfit = Number(line.profit_margin);
              }

              // 2. Prioritas Kedua: Reverse Engineer dari Harga Satuan jika data di atas kosong
              if (smartProfit === null && calcBase > 0 && safeNum(line.harga_satuan) > 0) {
                smartProfit = Math.round(((safeNum(line.harga_satuan) / calcBase) - 1) * 100);
              }

              // 3. Prioritas Ketiga: Nilai Profit/Overhead Global Proyek
              if (smartProfit === null) {
                smartProfit = options.globalOverhead ?? globalOverhead ?? 10;
              }

              wsAHSP.getCell(`L${mainHeaderRow}`).value = Number(smartProfit) / 100;
