...
const cleanStr = (val) => {
  ...
  // Hapus karakter kontrol non-printable (ASCII < 32) kecuali Tab/LF/CR
...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  const companyName = cleanStr(user?.full_name || 'ZONA GEOMETRY');
  const projectLocation = cleanStr(project.location || project.address || '-');
  const ppnPercent = project?.ppn_percent ?? project?.ppn ?? 11;
  const globalOverhead = project?.profit_percent ?? project?.overhead_percent ?? 10;
  ...
  const enrichedLines = sortedLines.map(line => {
    ...
    detailsForTkdn.forEach(d => {
      const k = safeNum(d.koefisien || 0);
      const itemCode = d.kode_item || d.kode || d.id;
      const p = safeNum(priceMap[itemCode] || d.harga_satuan_snapshot || d.harga_satuan || d.harga || 0);
      const dTkdn = safeNum(d.tkdn || d.tkdn_percent || 0);
      totalNilai += (k * p);
      totalTkdnValue += (k * p) * (dTkdn / 100);
    });
    const itemTkdnPercent = totalNilai > 0 ? (totalTkdnValue / totalNilai) : 0;

    // Pembulatan Harga Satuan (Round to nearest integer)
    const roundedHarga = Math.round(safeNum(line.harga_satuan || line.total_subtotal || 0));

    return { ...line, calculated_tkdn: itemTkdnPercent, rounded_harga: roundedHarga };
  });

  // Cover dipindah ke Advance Only. Statis Engine tidak merender cover.
  ...
  if (selectedSheets.includes('HARGA SATUAN') || selectedSheets.includes('HARGA SATUAN TERPAKAI')) {
    ...
    if (Array.isArray(enrichedLines)) {
      enrichedLines.forEach(line => {
        ...
        const rabVolume = safeNum(line.volume || 0);

        details.forEach(d => {
          const itemCode = d.kode_item || d.kode || d.id;
          if (!itemCode) return;
          if (!resources[itemCode]) {
            let itemPrice = safeNum(d.harga_satuan_snapshot || 0);
            if (itemPrice === 0) itemPrice = safeNum(priceMap[itemCode] || d.harga_satuan || d.harga || 0);
            resources[itemCode] = {
              kode: itemCode,
              ...
              tkdn: safeNum(d.tkdn || d.tkdn_percent || 0),
              totalVolume: 0
            };
          }
          resources[itemCode].totalVolume += safeNum(d.koefisien || 0) * rabVolume;
        });
      });
    }
    ...
    if (selectedSheets.includes('HARGA SATUAN')) {
      ...
      if (ws) {
        ...
        Object.values(resources).forEach(r => {
          ...
          if (jenis.includes('upah') || jenis.includes('tenaga')) {
            grouped['TENAGA KERJA'].push(r);
          } else if (jenis.includes('alat') || jenis.includes('peralatan') || jenis.includes('mesin')) {
            grouped['ALAT'].push(r);
          } else {
            grouped['BAHAN'].push(r);
          }
        });

        ['TENAGA KERJA', 'BAHAN', 'ALAT'].forEach(label => {
          ...
          if (items.length > 0) {
            ...
            items.forEach((r, idx) => {
              ...
              ws.getCell(`G${currentRow}`).value = safeNum(r.harga);
              ws.getCell(`G${currentRow}`).numFmt = '#,##0.00';
              ws.getCell(`I${currentRow}`).value = safeNum(r.tkdn) / 100;
    ...
    if (selectedSheets.includes('HARGA SATUAN TERPAKAI')) {
      ...
      if (ws) {
        ...
        ['TENAGA KERJA', 'BAHAN', 'ALAT'].forEach(label => {
          ...
          if (items.length > 0) {
            currentRow++;
            ws.getCell(`C${currentRow}`).value = label;
            ws.getRow(currentRow).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'F1F5F9' } };
            ...
            items.forEach((r, idx) => {
              ...
              ws.getCell(`F${currentRow}`).value = r.satuan;
              ws.getCell(`F${currentRow}`).alignment = { vertical: 'middle', horizontal: 'center' };
              ws.getCell(`G${currentRow}`).value = safeNum(r.totalVolume.toFixed(4));
              ws.getCell(`G${currentRow}`).alignment = { vertical: 'middle', horizontal: 'center' };
              ws.getCell(`G${currentRow}`).numFmt = '#,##0.00';
              ws.getCell(`H${currentRow}`).value = safeNum(r.harga);
              ws.getCell(`I${currentRow}`).value = r.totalVolume * r.harga;
              ws.getCell(`K${currentRow}`).value = totalProjectCostForPercent > 0 ? (r.totalVolume * r.harga) / totalProjectCostForPercent : 0;
              ws.getCell(`L${currentRow}`).value = safeNum(r.tkdn) / 100;
  ...
  if (selectedSheets.includes('HSP') || selectedSheets.includes('AHSP')) {
    ...
    if (wsHSP) {
      wsHSP.getColumn('A').font = { color: { argb: 'FFFFFFFF' } };
      for (let r = 1; r <= 4; r++) {
      ...
      wsHSP.getRow(5).getCell(7).border = {};
      clearDataRows(wsHSP, 6, 1000);
    }
    if (wsAHSP) {
      wsAHSP.getColumn('A').font = { color: { argb: 'FFFFFFFF' } };
      for (let r = 1; r <= 4; r++) {
      ...
      // Row 5: Only clear Column G border, keep Fill
      wsAHSP.getRow(5).getCell(7).border = {};
      clearDataRows(wsAHSP, 6, 5000);
    }


    const groupedItems = {};
    const ahspSource = (options.isCatalog && options.catAhsp && options.catAhsp.length > 0) ? options.catAhsp : enrichedLines;

    ahspSource.forEach(line => {
      ...
      Object.entries(subBabs).forEach(([subTitle, categories], sIdx) => {
        ...
        Object.entries(categories).forEach(([catTitle, items], cIdx) => {
          ...
          items.forEach((line) => {
            ...
            const uniqueId = line.id || line.master_ahsp_id;

            if (wsHSP) {
              wsHSP.getCell(`A${hspRow}`).value = uniqueId;
              wsHSP.getCell(`B${hspRow}`).value = itemCode;
              wsHSP.getCell(`C${hspRow}`).value = null;

              const uraianValue = cleanStr(line.uraian || line.nama_pekerjaan || '-');
              const satuanValue = cleanStr(line.satuan || line.satuan_pekerjaan || '-');
              const hargaValue = line.rounded_harga;
              const tkdnValue = line.calculated_tkdn;
            ...
            if (wsAHSP) {
              ...
              wsAHSP.getCell(`A${mainHeaderRow}`).value = uniqueId;
              wsAHSP.getCell(`B${mainHeaderRow}`).value = itemCode;
              wsAHSP.getCell(`D${mainHeaderRow}`).value = cleanStr(((line.uraian || line.nama_pekerjaan || '-') + "").toUpperCase());
              wsAHSP.getCell(`F${mainHeaderRow}`).value = cleanStr(line.satuan || line.satuan_pekerjaan || '-');
              ...
              // 1. DEKLARASIKAN PROFIT DARI DATA (Instruksi Mutlak)
              const profitVal = (line.profit_margin !== undefined && line.profit_margin !== null) ? Number(line.profit_margin) : 11;
              const headerRow = mainHeaderRow;

              // 2. TULIS PROFIT KE KOLOM L
              wsAHSP.getCell('L' + headerRow).value = profitVal / 100;
              wsAHSP.getCell('L' + headerRow).numFmt = '0.00%';
              ...
              ahspCats.forEach(cat => {
                ...
                if (filtered.length > 0) {
                  ...
                  ahspRow++;
                  filtered.forEach((d) => {
                    const koef = safeNum(d.koefisien || 0);
                    const itemCode = d.kode_item || d.kode || d.id;
                    const itemPrice = safeNum(priceMap[itemCode] || d.harga_satuan_snapshot || d.harga_satuan || d.harga || 0);
                    const subtotal = koef * itemPrice;

                    wsAHSP.getCell(`D${ahspRow}`).value = cleanStr(d.uraian);
                    wsAHSP.getCell(`E${ahspRow}`).value = cleanStr(d.kode_item || d.kode);
                    wsAHSP.getCell(`E${ahspRow}`).alignment = { vertical: 'middle', horizontal: 'center' };

                    wsAHSP.getCell(`F${ahspRow}`).value = cleanStr(d.satuan);
                    wsAHSP.getCell(`F${ahspRow}`).alignment = { vertical: 'middle', horizontal: 'center' };
                    wsAHSP.getCell(`G${ahspRow}`).value = koef;
                    wsAHSP.getCell(`G${ahspRow}`).alignment = { vertical: 'middle', horizontal: 'center' };
                    wsAHSP.getCell(`H${ahspRow}`).value = itemPrice;
                    wsAHSP.getCell(`H${ahspRow}`).numFmt = '#,##0.00';

                    if (cat.label === 'TENAGA KERJA') {
                      wsAHSP.getCell(`I${ahspRow}`).value = subtotal;
                      wsAHSP.getCell(`I${ahspRow}`).numFmt = '#,##0.00';
                    } else if (cat.label === 'BAHAN') {
                      wsAHSP.getCell(`J${ahspRow}`).value = subtotal;
                      wsAHSP.getCell(`J${ahspRow}`).numFmt = '#,##0.00';
                    } else {
                      wsAHSP.getCell(`K${ahspRow}`).value = subtotal;
                      wsAHSP.getCell(`K${ahspRow}`).numFmt = '#,##0.00';
                    }
                    wsAHSP.getCell(`N${ahspRow}`).value = safeNum(d.tkdn || d.tkdn_percent || 0) / 100;
              ...
              // Hitung Biaya Dasar (Statis)
              let sT = 0, sB = 0, sA = 0;
              if (startDetailRow <= endDetailRow) {
                details.forEach(d => {
                  const k = safeNum(d.koefisien || 0);
                  const p = safeNum(priceMap[d.kode_item || d.kode || d.id] || d.harga_satuan_snapshot || d.harga_satuan || d.harga || 0);
                  const j = (d.jenis_komponen || d.jenis || d.jenis_uraian || '').toLowerCase();
                  if (ahspCats[0].types.some(t => j.includes(t))) sT += (k * p);
                  else if (ahspCats[1].types.some(t => j.includes(t))) sB += (k * p);
                  else sA += (k * p);
                });
              }
              const totalBaseCost = sT + sB + sA;

              // A. JUMLAH BIAYA DASAR
              const baseCostRow = ahspRow;
              wsAHSP.getCell('D' + baseCostRow).value = 'JUMLAH BIAYA DASAR (Upah, Bahan & Alat)';
              wsAHSP.getCell('D' + baseCostRow).font = { bold: true };
              wsAHSP.getCell('I' + baseCostRow).value = sT;
              wsAHSP.getCell('J' + baseCostRow).value = sB;
              wsAHSP.getCell('K' + baseCostRow).value = sA;
              wsAHSP.getCell('M' + baseCostRow).value = totalBaseCost;
              applyBorder(wsAHSP, baseCostRow, 'B', 'N');
              ahspRow++;

              // B. OVERHEAD & PROFIT (Statis)
              const ohpRow = ahspRow;
              const ohpValue = totalBaseCost * (profitVal / 100);
              wsAHSP.getCell('D' + ohpRow).value = 'OVERHEAD & PROFIT (' + profitVal + '%)';
              wsAHSP.getCell('D' + ohpRow).font = { italic: true };
              wsAHSP.getCell('M' + ohpRow).value = ohpValue;
              applyBorder(wsAHSP, ohpRow, 'B', 'N');
              ahspRow++;

              // C. HARGA SATUAN PEKERJAAN
              const totalRow = ahspRow;
              const totalHargaSatuan = Math.round(totalBaseCost + ohpValue);
              wsAHSP.getCell('D' + totalRow).value = 'HARGA SATUAN PEKERJAAN (Biaya Dasar + OHP)';
              wsAHSP.getCell('D' + totalRow).font = { bold: true };
              wsAHSP.getCell('M' + totalRow).value = totalHargaSatuan;
              applyBorder(wsAHSP, totalRow, 'B', 'N');

              // Sinkronisasi ke Header (Column I, J, K, M, N)
              wsAHSP.getCell('I' + headerRow).value = sT;
              wsAHSP.getCell('J' + headerRow).value = sB;
              wsAHSP.getCell('K' + headerRow).value = sA;
              wsAHSP.getCell('M' + headerRow).value = totalHargaSatuan;

              wsAHSP.getCell(`I${headerRow}`).numFmt = '#,##0.00';
              wsAHSP.getCell(`J${headerRow}`).numFmt = '#,##0.00';
              wsAHSP.getCell(`K${headerRow}`).numFmt = '#,##0.00';
              wsAHSP.getCell(`M${headerRow}`).numFmt = '_(Rp* #,##0.00_);_(Rp* (#,##0.00);_(Rp* "-"??_);_(@_)';
              wsAHSP.getCell(`M${headerRow}`).font = { bold: false };

              wsAHSP.getCell(`N${headerRow}`).value = safeNum(line.tkdn || line.total_tkdn_percent || 0) / 100;
              wsAHSP.getCell(`N${headerRow}`).numFmt = '0.00%';
              wsAHSP.getCell(`N${headerRow}`).font = { bold: false };
              ahspRow++;
            }
          });
        });
    ...
    if (headerImageId !== null) {
      if (wsAHSP) {
        wsAHSP.getRow(1).height = 60;
        wsAHSP.addImage(headerImageId, {
          tl: { col: 0, row: 0 },
          br: { col: 14, row: 4 }, // Menutupi sampai kolom N, baris 4
          editAs: 'twoCell'
        });
      }
      if (wsHSP) {
        wsHSP.getRow(1).height = 60;
        wsHSP.addImage(headerImageId, {
          tl: { col: 0, row: 0 },
          br: { col: 8, row: 4 }, // Menutupi sampai kolom H, baris 4
          editAs: 'twoCell'
        });
      }
    }
  ...
  if (selectedSheets.includes('RAB')) {
    ...
    if (wsRAB) {
      ...
      wsRAB.getCell('E4').value = cleanStr((project?.nama_program || project?.program || '-').toUpperCase());
      wsRAB.getCell('E5').value = cleanStr((project?.nama_kegiatan || project?.kegiatan || '-').toUpperCase());
      wsRAB.getCell('E6').value = cleanStr((project?.nama_sub_kegiatan || project?.sub_kegiatan || '-').toUpperCase());
      wsRAB.getCell('E7').value = cleanStr((project?.work_name || project?.name || '-').toUpperCase());
      wsRAB.getCell('E8').value = cleanStr((project?.lokasi || projectLocation || '-').toUpperCase());
      wsRAB.getCell('E9').value = cleanStr(project?.tahun_anggaran || project?.fiscal_year || '2026');
      ...
      Object.entries(groupedRAB).forEach(([babTitle, items], bIdx) => {
        ...
        items.forEach((line, iIdx) => {
          ...
          wsRAB.getCell(`C${rabRow}`).value = cleanStr(line.uraian || '-');
          wsRAB.getCell(`F${rabRow}`).value = cleanStr(line.satuan || '-');
          wsRAB.getCell(`F${rabRow}`).alignment = { vertical: 'middle', horizontal: 'center' };
          wsRAB.getCell(`G${rabRow}`).value = safeNum(line.volume || 0);

          wsRAB.getCell(`H${rabRow}`).value = line.rounded_harga;
          wsRAB.getCell(`J${rabRow}`).value = line.calculated_tkdn;

          wsRAB.getCell(`I${rabRow}`).value = Math.round(safeNum(line.volume || 0) * line.rounded_harga);
          wsRAB.getCell(`K${rabRow}`).value = Math.round(safeNum(line.volume || 0) * line.rounded_harga * line.calculated_tkdn);
        ...
        const babSubtotal = items.reduce((s, l) => s + Math.round(safeNum(l.volume || 0) * l.rounded_harga), 0);
        const babTkdnTotal = items.reduce((s, l) => s + Math.round(safeNum(l.volume || 0) * l.rounded_harga * l.calculated_tkdn), 0);
        const babTkdnPercent = babSubtotal > 0 ? babTkdnTotal / babSubtotal : 0;

        wsRAB.getCell(`C${rabRow}`).value = `SUB TOTAL ${babTitle || ''}`.toUpperCase();
        wsRAB.getCell(`I${rabRow}`).value = babSubtotal;
        wsRAB.getCell(`J${rabRow}`).value = babTkdnPercent;
        wsRAB.getCell(`K${rabRow}`).value = babTkdnTotal;
        wsRAB.getRow(rabRow).font = { bold: true };
        wsRAB.getCell(`I${rabRow}`).numFmt = '#,##0.00';
      ...
      // Total Keseluruhan
      const grandSubtotal = enrichedLines.reduce((s, l) => s + Math.round(safeNum(l.volume || 0) * l.rounded_harga), 0);
      const grandTkdnTotal = enrichedLines.reduce((s, l) => s + Math.round(safeNum(l.volume || 0) * l.rounded_harga * l.calculated_tkdn), 0);
      const grandTkdnPercent = grandSubtotal > 0 ? grandTkdnTotal / grandSubtotal : 0;

      wsRAB.getCell(`C${rabRow}`).value = 'JUMLAH TOTAL';
      wsRAB.getCell(`I${rabRow}`).value = grandSubtotal;
      wsRAB.getCell(`J${rabRow}`).value = grandTkdnPercent;
      wsRAB.getCell(`K${rabRow}`).value = grandTkdnTotal;
      wsRAB.getRow(rabRow).font = { bold: true };
      wsRAB.getCell(`I${rabRow}`).numFmt = '#,##0.00';
      wsRAB.getCell(`J${rabRow}`).numFmt = '0.00%'; // Format TKDN
      wsRAB.getCell(`K${rabRow}`).numFmt = '#,##0.00';
  ...
  if (selectedSheets.includes('REKAP')) {
    ...
    if (wsRekap) {
      ...
      // Metadata Header
      wsRekap.getCell('E4').value = cleanStr((project?.nama_program || project?.program || '-').toUpperCase());
      wsRekap.getCell('E5').value = cleanStr((project?.nama_kegiatan || project?.kegiatan || '-').toUpperCase());
      wsRekap.getCell('E6').value = cleanStr((project?.nama_sub_kegiatan || project?.sub_kegiatan || '-').toUpperCase());
      wsRekap.getCell('E7').value = cleanStr((project?.work_name || project?.name || '-').toUpperCase());
      wsRekap.getCell('E8').value = cleanStr((project?.lokasi || projectLocation || '-').toUpperCase());
      wsRekap.getCell('E9').value = cleanStr(project?.tahun_anggaran || project?.fiscal_year || '2026');
      ...
      Object.keys(groupedRekap).forEach((babTitle) => {
        ...
        const babSubtotal = babLines.reduce((s, l) => s + Math.round(safeNum(l.volume || 0) * l.rounded_harga), 0);
        const babTkdn = babLines.reduce((s, l) => s + Math.round(safeNum(l.volume || 0) * l.rounded_harga * l.calculated_tkdn), 0);
        const babTkdnPercent = babSubtotal > 0 ? babTkdn / babSubtotal : 0;
      ...
      // BAGIAN TOTAL & PPN
      // -------------------------------------
      const totalSum = Object.values(groupedRekap).flat().reduce((s, l) => s + Math.round(safeNum(l.volume || 0) * l.rounded_harga), 0);
      const ppnVal = Math.round(totalSum * (ppnPercent / 100));

      const totalRow = rekapRow;
      wsRekap.getCell(`C${totalRow}`).value = 'Jumlah Harga Pekerjaan ( termasuk Biaya Umum dan Keuntungan )';
      wsRekap.getCell(`F${totalRow}`).value = totalSum;

      const ppnRow = rekapRow + 1;
      const displayPPN = safeNum(ppnPercent) === 0 ? "0%" : `${ppnPercent}%`;
      wsRekap.getCell(`C${ppnRow}`).value = `( Pajak Pertambahan Nilai ( PPN ) = ${displayPPN} )`;
      wsRekap.getCell(`F${ppnRow}`).value = ppnVal;

      const grandTotalRow = rekapRow + 2;
      wsRekap.getCell(`C${grandTotalRow}`).value = 'JUMLAH TOTAL HARGA PEKERJAAN';
      wsRekap.getCell(`F${grandTotalRow}`).value = totalSum + ppnVal;

      // Formatting & Border Bagian Total
      for (let r = totalRow; r <= grandTotalRow; r++) {
        wsRekap.getCell(`F${r}`).numFmt = (r === grandTotalRow) ? '#,##0' : '#,##0.00';
        if (r === grandTotalRow) {
          wsRekap.getCell(`F${r}`).value = Math.round(totalSum + ppnVal);
        }
  ...
  if (selectedSheets.some(s => s.toLowerCase() === 'schedule') && Array.isArray(options.scheduleData) && options.scheduleData.length > 0) {
    ...
    if (wsSched) {
      ...
      for (let w = 0; w < totalWeeks; w++) {
        ...
        // Baris 7: M1, M2, M3, M4 (Reset cycle)
        wsSched.getCell(7, col).value = `M${mCount}`;
        wsSched.getCell(7, col).alignment = { horizontal: 'center' };
        mCount++; if (mCount > 4) mCount = 1;

        // Baris 6: Merge Nama Bulan
      ...
      const projectSubtotal = Array.isArray(ahspLines)
        ? ahspLines.reduce((acc, l) => acc + (safeNum(l.volume || 0) * safeNum(l.harga_satuan || 0)), 0)
        : (options.scheduleData.reduce((acc, it) => acc + (safeNum(it.volume || 0) * safeNum(it.harga_satuan || it.total_subtotal || 0)), 0) || 1);
      ...
      Object.entries(groupedSched).forEach(([babTitle, items], bIdx) => {
        ...
        items.forEach((item) => {
          ...
          const itemTotalVal = safeNum(item.volume || 0) * safeNum(item.harga_satuan || item.total_subtotal || 0);
          ...
          for (let w = 0; w < itemWeeksSpanned; w++) {
            ...
            if (targetCol <= endCol) {
              wsSched.getCell(currentRow, targetCol).value = itemBobotVal / itemWeeksSpanned;
      ...
      if (Array.isArray(options.progressData) && options.progressData.length > 0) {
        const totalProjectPrice = options.scheduleData.reduce((sum, it) => sum + (safeNum(it.volume || 0) * safeNum(it.harga_satuan || it.total_subtotal || 0)), 0);

        options.progressData.forEach(p => {
          const weekIdx = Math.floor(safeNum(p.day_number) / 7);
          const item = options.scheduleData.find(it => it.id === p.entity_id);
          if (item && totalProjectPrice > 0) {
            const itemPrice = safeNum(item.harga_satuan || item.total_subtotal || 0);
            const valMoney = safeNum(p.val || 0) * itemPrice;
      ...
      for (let c = startCol; c <= endCol; c++) {
        ...
        // Rencana Progress Mingguan (Hitung manual dari kolom di baris-baris item)
        let colSum = 0;
        for (let r = 8; r < tRow; r++) {
          const v = wsSched.getCell(r, c).value;
          if (typeof v === 'number') colSum += v;
        }
        wsSched.getCell(`${colLetter}${rPM}`).value = colSum;
        wsSched.getCell(`${colLetter}${rPM}`).numFmt = '0.00%';

        // Kumulatif Rencana
        cumPlan += colSum;
        wsSched.getCell(`${colLetter}${kPM}`).value = cumPlan;
        wsSched.getCell(`${colLetter}${kPM}`).numFmt = '0.00%';

        // AKTUAL PROGRESS MINGGUAN
        ...
        if (actualVal > 0 || cumActual > 0) {
          cumActual += actualVal;
          wsSched.getCell(`${colLetter}${kA}`).value = cumActual;
          wsSched.getCell(`${colLetter}${kA}`).numFmt = '0.00%';
          wsSched.getCell(`${colLetter}${dev}`).value = cumActual - cumPlan;
  ...
  worksheets.forEach(ws => {
    ...
    let hasData = false;
    const startRow = lowName.includes('rab') ? 12 : 6;
    let highestRowWithData = startRow;

    if (lowName === 'cover') {
      hasData = true;
      highestRowWithData = 44;
    } else {
    ...
    if (!hasData) {
      ...
      let lastCol = 'K';
      if (lowName.includes('ahsp')) lastCol = 'O';
      else if (lowName.includes('hsp')) lastCol = 'I';
      else if (lowName.includes('terpakai')) lastCol = 'M';
      else if (lowName === 'harga satuan' || lowName === 'harga_satuan' || lowName === 'harga satuan master') lastCol = 'J';
      else if (lowName.includes('rekap')) lastCol = 'I';

      const pArea = `A1:${lastCol}${lastRow}`;
      let orient = 'portrait';
      if (lowName.includes('schedule') || lowName.includes('kurva-s')) orient = 'landscape';

      setupPrinter(ws, companyName, pArea, paperSize, orient);
    }
  });

  // 2. LAKUKAN PENGHAPUSAN AMAN DI SINI (Penawar Corrupt XML)
  sheetsToRemove.forEach(id => {
    try {
      const wsDelete = workbook.getWorksheet(id);
      if (wsDelete) {
        // UBAH STATUS MENJADI SANGAT TERSEMBUNYI (Trik Anti-Corrupt)
        wsDelete.state = 'veryHidden';
      }
      // DILARANG MENGGUNAKAN workbook.removeWorksheet(id);
    } catch (e) { }
  });
  ...
  a.href = url;
  const finalFileName = options.fileName || `Laporan_Proyek_${project.name || 'Export'}`;
