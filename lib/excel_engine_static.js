...
const generateProjectReport = async (project, user, ahspLines, selectedSheets, options = {}) => {
  ...
  worksheets.forEach(ws => {
    ...
    // TRIK ANTI CORRUPT: Sembunyikan, jangan dihapus
    if (!isSelected) {
      ws.state = 'veryHidden';
      return;
    }
    ...
    // TRIK ANTI CORRUPT: Sembunyikan sheet yang kosong
    if (!hasData) {
      ws.state = 'veryHidden';
    } else {
  ...
  // Fix "Ghost Active Tab" bug that corrupts MS Excel XML
  let firstVisibleTab = 0;
  for (let i = 0; i < workbook.worksheets.length; i++) {
    if (workbook.worksheets[i].state !== 'hidden' && workbook.worksheets[i].state !== 'veryHidden') {
      firstVisibleTab = i;
      break;
    }
  }
  workbook.views = [{ activeTab: firstVisibleTab }];
